...
/**
 * /**A* for line Arrangement : Path finder. Calculate a line from start to end with A*.
 * Deprecated. Kept only for reference.
 * A path finder implementation that uses the AStar heuristic based algorithm
 * to determine ONE path. 
 * 
 * @author Kevin Glass
 */
public class AStarPathFinder implements PathFinder {
				...
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(sx,sy,xp,yp)) {
						...
						if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
							...
							neighbour.heuristic = getHeuristicCost(current.x, current.y, xp, yp, tx, ty);
	 ...
	 */
	public float getHeuristicCost( int curX, int curY, int x, int y, int tx, int ty) {
		return heuristic.getCost(map, curX, curY, x, y, tx, ty);
	}
