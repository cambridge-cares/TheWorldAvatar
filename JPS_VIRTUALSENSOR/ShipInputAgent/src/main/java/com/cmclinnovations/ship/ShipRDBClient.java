...
public class ShipRDBClient {
    ...
    void createIriLookUpTable(Connection conn) {
        try (CreateTableColumnStep createStep = getContext(conn)
            ...
            createStep.column(SHIP_IRI_COLUMN).column(LOCATION_IRI_COLUMN).execute();
        }
    }

    void populateTable(List<Ship> ships, Connection conn) {
        InsertValuesStep2<Record, String, String> insertValueStep = getContext(conn).insertInto(SHIP_IRI_TABLE,
                SHIP_IRI_COLUMN, LOCATION_IRI_COLUMN);
        ships.stream().forEach(ship -> insertValueStep.values(ship.getIri(), ship.getLocationMeasureIri()));
