! <filter_points.for - A component of the City-scale
...
!***********************************************************************
      ...
      subroutine filter_points(incols,npoints,nhours,     &
                           snap,data_array,npoints_new,filter_snap,filter_array) 

!***********************************************************************
!***  Subroutine filters point sources that are outside of
!***  of the model domain defined by the user
!***********************************************************************
...
!out
      integer, dimension(npoints),intent(out)     :: filter_snap
      real, dimension(npoints,incols),intent(out) :: filter_array
      integer, intent(out)            :: npoints_new

!     Local declarations:

      integer            :: n,k
      real, parameter    :: buffer = 1000
      real               :: xcor
      real               :: ycor
...
! counter for filter array
      ...
      do n = 1, npoints
        ...
        ! get point source coordinates
        xcor = data_array(n,1)
        ycor = data_array(n,2)
        if (xcor > 1.e6) then
          call stopit('Point source UTM x-coordinate has to be without leading digits for UTM zone')
        endif

        if ( (xcor.lt.borderw) .or. &
             (xcor.gt.bordere) .or. &
             (ycor.lt.borders) .or. &
             (ycor.gt.bordern)       ) then
! *** These point sources will not be included
          if (fe_log) then
            write(funit_log,'(1X,A62,I5,F11.1,F11.1)')  &
              'Point source outside domain - excluded. Input point no. x y', n, xcor, ycor
          endif
       else
! *** These point sources will be included
