...
C
     ...
     *            T1, T2, TL1, TL2, TLIST, TLIST2, TN1, TNUM, TNUM2, UTM
     ...
     +                 UEAST, UNORTH, X, XCOMP, Y, YCOMP
      ...
      CHARACTER(len=300) FNAME,INPFIL,OUTFIL,SUMFIL
      INTEGER ISTATUS
      LOGICAL :: OUTFILEXISTS,SUMFILEXISTS  
...
C  READ THE INPUT FILE TO FIND THE MAXIMUM VALUES
      ...
      IF (ISTATUS /= 0) INPFIL=FNAME
      CALL GET_COMMAND_ARGUMENT(2,FNAME,ISTATUS)
      IF (ISTATUS /= 0) OUTFIL=FNAME
      CALL GET_COMMAND_ARGUMENT(3,FNAME,ISTATUS)
      IF (ISTATUS /= 0) SUMFIL=FNAME

      OPEN(UNIT=10,FILE=INPFIL,STATUS="OLD")
      
      INQUIRE(file=OUTFIL, exist=OUTFILEXISTS)
      if (OUTFILEXISTS) then
        OPEN(UNIT=12,FILE=OUTFIL,STATUS="REPLACE")
      ELSE
        OPEN(UNIT=12,FILE=OUTFIL,STATUS="NEW")
      end if

      INQUIRE(file=SUMFIL, exist=SUMFILEXISTS)
      if (SUMFILEXISTS) then
        OPEN(UNIT=14,FILE=SUMFIL,STATUS="REPLACE")
      ELSE
        OPEN(UNIT=14,FILE=SUMFIL,STATUS="NEW")
      end if
