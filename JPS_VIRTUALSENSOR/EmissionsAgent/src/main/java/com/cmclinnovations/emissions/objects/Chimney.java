...
public class Chimney {
    ...
    public Chimney(JSONObject result) {
    	...
    	for (int i = 0; i < numpar; i++) {
    		...
    		particles[i].setFlowrate(particlesArray.getJSONObject(i).getJSONObject("emission_rate").getDouble(VALUE_STRING));
    		particles[i].setDiameter(particlesArray.getJSONObject(i).getJSONObject("diameter").getDouble(VALUE_STRING)*1e-9); // convert from nm to m
    	...
    	for (int i = 0; i < numpol-1; i++) {
			...
			// NOx is sum of NO2 and NO, the others are obtained directly from python results
