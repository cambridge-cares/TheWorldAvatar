...
class ADMSMapSpeedTorqueNOxSoot(object):
    ...
    def __init__(self):
        ...
        # self.nox_surr = self.deserialise_surrogate('C:\\Users\\Kok Foong Lee\\Documents\\GitHub\\TheWorldAvatar\\JPS_VIRTUALSENSOR\\python_service\\speedLoadMap\\NOx_surr.bin') # path defined in Dockerfile
        self.nox_surr = self.deserialise_surrogate('/app/speedLoadMap/NOx_surr.bin')
        self.soot_surr = self.deserialise_surrogate('/app/speedLoadMap/Soot_surr.bin')
...
def calcEmissions(speed, torque):
    ...
    return  processor.get_adms_map(speed, torque)
...
if __name__ =="__main__":
    ...
    # print to std out which will be read by the calling code
    print(json.dumps(processor.get_adms_map(float(speed), float(torque))))