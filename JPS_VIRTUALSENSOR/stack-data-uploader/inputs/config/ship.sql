...
WITH distinct_mmsi AS (
SELECT DISTINCT "MMSI", "VesselType"
FROM "rawship"
),
time_series AS (
...
LEFT JOIN "rawship" rs
ON rs."MMSI" = dm."MMSI"
AND rs."BaseDateTime" >= ts.interval_start
AND rs."BaseDateTime" < ts.interval_start + '30 minutes'::interval
GROUP BY dm."MMSI", ts.interval_start, dm."VesselType"
ORDER BY dm."MMSI", ts.interval_start;

-- clean up NULL entries

DELETE FROM "ship"
WHERE "LAT" IS NULL OR "LON" IS NULL OR "SOG" IS NULL OR "COG" IS NULL;
