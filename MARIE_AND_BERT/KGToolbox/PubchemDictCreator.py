...
path = os.path.join(DATA_DIR, 'pubchem.csv')
df_pubchem = pd.read_csv(path, sep=',')[0:5001]
...
# with open(os.path.join(DEPLOYMENT_DIR, 'name_list.json'), 'w') as f:
#     f.write(json.dumps(name_list))
#     f.close()
#
#
# with open(os.path.join(DEPLOYMENT_DIR, 'cid_dict.json'), 'w') as f:
...
VALUE_DICT_PATH = os.path.join(DEPLOYMENT_DIR, 'pubchem_value_dict.pkl')
