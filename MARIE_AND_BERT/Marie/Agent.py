...
class AgentInterface():
    ...
    def run(self):
        ...
        my_matcher = QuestionAgentMatcher(question=self.question)
        #The agent to be called and the output requested from that agent
        agent, output = my_matcher.run()

        #Invoke the agent found
        if(agent != None):
            my_invoker = AgentInvoker(agent=agent, output=output, species_iri="http://www.theworldavatar.com/kb/ontospecies/Species_2fb70cfe-6707-4d6c-b977-bae913c4878f")
