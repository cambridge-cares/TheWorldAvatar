...
def run(weather_data = r"weather_data.csv",meteo_data = r"test.met",apl_data = r"test.APL"):
	...
	# The following code creates the emissions file for ADMS
	# The current emissions are read and varied
	storage = []
	f = open(apl_data, 'r+')
	for line in f:
		if re.search('MetDataFileWellFormedPath', line) != None:
			line = 'MetDataFileWellFormedPath = '+meteo_data+'\n'
			# line = 'MetDataFileWellFormedPath = test2.met\n'
			storage.append(line)
			continue
			
		if re.search('SrcPolEmissionRate', line) != None:
			storage.append(line)
			line = '  '+str(round(float(f.readline())*(1+((random.random()-0.5)*2)*0.2),2))+'\n'
			storage.append(line)
			continue
		storage.append(line)
	f.seek(0)
