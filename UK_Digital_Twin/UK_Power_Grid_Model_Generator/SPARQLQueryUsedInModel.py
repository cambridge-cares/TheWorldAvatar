...
def queryEGenInfo(topology_Endpoint, powerPlant_Endpoint, topoAndConsumpPath_Sleepycat, powerPlant_Sleepycat, localQuery):
    ...
    if localQuery == False and topology_Endpoint != None and powerPlant_Endpoint != None: 
        ...
        for q in qres:
            ...
            res = (json.loads(performQuery(powerPlant_Endpoint, queryStr_capacityAndPrimaryFuel)))[0]
            print('query is done')
            capa_PrimaryFuel.append([float(res['Capacity']), (str(res['PrimaryFuel']).split('#'))[1]])
...
# TODO: enable the federated query string
def queryEBusandRegionalDemand(topology_Endpoint, energyConsumption_Endpoint, SleepycatPath, localQuery):
    queryStr = """
    PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
