...
def plot_LSOA_temps(LSOA):
    ...
    for i in range(len(results)):
        if (
            results[i, 0]
            == "http://www.theworldavatar.com/kb/ontogasgrid/climate_abox/tasmin"
        ):
            min_temp.append(float(results[i, -1]))
            datetime_min.append(
                datetime.strptime(results[i, 1], "%Y-%m-%dT%H:%M:%S.000Z")
            )
        elif (
            results[i, 0]
...
for j in tqdm(range(len(elec_meters_results[:, 0]))):
    ...
    try:
        ...
        print(
            "No electricity meter data for ", elec_meters_results[j, 0].split("/")[-1]
        )
...
for i in range(len(gas_tensor)):
    for j in range(len(months)):
        monthly_gas_tensor[i, j] = (
            gas_tensor[i] * monthly_total_gas_demand[j] / total_uk_demand
        )
...
for i in range(len(elec_tensor)):
    for j in range(len(months)):
        monthly_elec_tensor[i, j] = (
            elec_tensor[i] * monthly_total_elec_demand[j] / total_uk_elec_demand
        )
...
def return_geo_df(month, uptake, temp_var_type):
    ...
    scaled_delta_elec = (delta_elec_values - np.mean(delta_elec_values)) / (
        np.std(delta_elec_values)
    )
    scaled_fuel_pov = (poverty_values - np.mean(poverty_values)) / (
        np.std(poverty_values)
    )
    inequality = scaled_delta_elec - scaled_fuel_pov
    df["Inequality Index"] = list(np.around(inequality, decimals=3))
    df["Emissions"] = (np.array(remaining_gas_values) * gas_co) + (
        np.array(remaining_elec_values) * elec_co
    )
    df["Change in Emissions"] = df["Emissions"].to_numpy() - (
        df["Gas"].to_numpy() * gas_co + df["Electricity"].to_numpy() * elec_co
    )
...
def dataframe_construction(temp_var_type, uptake, month, df_box, complete_df):
    ...
    new_df["Emissions"] = (np.array(remaining_gas_values) * gas_co) + (
        np.array(remaining_elec_values) * elec_co
    )
    new_df["Change in Emissions"] = new_df["Emissions"].to_numpy() - (
        new_df["Gas"].to_numpy() * gas_co + new_df["Electricity"].to_numpy() * elec_co
    )
...
def ret_cop_month_with_temps(var, LSOA):
    ...
    for j in range(len(temp_vars)):
        ...
        for i in tqdm(range(12)):
            df_box, complete_df = dataframe_construction(
                temp_var_type, uptake, i, df_box, complete_df
            )
        ...
        plt.plot(
            np.arange(len(var_val)),
            var_val,
            c="k",
            linestyle=styles[j],
            label=temp_vars_label[j],
        )
...
newcolors = np.vstack(
    (top(np.linspace(0, 1, 128)), np.flip(top(np.linspace(0, 1, 128)), axis=0))
)
...
def plot_variables(month, uptake, temp_var_type, line_var, LSOA1, LSOA2):
    ...
    cax = fig.add_axes([0.1, 0.06, 0.8, 0.03])
    cax2 = fig.add_axes([0.1, 0.15, 0.8, 0.03])
    plt.subplots_adjust(bottom=0.248, top=0.935)
    keys = ["A", "B", "C"]
    for i in range(len(temp_vars)):
        ...
        boundary = [
            min(boundary.values[:, 0]),
            min(boundary.values[:, 1]),
            max(boundary.values[:, 2]),
            max(boundary.values[:, 3]),
        ]
    ...
    axs["E"].vlines(
        month, median_store1[month, 0], median_store1[month, 2], color="r", alpha=0.4
    )
    axs["F"].vlines(
        month, median_store2[month, 0], median_store2[month, 2], color="r", alpha=0.4
    )
    ...
    ptE = figtr.transform(ax1tr.transform((-2.1e3, 6.8e6)))
    arrow = matplotlib.patches.FancyArrowPatch(
        ptB,
        ptE,
        transform=fig.transFigure,
