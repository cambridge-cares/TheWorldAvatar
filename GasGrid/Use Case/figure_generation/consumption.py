...
def return_geo_df(month,uptake,temp_var_type):
    ...
    for i in range(len(gas_values)):
        ...
        elec_values[i] = sum(monthly_elec_tensor[i,:])/elec_meters_tensor[i]
...
def plot_variables(vars,var_names,inset,month,uptake,temp_var_type):
    ...
    for i in range(len(vars)):
        ...
        val_values = my_geo_df[vars[i]].values
        ...
        tl  = my_geo_df.plot(column=vars[i],cmap=color_theme,\
            ...
            norm = divnorm,\
            cax=cax1,
            legend_kwds={'label':'kWh/year/household'})   
        ...
        axs.set_title(var_names[i],y=1.08)
        ...
        plt.savefig('figure_output/'+vars[i].lower()+'_consumption'+'.png',dpi=200) 
        plt.savefig('figure_output/'+vars[i].lower()+'_consumption'+'.pdf') 
