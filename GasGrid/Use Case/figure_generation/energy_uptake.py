...
def dataframe_construction(temp_var_type,uptake):
    ...
    for i in range(len(gas_values)):
        ...
        gas_values[i] = sum(monthly_gas_tensor[i,:])
        # assigning elec consumption 
        elec_values[i] = sum(monthly_elec_tensor[i,:])
    ...
    new_df['Remaining Elec']    = list(np.around(remaining_elec_values,decimals=3))
...
def plot_var_annual_uptake_with_temps():
    ...
    for u in range(len(uptakes)):
        for j in range(len(temp_vars)):
            ...
            gas_sum = complete_df['Remaining Gas'].values
            elec_sum = complete_df['Remaining Elec'].values
    ...
    print('PERCENTAGE DIFFERENCE WITHIN BOUNDS (ELEC): ',((results_elec[-1,0]-results_elec[-1,2])/results_elec[-1,1])*100)
    print('FINAL VALUE (ELEC): ',results_elec[-1,1])

    plt.savefig('figure_output/a_'+str(alpha)+'n_'+str(nb)+'/energy_uptake.pdf')
    plt.savefig('figure_output/a_'+str(alpha)+'n_'+str(nb)+'/energy_uptake.png',dpi=200)
