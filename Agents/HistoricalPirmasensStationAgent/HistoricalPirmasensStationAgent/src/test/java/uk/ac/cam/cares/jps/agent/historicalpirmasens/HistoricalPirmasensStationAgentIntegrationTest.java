...
@Testcontainers
public class HistoricalPirmasensStationAgentIntegrationTest {
    ...
    private HistoricalPirmasensStationAgent agent;
   ...
   private final String[] keys = {"Niederschlag","Ozon","eBC_PM2-5","Gesamt_UV_Strahlung","Stickstoffdioxid","Relative_Feuchte","Luftdruck", "Temperatur" ,"Windrichtung_rohwert", "PM10", "Stickstoffmonoxid", "Windgeschwindigkeit_rohwert"};
    ...
    public void initializeAgent() throws IOException {
        ...
        try {
            SystemLambda.withEnvironmentVariable("TEST_MAPPINGS", mappingFolder.getCanonicalPath()).execute(() -> {
                agent = new HistoricalPirmasensStationAgent(propertiesFile);
            });
        }
        catch (Exception e) {
        	throw new JPSRuntimeException("Encountered an error while using SystemLamda.withEnvironmentVariable");
        }
        // Create agent
    ...
    public void testUpdateTimeSeries() {
        ...
        // Assert timestamps
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[0]), zoneOffSet)
                .isEqual(tsClient.getMinTime(IRIs.get(0))));
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[timestamps.length-1]), zoneOffSet)
