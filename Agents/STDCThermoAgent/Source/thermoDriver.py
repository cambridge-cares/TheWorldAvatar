...
def usage():
    usagemsg = """ Usage:
    thermoDriver.py -x <xml_file> [OPTIONS]
...
def main(argv):
    try:
        opts, args = getopt.getopt(argv,"hx:d:c:",["help", "xml_file=", "out_datfile=", "out_csvfile=", "tfit=", \
            "tcsv=", "href="])
    ...
    if len(xml_file) == 0: #or 
        # exists
        printErrorCode(0,**{'file':xml_file})
    elif os.path.isfile(xml_file) == False:
        printErrorCode(1, **{'file':xml_file})

    
    chs.RunThermoScriptFromCmd(xml_file,out_datfile,out_csvfile,tfit_range,tcsv_range,href)
