...
line AS (
    SELECT 
        ts.time as time,
        LAG(ts.geom) OVER (PARTITION BY device_id ORDER BY ts.time) AS prev_geom,
        ST_MakeLine(LAG(ts.geom) OVER (PARTITION BY device_id ORDER BY ts.time), ts.geom) AS geom,
        ts.speed AS speed,
