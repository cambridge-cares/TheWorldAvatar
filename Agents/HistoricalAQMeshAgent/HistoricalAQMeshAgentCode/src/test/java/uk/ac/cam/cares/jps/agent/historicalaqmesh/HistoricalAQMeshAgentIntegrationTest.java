...
@Testcontainers
public class HistoricalAQMeshAgentIntegrationTest {
    ...
    // AQMesh in[ut agent
    private HistoricalAQMeshAgent agent;
    ...
    public void initializeAgent() throws IOException {
        ...
        try {
        	SystemLambda.withEnvironmentVariable("TEST_MAPPINGS", mappingFolder.getCanonicalPath()).execute(() -> {
        		 agent = new HistoricalAQMeshAgent(propertiesFile);
        	 });
        }
        catch (Exception e) {
        } 
    ...
    public void testUpdateTimeSeries() {
        ...
        // Assert timestamps
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[0]), HistoricalAQMeshAgent.ZONE_OFFSET)
                        .isEqual(tsClient.getMinTime(gasIRIs.get(0))));
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[0]), HistoricalAQMeshAgent.ZONE_OFFSET)
                        .isEqual(tsClient.getMinTime(particleIRIs.get(0))));
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[timestamps.length-1]), HistoricalAQMeshAgent.ZONE_OFFSET)
    ...
    public void testUpdateTimeSeriesWithPruning() {
        ...
        for(int i = 0; i < timestamps.length ; i++) {
            if (i < (timestamps.length-1)) {
                times.add(OffsetDateTime.of(LocalDateTime.parse(timestamps[i]), HistoricalAQMeshAgent.ZONE_OFFSET));
            }
        }
