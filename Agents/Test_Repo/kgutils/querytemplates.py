...
def output_query_template(keyword: str, year: str):
    '''
    ...
    ?meteriri <{GAS_HAS_CONSUM_ELECMETERS}> ?meter;
              <{COMP_HAS_STARTUTC}>  "{year + "-01-01T12:00:00Z"}"^^<{XSD_DATETIME}>.
    ?energy <{OM_HAS_PHENO}> ?elec;
            <{OM_HAS_VALUE}> ?usageiri.
    ?usageiri <{OM_HAS_NUMERICALVALUE}> ?usage.
    ?elec <{COMP_HAS_STARTUTC}>  "{year + "-01-01T12:00:00Z"}"^^<{XSD_DATETIME}>.
    """
    ...
    if keyword == 'Gas':
        ...
        ?energy <{OM_HAS_PHENO}> ?gas;
                 <{OM_HAS_VALUE}> ?usageiri.
        ?usageiri <{OM_HAS_NUMERICALVALUE}>  ?usage.
        ?gas      <{COMP_HAS_STARTUTC}>  "{year + "-01-01T12:00:00Z"}"^^<{XSD_DATETIME}>.
    ...
    FILTER (regex(str(?start), "{year}-\\\d\\\d-01T12:00:00Z") && datatype(?start) = xsd:dateTime)
        """

    if keyword == 'Fuel poverty':
        query+= f"""  <{OFP_HASHOUSEHOLD}> ?housesiri.
     ?housesiri <{OFP_FUELPOOR}> ?a;
             <{OFP_VALIDFROM}> "{year + "-01-01T12:00:00Z"}"^^<{XSD_DATETIME}>;
