...
import com.cmclinnovations.jps.agent.configuration.KineticsAgentProperty;
...
public class OntoKinKG {
	 ...
	 * @throws IOException
	 */
	public static String getThermoModelsOfSpecies(String speciesIRI, KineticsAgentProperty kineticsAgentProperty) throws IOException{
		String query = formThermoModelQuery(speciesIRI);
		System.out.println("Query:"+query);
		String httpURL = kineticsAgentProperty.getOntoKinRepositoryIRI().concat(URLEncoder.encode(query, "UTF-8").concat("&Accept=application/sparql-results%2Bjson"));
		for(int i=0;i<5;i++){
		try{
	 ...
	 * @throws IOException
	 */
	public static void deleteThermoModelsOfSpecies(String triple, KineticsAgentProperty kineticsAgentProperty) throws IOException{
		String tokens[] = triple.split(" ");
		if(tokens.length>=2){
			removeThermoModel(kineticsAgentProperty.getRdf4jServerURL(), kineticsAgentProperty.getOntoKinRepositoryID(), tokens[0], tokens[1], null);
		}
	}
	 ...
	 * 
	 * @param triple
	 * @param kineticsAgentProperty
	 * @throws IOException
	 */
	public static void insertThermoModelsOfSpecies(String triple, KineticsAgentProperty kineticsAgentProperty) throws IOException{
		String tokens[] = triple.split(" ");
		if(tokens.length>=2){
			triple = triple.replaceFirst(tokens[0].concat(" "), "");
			triple = triple.replaceFirst(tokens[1].concat(" "), "");
			addThermoModel(kineticsAgentProperty.getRdf4jServerURL(), kineticsAgentProperty.getOntoKinRepositoryID(), tokens[0], tokens[1], triple.replaceAll(tokens[0], triple));
		}
	}
