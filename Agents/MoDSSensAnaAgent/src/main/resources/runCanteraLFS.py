...
class interrupter(object):
    ...
    def __call__(self, _):
        tnow = timer()
        if tnow - self.start > self.tmax:
            raise Exception('Max simulation wall time exceeded.')
        return 0
...
# Setup the initial guess of temperature profile
flame.set_initial_guess(locs=[0.0, 0.7, 0.8, 1.0])
...
flame.set_interrupt(interrupter(timer(), 600))
