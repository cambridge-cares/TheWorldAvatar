...
public class ESPHomeUpdateAgentLauncher extends JPSAgent {
    ...
    public JSONObject processRequestParameters(JSONObject requestParams) {
      ...
      if (validateInput(requestParams)) {
        	LOGGER.info("Passing request to ESPHome Update Agent..");
 ...
 // TODO: Use proper argument parsing
    ...
    public static JSONObject initializeAgent(String[] args) {
        ...
        // Create the agent
        ESPHomeUpdateAgent agent;
        try {
            agent = new ESPHomeUpdateAgent(args[0]);
        } catch (IOException e) {
        ...
        if(!statusAndTimeStamp.isEmpty()) {
            // Update the data
            agent.updateData(statusAndTimeStamp);
