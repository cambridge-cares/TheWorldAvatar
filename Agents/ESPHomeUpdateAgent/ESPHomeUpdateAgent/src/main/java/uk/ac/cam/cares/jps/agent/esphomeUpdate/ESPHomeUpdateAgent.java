...
/**
 * Class to retrieve data from the ESPHome API and storing it with connection to The World Avatar (Knowledge Base).
 * @author  */
public class ESPHomeUpdateAgent{
     ...
     */
    public static final String generatedIRIPrefix = TimeSeriesSparql.TIMESERIES_NAMESPACE + "esphome";
    /**
     * The time unit used for all time series maintained by the esphome agent
     */
     ...
     */
    public ESPHomeUpdateAgent(String propertiesFile) throws IOException {
    ...
    private void readMappings(String mappingFolder) throws IOException {
        ...
        // Create a mapper for each file
            ...
            for (File mappingFile: mappingFiles) {
                JSONKeyToIRIMapper mapper = new JSONKeyToIRIMapper(ESPHomeUpdateAgent.generatedIRIPrefix, mappingFile.getAbsolutePath());
                mappings.add(mapper);
    ...
    public void updateData(JSONObject statusAndTimeStamp)throws IllegalArgumentException {
    	...
    	Map<String, List<?>> timeStampReadingsMap = jsonObjectToMapForTimeStamp(statusAndTimeStamp);
    	Map<String, List<?>> statusReadingsMap = jsonObjectToMap(statusAndTimeStamp);
        
        
        // Only do something if all readings contain data
        if(!statusReadingsMap.isEmpty() && !timeStampReadingsMap.isEmpty()) {
            List<TimeSeries<OffsetDateTime>> timeSeries;
            try {
                timeSeries = convertReadingsToTimeSeries(statusReadingsMap, timeStampReadingsMap);
            }
    ...
    private OffsetDateTime convertStringToOffsetDateTime(String timestamp) {
        ...
        return OffsetDateTime.of(localTime, ESPHomeUpdateAgent.ZONE_OFFSET);
    }
