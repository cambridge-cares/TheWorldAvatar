...
public class ESPHomeUpdateAgentIntegrationTest {
    ...
    private final String[] timestamps = new String[1];
    ...
    public void initializeAgent() throws IOException {
        ...
        try {
        	SystemLambda.withEnvironmentVariable("TEST_MAPPINGS", mappingFolder.getCanonicalPath()).execute(() -> {
        		 agent = new ESPHomeUpdateAgent(propertiesFile);
        	 });
        }
        catch (Exception e) {
        } 
    ...
    public void testUpdateTimeSeries() {
        ...
        // Assert timestamps
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[0]), ESPHomeUpdateAgent.ZONE_OFFSET)
                        .isEqual(tsClient.getMinTime(IRIs.get(0))));
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[timestamps.length-1]), ESPHomeUpdateAgent.ZONE_OFFSET)
    ...
    public void testUpdateTimeSeriesWithPruning() {
        ...
        for(int i = 0; i < timestamps.length ; i++) {
            if (i < (timestamps.length-1)) {
                times.add(OffsetDateTime.of(LocalDateTime.parse(timestamps[i]), ESPHomeUpdateAgent.ZONE_OFFSET));
            }
        }
