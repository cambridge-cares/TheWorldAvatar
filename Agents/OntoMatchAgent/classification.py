...
class TrainTestGenerator():
    ...
    def generate_sim_vectors_for_matches_and_split(params, tgt_file, train_sizes):
        ...
        dframe = util.read_csv(match_file)
        ...
        blocking.create_iterator(srconto, tgtonto, params_blocking)
...
def calculate_scores_for_index_set(df1, df2, params_mapping, index_set):
    ...
    score_manager = scoring.ScoreManager(df1, df2, pair_iterator=pair_iterator)
...
def filter_scores_for_index_set(df_scores, number=None, rate=None, mode='random', center=0, p=2, radius=1):
    ...
    columns = util.get_prop_columns(df_scores)
    if center == 0:
        if p == 'max':
...
def select_samples_XXX(df1, df2, params_mapping, m_mode, n_mode, match_index=None, nonmatch_index=None, split=[.8, .2], missing_value=None):
    pass
