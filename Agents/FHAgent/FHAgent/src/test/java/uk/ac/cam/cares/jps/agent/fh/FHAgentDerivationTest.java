...
public class FHAgentDerivationTest {
    ...
    private static final String W3 = "http://www.w3.org/2006/";
    ...
    private static final Prefix P_W3 = SparqlBuilder.prefix("w3", iri(W3));

    private static final Iri Service = P_AGENT.iri("MSM.owl#Service");
    private static final Iri hasTimeSeries = P_TS.iri("hasTimeSeries");
    private static final Iri hasHttpUrl = P_AGENT.iri("MSM.owl#hasHttpUrl");
    private static final Iri hasTime = P_W3.iri("time#hasTime");
    ...
    public void initializeAgent() throws IOException {
        ...
        String[] clientPropParam = {"db_url="+jdbcURL, "db.user="+postgresUsername, "db.password="+postgresPassword, "sparql.query.endpoint="+sparql_endpoint};
    ...
    public void testInstantiateAgent() {
        ...
        queryURL.where(queryURL.var().has(hasHttpUrl, queryURL.var()));

        JSONArray queryResult = storeClient.executeQuery(queryURL.getQueryString());
        //Should only have 1 FHAgent instance; 1 column for iri, 1 for url
        Assert.assertTrue(queryResult.length() == 2);
        JSONObject column = queryResult.getJSONObject(1);
        String agentIRI = column.getString(column.keys().next());
        Assert.assertEquals("http://localhost:1010/fh-agent/instantiate", agentIRI);

        //Check if agent is instantiated
        SelectQuery query = Queries.SELECT();
        query.where(query.var().has(RDF.TYPE, Service));
        Assert.assertTrue(storeClient.executeQuery(query.getQueryString()).length() > 1);

        //Check raw and derived var instances
        //Derived var has timeseries
        SelectQuery queryDeriv = Queries.SELECT();
        queryDeriv.where(queryDeriv.var().has(hasTimeSeries, queryDeriv.var()));
        //Must be equal to the number of derived variables
        Assert.assertTrue(storeClient.executeQuery(queryDeriv.getQueryString()).length() == testAgent.getNumberOfTimeSeries());

        //Raw var has timestamp
        SelectQuery queryRaw = Queries.SELECT();
        queryRaw.where(queryRaw.var().has(hasTime, queryRaw.var()));
        //IRI Map contains both derived and raw var
        //The number of raw var instances must be equal to irimap size - # of derived var
        Assert.assertTrue(storeClient.executeQuery(queryRaw.getQueryString()).length() == testDerivator.iriMap.size()-testAgent.getNumberOfTimeSeries());
    }
