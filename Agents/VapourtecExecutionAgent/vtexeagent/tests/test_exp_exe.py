...
def initialise_agent(initialise_triple_store):
    with initialise_triple_store as container:
        ...
        config = ExeAgentConfig(
            str(Path(__file__).absolute().parent) + '/test_conf.json')

        # Initialise DoE agent with temporary docker container endpoint
        exe_agent = VapourtecExecutionAgent(
            config.ONTOAGENT_SERVICE, config.PERIODIC_TIMESCALE, config.DERIVATION_INSTANCE_BASE_URL, endpoint)
        ...
        for f in ['ontoagent/Service__Execution.ttl', 'sample_data/new_exp_data.ttl', 'sample_data/duplicate_ontorxn.ttl', 'sample_data/dummy_lab.ttl', 'sample_data/rxn_data.ttl']:
            data = pkgutil.get_data(
                'chemistry_and_robots', 'resources/'+f).decode("utf-8")
...
def test_exp_exe(initialise_agent, derivation_input):
    ...
    derivation_iri = exe_agent.derivationClient.createAsyncDerivationForNewInfo(
        exe_agent.agentIRI, derivation_input)

    # Check if the derivation instance is created correctly
    assert sparql_client.checkInstanceClass(
        derivation_iri, ONTODERIVATION_DERIVATIONASYN)
    ...
    while currentTimestamp_derivation == 0:
        time.sleep(20)
        currentTimestamp_derivation = int(
            sparql_client.performQuery(query_timestamp)[0]['time'])

    # Wait some arbitrary time until the cleaning up is done by the derivation client
    time.sleep(20)

    response = sparql_client.performQuery("""SELECT ?output WHERE {?output <%s> <%s>}""" % (
