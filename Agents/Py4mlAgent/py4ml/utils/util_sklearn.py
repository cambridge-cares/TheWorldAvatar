...
def train_and_test(x_train, y_train, x_val, y_val, x_test, y_test, model, cross_validation, metric, log_dir,
    ...
    if cross_validation:
        ...
        if not best_trial_retrain:
            ...
            if metric == 'mse':
                result_train = py4ml.utils.util.calculate_metrics(y_train, y_pred_train)
                result_val = py4ml.utils.util.calculate_metrics(y_val, y_pred_val)
                result_test = py4ml.utils.util.calculate_metrics(y_test, y_pred_test)
            else: # accuracy
