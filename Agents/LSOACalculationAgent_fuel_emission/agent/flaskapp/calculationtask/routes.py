...
@calculationtask_bp.route('/api/lsoacalculationagent_fuel_emission/calculation/fuel_emission', methods=['GET'])
def api_calculation_fuel_emission():
    #
    ...
    if 'df_electricity' in query.keys():
        ...
        logger.info('Electricity consumption data recieved, only emission of electricity will be returned...')

        # Get df_gas
    if 'df_gas' in query.keys():
        inputs['df_gas'] = pd.DataFrame.from_dict(query['df_gas'])
        logger.info('Gas consumption data recieved, only emission of electricity will be returned...')
    ...
    if 'df_gas' not in query.keys() and 'df_electricity' not in query.keys():
        logger.error('Both Gas and Eleciricty consumption data are not provided! No emission to determine...')
        raise InvalidInput('Both Gas and Eleciricty consumption data are not provided! No emission to determine...')
    ...
    try: 
        if 'df_gas' in query.keys() and 'df_electricity' in query.keys():
            df_emission_total, df_emission_elec, df_emission_gas = calculating_fuel_emission(inputs['df_electricity'],inputs['df_gas'], \
                                                                            inputs['year'], inputs['annual'] )
            
            output = {'df_emission_total': df_emission_total.to_dict(orient='list'),
                    'df_emission_elec': df_emission_elec.to_dict(orient='list'),
                    'df_emission_gas': df_emission_gas.to_dict(orient='list'),
                    }
        if 'df_gas' in query.keys() and 'df_electricity' not in query.keys():
            df_emission_gas = calculating_single_emission_gas(inputs['df_gas'],inputs['year'], inputs['annual'] )
            output = {
                    'df_emission_gas': df_emission_gas.to_dict(orient='list')
                    }
        if 'df_gas' not in query.keys() and 'df_electricity' in query.keys():
            df_emission_elec = calculating_single_emission_elec(inputs['df_electricity'],inputs['year'], inputs['annual'] )
            output = {
                    'df_emission_elec': df_emission_elec.to_dict(orient='list')
                    }
