...
def api():
    ...
    if device.__contains__('weatherStation'):
        ...
        try:
            dni_iri = QueryData.query_direct_normal_irradiance(iri)
        except Exception as ex:
            logging.error("SPARQL query for global horizontal irradiance IRI not successful")
            raise KGException("SPARQL query for global horizontal irradiance IRI not successful.") from ex

        wind_speed = query_all_timeseries(wind_speed_iri)
        logging.info("Windspeed: " + str(wind_speed))
        air_temperature = query_all_timeseries(air_temperature_iri)
        logging.info("air_temperature: " + str(air_temperature))
        dni = query_all_timeseries(dni_iri)
        logging.info("dni: " + str(dni))
        try:
            wind_dates = [d.toString() for d in wind_speed.getTimes()]
            air_temperature_dates = [d.toString() for d in air_temperature.getTimes()]
        ...
        for i in range(len(wind_speed_values)):
            print(str(wind_speed_values[i]) + ' m/s at this timing: ' + wind_dates[i])
            print(str(air_temperature_values[i]) + ' degree celsius at this timing: ' + air_temperature_dates[i])
            print(str(dni_values[i]) + ' W/m^2 at this timing: ' + dni_dates[i])
        iri_list = check_data_iris.check_data_iris_and_create_if_not_exist(PROPERTIES_FILE)
        ...
        try:
            ...
            if not boolean_value:
                timeseries_instantiation.init_timeseries(iri_list, [DATACLASS, DATACLASS], TIME_FORMAT)
                timeseries_instantiation.link_to_NTU_KG(iri_list)
        except Exception as ex:
            logging.error("Unable to initialise timeseries")
            raise TSException("Unable to initialise timeseries") from ex
