...
public class HeatEmissionQuery extends JSONObject {
    private static final Logger LOGGER = LogManager.getLogger(HeatEmissionQuery.class);

    // String inputBounds_sample =
    // "{\"job\":{\"lower_bounds\":\"8464.256074442204#23588.08319044689#0\",\"upper_bounds\":\"17619.669922658715#30520.376177137474#105\"}}\r\n";
    /**
     * This part is the main method that takes input form HeatEmissionAgent and pass
     * them in the
     * beow mentioned queries. The query in "jibusinessunits" returns the lists of
     * chemical
     * plants, plant items, IRIs, CO2 emission, fuel CEI and thermal efficiency; the
     * query in
     * "jriEPSG24500" helps return the heat emission coordinates after
     * "HeatEmissionCoordinate"
     * calculation. After the calculation of heat emission values based on CO2
     * emission, CEI and
     * thermal efficiency, we return a final list that contains the heat emission
     * values and
     * respective coordinates within a specified region.
     */

    public static JSONObject performCrossDomainQ(JSONObject jsonObject) {
        ...
        for (int i = 0; i < IRIandCO2QueryResult.length(); i++) {
            ...
            if (plantInfoQueryResult.length() == 0) {
                LOGGER.info(ChemPlantName);
                System.out.println(ChemPlantName);
                // throw new JPSRuntimeException(ChemPlantName);
                continue;
            }
