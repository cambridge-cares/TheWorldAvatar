...
import uk.ac.cam.cares.jps.agent.heat.HeatEmissionAgent;
...
public class HeatEmissionAgentTest {
	...
	public void testProcessRequestParameters() throws NoSuchMethodException, Exception {
		...
		Method processRequestParameters = Query.getClass().getDeclaredMethod("processRequestParameters", JSONObject.class);
        
		// Test empty request parameters
		JSONObject requestParams_1 = new JSONObject();
        try {processRequestParameters.invoke(Query, requestParams_1);
        } catch(Exception e) {
