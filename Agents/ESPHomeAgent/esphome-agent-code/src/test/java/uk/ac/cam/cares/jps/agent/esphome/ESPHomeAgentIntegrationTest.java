...
public class ESPHomeAgentIntegrationTest {
    ...
    public void initializeMockTimeSeriesandAgent() throws IOException {
        ...
        writePropertyFile(propertiesFile, Arrays.asList("db.user=postgres", "db.password=cares1010", "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+IRIs.get(0), "esphome.url=test_url", "esphome.threshold=25"));
    ...
    testRequestParams.remove("apiProperties");
    testMessage = new JSONObject();
    testMessage = agent.processRequestParameters(testRequestParams);
    Assert.assertEquals("Unable to validate request sent to the agent.",testMessage.get("message"));
    
    //test missing client properties key in request params
    testRequestParams.remove("clientProperties");
    testRequestParams.put("apiProperties", "TEST_APIPROPERTIES");
    testMessage = new JSONObject();
    testMessage = agent.processRequestParameters(testRequestParams);
    Assert.assertEquals("Unable to validate request sent to the agent.",testMessage.get("message"));
    ...
    try {
    	SystemLambda.withEnvironmentVariable("TEST_CLIENTPROPERTIES", propertiesFile).and("TEST_APIPROPERTIES", propertiesFile).execute((Statement) () -> {
    ...
    public void testArgumentMismatch() throws IOException {
    	...
    	try {
    		...
    		Assert.fail();
            	...
            	Assert.assertEquals(e.getMessage(),"Need two properties files in the following order:1) time series client 2) API.");
        }
    ...
    public void getTimeSeriesIRIError() throws IOException {
    	...
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	try {
    		agent.initializeAgent(args);
    		Assert.fail();
    	} catch (JPSRuntimeException e) {
    ...
    public void esphomeAPIConstructorError() throws IOException {
    	...
    	try {
    		...
    		Assert.assertEquals("Could not construct ESPHomeAPI needed to send POST and GET request to the ESPHome web server.", e.getMessage());
    	}	
    }
    ...
    public void getLatestDataError() throws IOException {
        ...
        writePropertyFile(propertiesFile, Arrays.asList("db.user=postgresql", "db.password=wrong_password", "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=25"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	try {
    		agent.initializeAgent(args);
    		Assert.fail();
    ...
    @Test
    public void esphomeAPICheckStatusError() throws IOException {
    	String propertiesFile = Paths.get(folder.getRoot().toString(), "all.properties").toString();
        writePropertyFile(propertiesFile, Arrays.asList("db.user="+postgres.getUsername(), "db.password="+postgres.getPassword(), "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=25"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	try {
    		agent.initializeAgent(args);
    		Assert.fail();
    	}catch (JPSRuntimeException e) {
    		Assert.assertEquals("Could not establish connection with ESPHome web server to check status.", e.getMessage());
    	}	
    }
    
    
    @Test
    public void testEspHomeSwitchControlSwitchIsAlreadyOn() throws IOException {
    	String propertiesFile = Paths.get(folder.getRoot().toString(), "all.properties").toString();
        writePropertyFile(propertiesFile, Arrays.asList("db.user="+postgres.getUsername(), "db.password="+postgres.getPassword(), "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=25"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	JSONObject message = new JSONObject();
    	message.put("message", "The switch is already in the ON state.");
    	try(MockedConstruction<ESPHomeAPI> ignored = Mockito.mockConstruction(ESPHomeAPI.class,
                (mock, context) -> {
                    Mockito.when(mock.checkStatus()).thenReturn(true);
                    //latest value should be 26.5 according to addMockTimeSeriesData()
                    //26.5 > threshold value of 25, status of switch = true
                    Mockito.when(mock.esphomeSwitchControl(26.5, true)).thenReturn(message);
                })) {
    		JSONObject result = agent.initializeAgent(args);
    		Assert.assertEquals("The switch is already in the ON state.", result.getJSONArray("message").get(0));
    		Assert.assertEquals("A request has been successfully sent to the ESPHome web server.", result.getJSONArray("message").get(1));
    }
    }
    
    @Test
    public void testEspHomeSwitchControlTurnOnSwitch() throws IOException {
    	String propertiesFile = Paths.get(folder.getRoot().toString(), "all.properties").toString();
        writePropertyFile(propertiesFile, Arrays.asList("db.user="+postgres.getUsername(), "db.password="+postgres.getPassword(), "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=25"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	JSONObject message = new JSONObject();
    	...
    	try(MockedConstruction<ESPHomeAPI> ignored = Mockito.mockConstruction(ESPHomeAPI.class,
                (mock, context) -> {
                    Mockito.when(mock.checkStatus()).thenReturn(false);
                    //26.0 > threshold, status of switch = false
                    Mockito.when(mock.esphomeSwitchControl(26.5, false)).thenReturn(message);
                })) {
    		JSONObject result = agent.initializeAgent(args);
    		Assert.assertEquals("A POST request has been sent to turn on the device or component.", result.getJSONArray("message").get(0));
    		Assert.assertEquals("A request has been successfully sent to the ESPHome web server.", result.getJSONArray("message").get(1));
    }
    }
    
    @Test
    public void testEspHomeSwitchControlTurnOffSwitch() throws IOException {
    	String propertiesFile = Paths.get(folder.getRoot().toString(), "all.properties").toString();
    	//change threshold value to 27
        writePropertyFile(propertiesFile, Arrays.asList("db.user="+postgres.getUsername(), "db.password="+postgres.getPassword(), "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=27"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	JSONObject message = new JSONObject();
    	...
    	try(MockedConstruction<ESPHomeAPI> ignored = Mockito.mockConstruction(ESPHomeAPI.class,
                (mock, context) -> {
                    Mockito.when(mock.checkStatus()).thenReturn(true);
                    //26.5 < threshold, status of switch = true
                    Mockito.when(mock.esphomeSwitchControl(26.5, true)).thenReturn(message);
                })) {
    		JSONObject result = agent.initializeAgent(args);
    		Assert.assertEquals("A POST request has been sent to turn off the device or component.", result.getJSONArray("message").get(0));
    		Assert.assertEquals("A request has been successfully sent to the ESPHome web server.", result.getJSONArray("message").get(1));
    }
    }
    
    @Test
    public void testEspHomeSwitchControlSwitchIsAlreadyOff() throws IOException {
    	String propertiesFile = Paths.get(folder.getRoot().toString(), "all.properties").toString();
    	//change threshold value to 27
        writePropertyFile(propertiesFile, Arrays.asList("db.user="+postgres.getUsername(), "db.password="+postgres.getPassword(), "sparql.query.endpoint="+endpoint, "sparql.update.endpoint="+endpoint, "dataIRI="+examplePrefix+key, "esphome.url=test_url", "esphome.threshold=27"));
    	String [] args = new String[] {propertiesFile, propertiesFile};
    	...
    	try(MockedConstruction<ESPHomeAPI> ignored = Mockito.mockConstruction(ESPHomeAPI.class,
                (mock, context) -> {
                    Mockito.when(mock.checkStatus()).thenReturn(false);
                  //26.5 < threshold, status of switch = false
                    Mockito.when(mock.esphomeSwitchControl(26.5, false)).thenReturn(message);
                })) {
    		JSONObject result = agent.initializeAgent(args);
    		Assert.assertEquals("The switch is already in the OFF state.", result.getJSONArray("message").get(0));
    		Assert.assertEquals("A request has been successfully sent to the ESPHome web server.", result.getJSONArray("message").get(1));
    }
    }
}
