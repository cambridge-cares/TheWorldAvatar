...
public class ThingsBoardInputAgentIntegrationTest {
    ...
    private ThingsBoardInputAgent agent;
    ...
    public void createExampleReadings() {
        ...
        for(String key: keys) {
        	...
        	for (int i = 0; i < ts.length; i++) {
        		...
        		tsAndValue.put(ThingsBoardInputAgent.timestampKey, ts[i]);
    ...
    public void testUpdateTimeSeries() {
        ...
        // Assert timestamps
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[0]), ThingsBoardInputAgent.ZONE_OFFSET)
                        .isEqual(tsClient.getMinTime(IRIs.get(0))));
        Assert.assertTrue(OffsetDateTime.of(LocalDateTime.parse(timestamps[timestamps.length-1]), ThingsBoardInputAgent.ZONE_OFFSET)
    ...
    public void testUpdateTimeSeriesWithPruning() {
        ...
        for(int i = 0; i < timestamps.length ; i++) {
            if (i < (timestamps.length-1)) {
                times.add(OffsetDateTime.of(LocalDateTime.parse(timestamps[i]), ThingsBoardInputAgent.ZONE_OFFSET));
            }
        }
