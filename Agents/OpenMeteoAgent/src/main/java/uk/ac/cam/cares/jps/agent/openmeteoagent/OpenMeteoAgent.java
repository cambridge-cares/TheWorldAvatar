...
public class OpenMeteoAgent extends JPSAgent {
    ...
    private List<String> API_PARAMETERS = Arrays.asList(API_PARAMETER_TEMP, API_PARAMETER_HUMIDITY,
            API_PARAMETER_DEWPOINT, API_PARAMETER_PRESSURE, API_PARAMETER_RAIN, API_PARAMETER_SNOW, API_PARAMETER_CLOUD,
            API_PARAMETER_DNI, API_PARAMETER_DHI, API_PARAMETER_WINDSPEED, API_PARAMETER_WINDDIRECTION);
    private List<String> ontoems_conecpts = Arrays.asList(ONTOEMS_TEMP, ONTOEMS_HUMIDITY, ONTOEMS_DEWPOINT,
            ONTOEMS_PRESSURE, ONTOEMS_RAIN, ONTOEMS_SNOW, ONTOEMS_CLOUD, ONTOEMS_DNI, ONTOEMS_DHI, ONTOEMS_WINDSPEED,
            ONTOEMS_WINDDIRECTION);
    ...
    public JSONObject processRequestParameters(JSONObject requestParams) {
        ...
        if (validateInput(requestParams)) {
            if (requestParams.getString(KEY_REQ_URL).contains(URI_RUN)) {
                ...
                JSONObject response = getWeatherData(latitude, longitude, requestParams.getString(KEY_START),
                        requestParams.getString(KEY_END));
                ...
                if (stationIRI.isEmpty()) {
                    ...
                    try (Connection conn = rdbStoreClient.getConnection()) {
                        for (int i = 0; i < queryResults.length(); i++) {
                            ...
                            OffsetDateTime min = tsClient.getMinTime(queryResults.getJSONObject(i).getString("measure"),
                                    conn);
                            OffsetDateTime max = tsClient.getMaxTime(queryResults.getJSONObject(i).getString("measure"),
                                    conn);
                            tsClient.deleteTimeSeriesHistory(queryResults.getJSONObject(i).getString("measure"), min,
                                    max, conn);

                            // new time series
                            List<String> dataIRI = Arrays.asList(queryResults.getJSONObject(i).getString("measure"));
                            String apiParam = ontoems_api.get(
                                    queryResults.getJSONObject(i).getString("weatherParameter").split(ontoemsURI)[1]);
                            tsList.add(new TimeSeries<>(timesList, dataIRI,
                                    List.of((List<Double>) parsedData.get(apiParam).get(1))));
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    public boolean validateInput(JSONObject requestParams) {
        ...
        try {
            if (requestParams.get(KEY_REQ_METHOD).equals(HttpMethod.POST)) {
                ...
                if (requestParams.getString(KEY_REQ_URL).contains(URI_RUN)) {
                    validate = validate && !requestParams.getString(KEY_START).isEmpty()
                            && !requestParams.getString(KEY_END).isEmpty()
                    ...
                    // start date cannot be later than end date, and end date cannot be later than
                    // the date at the time of the incoming request
    ...
    /**
     * Calls the Open-Meteo Historical Weather API to retrieve the hourly weather
     * data from start_date to end_date with weather parameter in API_PARAMETERS for
     * weather station located at (latitude, longitude)
     * 
     ...
     * @param weatherUnit JSONObject containing the units of the weather data
     *                    retrieved by the API
     * @return a Map object with the key being the API weather parameters, and the
     *         values being the corresponding units ontology and the corresponding
     *         data parsed to a list
     */
    private Map<String, List<Object>> parseWeatherData(JSONObject weatherData, JSONObject weatherUnit) {
        ...
        for (String parameter : API_PARAMETERS) {
            ...
            tempType = new TypeToken<List<Double>>() {
            }.getType();
     ...
     */
    private void createUpdate(WhereBuilder builder, String station, String quantity, String type, String measure,
            String unit) {
    ...
    /**
     * Creates query for the quantity IRI, quantity type, measure IRI, and time
     * series IRI related to the reporting station IRI
     * 
     * @param builder where builder
     * @param station reporting station IRI
     */
    ...
    /**
     * Queries for the OntoEMS:ReportingStation IRI with the given lat, lon
     * coordinate
     * 
    ...
    /**
     * Queries for and returns the quantity IRI, quantity type, measure IRI, and
     * time series IRI related to stationIRI
     * 
     * @param stationIRI IRI of weather station
     * @return query result for quantity IRI, quantity type, measure IRI, and time
     *         series IRI related to stationIRI
     */
     ...
     * @param durations numeric duration of the averaging period for time series of
     *                  TimeSeriesClient.Type.AVERAGE
     * @param units     temporal unit of the averaging period for time series of
     *                  TimeSeriesClient.Type.AVERAGE
     */
    private void createTimeSeries(List<List<String>> dataIRI, List<List<Class<?>>> dataClass, List<String> timeUnit,
            List<TimeSeriesClient.Type> type, List<Duration> durations, List<ChronoUnit> units) {
        try (Connection conn = rdbStoreClient.getConnection()) {
     ...
     */
    private void instantiateWeather(String stationIRI, Map<String, List<Object>> parsedData,
            List<OffsetDateTime> timesList, List<TimeSeries<OffsetDateTime>> tsList) {
        WhereBuilder wb = new WhereBuilder()
                .addPrefix("ontoems", ontoemsURI)
                .addPrefix("om", omURI)
                .addPrefix("rdf", rdfURI);
