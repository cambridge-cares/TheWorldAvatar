...
class Downloader:
    def __init__(self, conf):
        ...
        self.post_calculation = getattr(importlib.import_module("data_classes.calculations") ,post_cal_name) if post_cal_name else None
    ...
    def download_tsinstance(self) -> ts_data_classes.TimeSeriesInstance:
        ...
        if 'dynamicGenerated' in conf:
            ...
            url = conf['url']
        raw_response = requests.request(conf['method'], url, headers=magic_header, params=dict(self.conf['sourceparams']) if 'sourceparams' in self.conf else None)
        # Add switch of parser types later
        print(dict(self.conf['output']))
        parsed = self.format_parser(raw_response.content if self.format=='xlsx' else raw_response.text,**dict(self.conf['output']))
...
def xlsx_parser(raw_response, src_iri,value_col,time_col,sheetname,**kwargs) -> ts_data_classes.TimeSeriesInstance:
    ...
    return ts_data_classes.TimeSeriesInstance(src_iri=src_iri, values=df[value_col_label].tolist(),times=df[time_col_label].tolist())