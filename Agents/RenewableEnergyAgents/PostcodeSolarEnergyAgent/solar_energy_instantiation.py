...
def format_data(sensor_data):
    ...
    for sensor in sensor_data.keys():
        ...
        sensor_dict['timeseries'] = {'January' : sensor_data[sensor][7], 'February' : sensor_data[sensor][8], 'March' : sensor_data[sensor][9],
                                     ...
                                     'October' : sensor_data[sensor][16], 'November' : sensor_data[sensor][17], 'December' : sensor_data[sensor][18], 'NearestSolarPower' : []}
    ...
    query = utils.create_sparql_prefix('ontoweather') + \
            ...
            '''SELECT ?cons \
               WHERE { \
                 ...
                 ?cons rdf:type ontoweather:SolarSensor \
...
def nearest_solar_data(postcode, KGClient, TSClient):
    print(postcode)
    ...
    solar_powers = solar_power(ts_json[0]['values'][0])
...
def instantiate_energy_data():
    ...
    units = {'January': 'kWh', 'February': 'kWh', 'March': 'kWh', 'April': 'kWh', 'May': 'kWh', 'June': 'kWh', 
             'July': 'kWh', 'August': 'kWh', 'September': 'kWh', 'October': 'kWh', 'November': 'kWh', 'December': 'kWh', 'NearestSolarPower': 'kW'}
    ...
    for postcode in postcode_instances:
        ...
        solar_powers = nearest_solar_data(postcode, KGClient, TSClient)
        postcode['timeseries']['NearestSolarPower'] = solar_powers
