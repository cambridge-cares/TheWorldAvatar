...
def query_midas_solar():
    '''Queries all of the MIDAS solar sensors and saves the data to files ready to be visualised'''
    ...
    file_name = os.path.join(utils.OUTPUT_DIR, 'sensors', 'MIDASSensorsSolar.geojson')
    with open(file_name, 'w') as f:
        json.dump(geojson, indent=4, fp=f)
    file_name = os.path.join(utils.OUTPUT_DIR, 'sensors', 'MIDASSensorsSolar-meta.json')
    with open(file_name, 'w') as f:
        json.dump(metadata, indent=4, fp=f)
    file_name = os.path.join(utils.OUTPUT_DIR, 'sensors', 'MIDASSensorsSolar-timeseries.json')
    with open(file_name, 'w') as f:
