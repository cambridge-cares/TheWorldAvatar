...
def get_consumers_in_circle(center, radius):
    ...
    # Define query
    query = kg_utils.create_sparql_prefix('ex') + \
            kg_utils.create_sparql_prefix('rdf') + \
            kg_utils.create_sparql_prefix('geo') + \
            kg_utils.create_sparql_prefix('geolit') + \
            '''SELECT ?cons \
               WHERE { \
                 SERVICE geo:search \
                 { \
...
def get_geojson_data(terminal):
    '''
        Returns coordinates ([lon, lat]), ground elevation (z), and name (label) for the given 'terminal'
    '''
...
if __name__ == '__main__':
    ...
    geojson_terminals_points = geojson_initialise_dict()
    geojson_offtakes_points = geojson_initialise_dict()
    geojson_pipes_polygons = geojson_initialise_dict()
    metadata_terminals_points = []
    metadata_offtakes_points = []
    metadata_pipes_polygons = []
    ts_dict = {'ts': [],
               'id': [],
               'units': [],
               'headers': []
               }
    ts_data_terminals_points = copy.deepcopy(ts_dict)
    geojson_offtakes_points = copy.deepcopy(ts_dict)
    geojson_pipes_polygons = copy.deepcopy(ts_dict)

    # Initialise feature_id count (required to map meta and timeseries data to GeoJSON objects)
    feature_id = 0

    # Get terminals of interest
    terminals = kg_utils.get_instantiated_terminals(kg_utils.QUERY_ENDPOINT)
    ...
    for t in terminals:
        ...
        coords, z, name = get_geojson_data(t)
    ...
    for i in range(len(geojson)):
        ...
        file_name = os.path.join(kg_utils.OUTPUT_DIR, folder[i], filename[i] + '.geojson')
        with open(file_name, 'w') as f:
            json.dump(geojson[i], indent=4, fp=f)
        file_name = os.path.join(kg_utils.OUTPUT_DIR, folder[i], filename[i] + '-meta.json')
        with open(file_name, 'w') as f:
            json.dump(metadata[i], indent=4, fp=f)
        file_name = os.path.join(kg_utils.OUTPUT_DIR, folder[i], filename[i] + '-timeseries.json')
        with open(file_name, 'w') as f:
