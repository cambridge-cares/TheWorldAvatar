...
def construct_hypo_reactor(sparql_client: ChemistryAndRobotsSparqlClient, rxn_exp_instance: ReactionExperiment, internal_standard_instance: InternalStandard) -> Tuple[HypoReactor, float, dict]:
    ...
    if len(_run_volume_dct) > 1:
        ...
        try:
            run_volume_dct = {reference_input_chemical:utils.unit_conversion(_run_volume_dct[reference_input_chemical].hasNumericalValue,
                _run_volume_dct[reference_input_chemical].hasUnit, utils.UNIFIED_VOLUME_UNIT)}
        except NotImplementedError:
            raise Exception("Unit conversion of ReactionScale from <%s> to <%s> is FAILED for ReactionExperiment: %s." % (
                _run_volume_dct[reference_input_chemical].hasUnit, utils.UNIFIED_VOLUME_UNIT, str(rxn_exp_instance)))
    ...
    if internal_standard_instance.representsOccurenceOf not in species_role_dct:
        utils.deep_update(species_role_dct, {internal_standard_instance.representsOccurenceOf:internal_standard_instance.clz})
    else:
    ...
    for ic_key in input_chemical_dct:
        utils.deep_update(species_pump_conc_dct,
        {ic_key:{species_role_dct[p.representsOccurenceOf]:{p.representsOccurenceOf:utils.DimensionalQuantity(hasUnit=p.hasProperty.hasValue.hasUnitOfMeasure,
    ...
    try:
        _temp_value = utils.unit_conversion_return_value(species_pump_conc_dct[reference_input_chemical][ONTOKIN_REACTANT][reference_solute].hasNumericalValue,
            ...
            utils.UNIFIED_CONCENTRATION_UNIT) * utils.unit_conversion_return_value(run_volume_dct[reference_input_chemical].hasNumericalValue,
            run_volume_dct[reference_input_chemical].hasUnit, utils.UNIFIED_VOLUME_UNIT)
        utils.deep_update(run_mol_dct, {reference_input_chemical:{reference_solute:utils.DimensionalQuantity(hasUnit=utils.UNIFIED_MOLE_UNIT, hasNumericalValue=_temp_value)}})
    except NotImplementedError:
    ...
    total_run_volume = utils.DimensionalQuantity(hasUnit=utils.UNIFIED_VOLUME_UNIT,
        hasNumericalValue=sum([utils.unit_conversion_return_value_dq(dq, utils.UNIFIED_VOLUME_UNIT) for dq in list(run_volume_dct.values())]))
    ...
    for ic in run_volume_dct:
        ...
        for _role in species_role_in_input_chemical_dct[ic]:
            ...
            utils.deep_update(run_conc_dct, {ic:{_species_iri:utils.DimensionalQuantity(hasUnit=utils.UNIFIED_CONCENTRATION_UNIT,hasNumericalValue=_sp_run_conc)}})
            ...
            if _role == ONTOHPLC_INTERNALSTANDARD:
                utils.deep_update(_dct_IS_run_conc_moleperlitre, {_species_iri:utils.unit_conversion_return_value(_sp_run_conc, utils.UNIFIED_CONCENTRATION_UNIT, OM_MOLEPERLITRE)})
        ...
        hypo_pump_inlet = HypoPumpRunStream(
            ...
            inlet_iri=ic,
            run_volume=utils.DimensionalQuantity(hasUnit=run_volume_dct[ic].hasUnit, hasNumericalValue=run_volume_dct[ic].hasNumericalValue),
            eq_ratio=utils.DimensionalQuantity(hasUnit=stoi_ratio_dct[ic].hasUnit,hasNumericalValue=stoi_ratio_dct[ic].hasNumericalValue),
            solute=lst_solute,
            solvent=solvent_ins
        )
    ...
    hypo_reactor = HypoReactor(
        ...
        residence_time=utils.unit_conversion(_res_time, _res_time_unit, utils.UNIFIED_TIME_UNIT),
        reactor_temperature=utils.unit_conversion(_rxn_temp, _rxn_temp_unit, utils.UNIFIED_TEMPERATURE_UNIT),
        reactor_volume=utils.unit_conversion(_reactor_vol, _reactor_vol_unit, utils.UNIFIED_VOLUME_UNIT),
