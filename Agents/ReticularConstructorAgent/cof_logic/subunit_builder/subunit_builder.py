...
def update_atom_bonds(lfr_atoms, old_uuid, new_uuid):
...
def substitute_atom_properties(lfr_atoms, target_atom_uuid, reference_atom_data, x_outer_uuid):
    ...
    rotation_matrix = rotation_matrix_from_vectors(original_vector, target_vector)
...
def find_bs_and_update(json_file_path, ref_atom_uuid, ref_atom_data, bs_type):
    if bs_type == "NH2":
        target_atom = "N"
    elif bs_type == "MDCO":
        target_atom = "C"
            ...
            for bond in atom_data.get('bond', []):
                ...
                if to_atom_uuid in lfr_atoms:
                    ...
                    if next_atom_data['atom'] == target_atom:
                        print(f"Found {target_atom} atom at position {to_atom_uuid}")
    ...
    if x_atom_uuid_to_remove and target_atom_uuid:
        ...
        lfr_atoms = substitute_atom_properties(lfr_atoms, target_atom_uuid, ref_atom_data, x_outer_uuid)
        
        # Next, update all bonds referencing the target atom UUID
        update_atom_bonds(lfr_atoms, target_atom_uuid, ref_atom_uuid)
...
def shift_atoms_to_origin(json_file_path):
    ...
    midpoint = calculate_midpoint(x_atoms)
    
    if midpoint is None:
        print("No X atoms found in the JSON file.")
        return

    print(f"Midpoint of X atoms: {midpoint}")

    shifted_atoms, reference_atoms = shift_atoms(atoms, midpoint)
...
def update_lfr_files(core_atoms, reference_atoms, lfr_folder_path, bs_type):
    for i, ref_atom_uuid in enumerate(reference_atoms):
        ...
        if not os.path.exists(lfr_file_path):
            ...
            continue
        find_bs_and_update(lfr_file_path, ref_atom_uuid, ref_atom_data, bs_type)

def create_and_update_subcomponent(core_json_path, subcomponent_json_path, lfr_folder_path):
...
def generate_xyz_from_json(json_file_path, xyz_file_path):
...
if __name__ == "__main__":
    ...
    core_atoms, reference_atoms = shift_atoms_to_origin(core_json_path)

    # Update the lfr files with the atoms from core.json
    update_lfr_files(core_atoms, reference_atoms, lfr_folder_path, bs_type)
    ...
    create_and_update_subcomponent(core_json_path, subcomponent_json_path, lfr_folder_path)
    generate_xyz_from_json(subcomponent_json_path, subcomponent_xyz_path)
