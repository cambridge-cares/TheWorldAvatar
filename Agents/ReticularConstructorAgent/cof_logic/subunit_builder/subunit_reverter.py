...
class SubunitReverter:
    ...
    def process_bonds(self, bonds, row_mapping):
        ...
        if bonds:
            ...
            if len(bonds) > 1:
                middle_bonds = '/'.join(f"{row_mapping[bond['to_atom']]}:{bond['bond_order']}" for bond in bonds[1:])
    ...
    def process_json_to_custom_format_with_bonds(self):
        ...
        with open(self.output_file_path, 'w') as output_file:
            ...
            for row_number, (uuid, atom_data) in enumerate(data.items(), 1):
                ...
                output_file.write(f"{atom:<2} {coords_str}MMTYPE={mmtype} QMMM={qmmm} {bond_str}\n")
