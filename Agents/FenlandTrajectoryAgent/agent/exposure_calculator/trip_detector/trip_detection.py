...
DEFAULT_PARAMS = {
    ...
    "MAX_RASTER_SIZE": 14000*14000, # originally, greatest possible number of cells per row/column, default 30000
    "INTERPOLATE_MAX_DELAY": 120,  # seconds
    "INTERPOLATE_MAX_DROP_TIME": 360000,  # seconds, default: 3600 || high value ~ infinity
    "INTERPOLATE_MAX_DISTANCE": 100000,  # meters, default: 100 || high value ~ infinity
    ...
    "SIGNIFICANT_GAP_LENGTH": 900  # NEW parameter! Minimum length of a data gap to be considered a missing data period
}
...
# NB In spite of the name suggesting otherwise, we need a full
# time stamp in the relevant column, not just a date!!!
AI4PH_COLUMNS = {
    "utc_date": "Timestamp",
    "lat": "LATITUDE",
    "lon": "LONGITUDE",
...
def get_data_gaps(df, pid, columns,
    ...
    """
    df = df.sort_values(columns["utc_date"])
    df.loc[:, "gap_time"] = ((df[columns["utc_date"]] - df.shift(1)[columns["utc_date"]]) / np.timedelta64(1, 's')).fillna(0)
    gaps = df.loc[df["gap_time"] > 1, (columns["utc_date"], "gap_time")]
    sig_gaps = gaps.loc[gaps["gap_time"] > gap_length].copy()
    sig_gaps.loc[:, "end_time"] = sig_gaps[columns["utc_date"]] - np.timedelta64(1, 's')
    sig_gaps.loc[:, "start_time"] = sig_gaps["end_time"] - pd.to_timedelta(sig_gaps["gap_time"], unit="s")
...
def resize_cells(w, h, cell_size, kernel_bandwidth, max_raster_size):
    ...
    if raster_size > max_raster_size:
        ...
        if kernel_bandwidth / 2.0 >= cell_size:
            print(f"Changing raster resolution to {cell_size} m to keep raster size below threshold.")
        else:
            print(f"FATAL ERROR: New cell size {cell_size} m is incompatible with kernel bandwidth!")
            sys.exit()
    return cell_size
...
def downsample_trace(df, columns, frequency=5):
    ...
    downsampled = downsampled.sort_values(columns["utc_date"]).reset_index(drop=True)
    return downsampled
...
def interpolate_over_period(df, columns, frequency, noise=3.0):
    ...
    new_rows = df.copy().set_index(columns["utc_date"]).resample(f"{int(frequency)}S").asfreq().replace([0.0], np.nan)
    ...
    new_rows = new_rows[(new_rows[columns["utc_date"]] >= df.iloc[0][columns["utc_date"]]) & (new_rows[columns["utc_date"]] <= df.iloc[1][columns["utc_date"]])].copy()
...
def interpolate_over_period_test1(df, columns, frequency, noise=3.0):
    ...
    from_pt = Point((df.iloc[0][columns["utm_e"]], df.iloc[0][columns["utm_n"]]))
    to_pt = Point((df.iloc[-1][columns["utm_e"]], df.iloc[-1][columns["utm_n"]]))
    n_samples = len(new_rows.index)

    ipath = interpolate_graph(from_pt, to_pt, r'test_data/utm_roadnet.bin', impedance='weighted_length')
    if ipath.is_empty:
...
def interpolate_over_period_test2(df, columns, frequency, noise=3.0):
    ...
    ipath = interpolate_graph(from_pt, to_pt, r'test_data/utm_roadnet.bin', impedance='weighted_length')
    if ipath.is_empty:
        logging.warning('Interpolation returned with no valid path, defaulting to straight line')
        ipath = LineString([from_pt, to_pt])
    igps_fix = density_sample(ipath, n_samples, gps_dsty_rst=r'test_data/sd_gps_asap_KDE50c10.tif', noise=noise)
    igps_coords = map(lambda p: (p.x, p.y), igps_fix)
    igps_x, igps_y = zip(*igps_coords)
    new_rows.loc[:,columns["utm_e"]] = igps_x
    new_rows.loc[:,columns["utm_n"]] = igps_y
...
def interpolate_over_period_test3(df, columns, frequency, noise=3.0, min_dist=50):
    ...
    if ipath.is_empty:
        logging.warning('Interpolation returned with no valid path, defaulting to straight line')
        ipath = LineString([from_pt, to_pt])
    igps_fix = density_sample(ipath, n_samples, gps_dsty_rst=r'test_data/sd_gps_asap_KDE50c10.tif', noise=noise, min_dist=min_dist)
    igps_coords = map(lambda p: (p.x, p.y), igps_fix)
    igps_x, igps_y = zip(*igps_coords)
    new_rows.loc[:,columns["utm_e"]] = igps_x
    new_rows.loc[:,columns["utm_n"]] = igps_y
...
def interpolate_over_dropped_periods(df, columns, frequency, max_delay=120,
    ...
    differences = ((df[columns["utc_date"]].shift(-1) - df[columns["utc_date"]]).shift(1) / np.timedelta64(1, 's')).fillna(0.0)
...
def interpolate_over_dropped_periods_median(df, columns, frequency,
    ...
    med_prev = df.rolling(bins).median().rename(columns={columns["utm_e"]: 'prev_e', columns["utm_n"]: 'prev_n'})
    ...
    # (the delay is less than max_drop_time OR distance delta is less than max_distance)
    delay_mask = differences > max_delay
    drop_distance_mask = ((differences < max_drop_time) | (med_distance_delta < max_distance))
    df['gap'] = delay_mask & drop_distance_mask
...
def extract_hotspots(points, x_bounds, y_bounds, kernel_bandwidth, pid,
    ...
    x_grid = np.arange(x_bounds[0] - cell_size, x_bounds[1] + cell_size, cell_size)
    y_grid = np.arange(y_bounds[0] - cell_size, y_bounds[1] + cell_size, cell_size)
    ...
    if plot:
        ...
        plt.savefig(f"{os.getcwd()}/temp/{pid}_temp_kernel_k{str(kernel_bandwidth)}_ds{qualifiers}.png") #plt.show()
    ...
    indexes = [y_indices[i] * len(x_grid) + x_indices[i] for i in range(len(y_indices))]
    ...
    centroids = ndi.center_of_mass(sink_mask, temp_markers, index=range(1, maximum + 1))
    ...
    zone_labels = [labels.reshape((len(y_grid) * len(x_grid),))[x] for x in indexes]
...
def refine_hotspots(df, columns, hotspots, data_gaps, min_keep_duration,
    ...
    if not fake_points.empty:
        fake_points.loc[:, columns["utc_date"]] = fake_points["start_time"] + np.timedelta64(1, 's')
...
def filter_visits(dwell_table, hotspots, min_visit_timespan, min_visit_duration):
    ...
    filtered_visits = filtered_visits[filtered_visits["zone"].isin(hotspots["hotspot_id"].unique())]
...
def build_visits(dwell_table, hotspots, min_visit_timespan, min_visit_duration):
    ...
    visit_table = filter_visits(dwell_table, hotspots, min_visit_timespan, min_visit_duration)
...
def detect_path(gps_data, columns, from_time, to_time, max_drop_time,
    ...
    for _, g in split_groups:
        ...
        if len(segments) > 1:
            ...
            duration = (selected[columns["utc_date"]].max() - selected[columns["utc_date"]].min()) / np.timedelta64(1, 's')
...
def build_paths(gps_data, columns, visits, data_gaps, max_drop_time, min_keep_time):
    ...
    for i in range(len(stop_events)):
        ...
        paths = detect_path(gps_data, columns, from_time, to_time, max_drop_time, min_keep_time, paths)
    ...
    to_time = gps_data.iloc[-1][columns["utc_date"]] + pd.Timedelta(1, unit="s")
    paths = detect_path(gps_data, columns, from_time, to_time, max_drop_time, min_keep_time, paths)
    ...
    for _, t in time_series.items():
        while t>stop_time:
            try:
                ...
                stop_time = last_time
        index_list.append(0 if t<start_time else (int(index>0) if binary else index))
    return index_list
...
def detect_trips(gps_data, pid, columns, params=DEFAULT_PARAMS,
    ...
    gps_data = get_norm_modified_kernel(gps_data, zone_stats, buffer_weight, pid, kernel_bandwidth, i_string)
    ...
    hotspots, dwells = refine_hotspots(gps_data, columns, hotspots, gaps, min_hotspot_keep)
    print(f"[{strftime('%H:%M:%S')}] Building visits from dwells.")
    visit_table = build_visits(dwells, hotspots, min_visit_timespan, min_visit_duration)
    ...
    final_bouts = build_paths(gps_data, columns, visit_table, gaps, max_drop_time, min_path_keep)
    print(f"[{strftime('%H:%M:%S')}] Building incidents table.")
    incidents_table = build_incident_table(final_bouts, hotspots, visit_table, gaps, pid)

    print(f"[{strftime('%H:%M:%S')}] Building trip, visit, and gap look-ups...")
    gps_data[CH_TRIP_INDEX] = make_index(final_bouts.iterrows(), gps_data["Timestamp"], False)
    gps_data[CH_VISIT_INDEX] = make_index(visit_table.iterrows(), gps_data["Timestamp"], False)
    gps_data[CH_GAP] = make_index(gaps.iterrows(), gps_data["Timestamp"], True)
...
def main():
    ...
    PID = 'test'
    TEST_FILE_DIR = os.path.join("input", "SampleGPSTrajectory_02-processed.csv")
    OUT_FILE_DIR = os.path.join("output", "SampleGPSTrajectory_02_incidents_table.csv")

    # Load GPS track.
    try:
        gps_data = pd.read_csv(TEST_FILE_DIR, parse_dates=[AI4PH_COLUMNS["utc_date"]])
    except FileNotFoundError:
        print("ERROR: unable to locate gps .csv file at ", TEST_FILE_DIR, ". Aborting.")
        exit()

    gps_data, incidents_table, _, _, _ = detect_trips(
        gps_data, PID, AI4PH_COLUMNS, params=DEFAULT_PARAMS,
        interpolate_helper_func=None) #interpolate_over_period_test1

    gps_data.to_csv(os.path.join("output", "SampleGPSTrajectory_02_trip_detection.csv"),
        encoding='utf-8', index=False)
    incidents_table.to_csv(OUT_FILE_DIR, index=False)

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    #import cProfile
    #cProfile.run('main()', r'temp\prof_main_gap.file')
    main()
