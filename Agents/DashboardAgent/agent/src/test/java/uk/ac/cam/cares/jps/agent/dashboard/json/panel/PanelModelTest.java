...
class PanelModelTest {
    ...
    void genModifiableSampleData() {
        ...
        SAMPLE_ROOMS = TestUtils.genSampleRoomMeasureMap();
    }
    ...
    @Test
    void testConstructForOnlyRooms() {
        // Construct and execute the method
        String result = new PanelModel(SAMPLE_DB_CONNECTION_ID_MAP, SAMPLE_ROOMS).construct();
        // Test outputs
        assertEquals(genExpectedRowSyntax(SAMPLE_DB_CONNECTION_ID_MAP, TestUtils.genSampleRoomMeasureMap()), result);
    }

    @Test
    void testConstructForBothAssetsAndRooms() {
        // Construct and execute the method
        String result = new PanelModel(SAMPLE_DB_CONNECTION_ID_MAP, TestUtils.genSampleComplexMeasureMap()).construct();
        // Test outputs
        assertEquals(genExpectedRowSyntax(SAMPLE_DB_CONNECTION_ID_MAP, TestUtils.genSampleComplexMeasureMap()), result);
    }
    ...
    private static String genExpectedRowSyntax(Map<String, String> databaseConnectionMap, Map<String, Map<String, List<String[]>>> items) {
            ...
            for (String roomMeasure : roomMetadata.keySet()) {
                ...
                if (!roomMeasure.equals(StringHelper.ROOM_KEY)) {
                    ...
                    builder.append("{\"id\":null, \"type\":\"row\", \"collapsed\":true,")
                            ...
                            .append("\"panels\": [").append(genExpectedPanelsSyntax(ROW_NUMBER, StringHelper.ROOM_KEY, databaseConnectionMap, expectedIndividualMap))
                            .append("]}");
                    ROW_NUMBER++;
                }
            }
        ...
        for (String assetType : items.keySet()) {
            ...
            builder.append("{\"id\":null, \"type\":\"row\", \"collapsed\":true,")
                    ...
                    .append(",\"x\": 0,\"y\": ").append(ROW_NUMBER).append("},")
                    // To generate the panels for assets
                    .append("\"panels\": [").append(genExpectedPanelsSyntax(ROW_NUMBER, assetType, databaseConnectionMap, assetMetadata))
                    .append("]}");
            ROW_NUMBER++;
        }
    ...
    private static String genExpectedPanelsSyntax(int rowNumber, String itemType, Map<String, String> databaseConnectionMap, Map<String, List<String[]>> itemMetadata) {
        StringBuilder builder = new StringBuilder();
            ...
            if (!measure.equals(StringHelper.ASSET_KEY) && !measure.equals(StringHelper.ROOM_KEY)) {
                ...
                if (itemType.equals(StringHelper.ROOM_KEY)) {
                    ...
                    builder.append(GaugeTest.genExpectedResults(expectedConfigItems, expectedGeometryPosition, itemMetadata.get(measure),
                                    GaugeTest.genAverageQuery(itemMetadata.get(measure))))
                            .append(",");
