...
public class TransformationOptions {
     ...
     *
     * @param timeSeriesMetadata A queue of items and their metadata for the specified measure.
     */
    public void addOrganizeTransformation(Queue<String[]> timeSeriesMetadata) {
     ...
     * @param timeSeriesMetadata A queue of items and their metadata for the specified measure.
     */
    public void addOrganizeTransformation(String nameSuffix, Queue<String[]> timeSeriesMetadata) {
        ...
        // Ensure that the queue is extracted into a list so that the values are still available after processing
        List<String[]> timeSeriesList = new ArrayList<>(timeSeriesMetadata);
        for (int indexCounter = 1; indexCounter <= timeSeriesMetadata.size(); indexCounter++) {
            String[] metadata = timeSeriesList.get(indexCounter - 1);
     ...
     * @param timeSeriesMetadata A queue of items and their metadata for the specified measure.
     */
    public void addGroupByTransformation(String aggregate, Queue<String[]> timeSeriesMetadata) {
        StringBuilder transformationBuilder = new StringBuilder();
        StringBuilder fieldAggregations = new StringBuilder();
        // Process metadata into the required format
        timeSeriesMetadata.forEach(metadata -> {
            // Only append a comma at the start if it is not the first set
