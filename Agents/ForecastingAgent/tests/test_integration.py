...
def test_create_forecast(
    initialise_clients, create_example_agent, derivation_input_set, expected_error_msg
):
    """
    Test if forecasting agent throws expected error messages for invalid inputs
    """

    # Get required clients from fixture
    sparql_client, _, derivation_client, _ = initialise_clients

    # Initialise all triples in test_triples (first DELETES ALL DATA)
    cf.initialise_triples(sparql_client)
    ...
    with pytest.raises(TypeError) as exc_info:
        ...
        derivation_client.createSyncDerivationForNewInfo(agent.agentIRI, derivation_input_set,
                                                         cf.ONTODERIVATION_DERIVATIONWITHTIMESERIES)
        #derivation_client.createAsyncDerivationForNewInfo(agent.agentIRI, derivation_input_set)
