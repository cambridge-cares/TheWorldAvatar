...
public class WeatherAgentIntegrationTest {
    ...
    @Container
    private GenericContainer<?> ontop = new GenericContainer<>(DockerImageName.parse("ontop/ontop-endpoint:4.2.1"))
            .withExposedPorts(8080);

    @Container
    private GenericContainer<?> stack_manager = new GenericContainer<>(
            DockerImageName.parse("docker.cmclinnovations.com/stack-manager:1.2.0")).withExposedPorts(8080);
    ...
    DockerImageName postgisImage = DockerImageName.parse("postgis/postgis:13-3.2")
    ...
    public void init() throws IOException, URISyntaxException {
        ...
        postgres.start();
        // Connection conn =
        // DriverManager.getConnection("jdbc:tc:postgis:14-3.2:///timeseries",
        // "postgres", "postgres");
        ...
        String postgresDriver = Paths.get(System.getProperty("user.home"),
                ".m2/repository/org/postgresql/postgresql/42.4.1/postgresql-42.4.1.jar").toString();
        ...
        URIBuilder kgbuilder = new URIBuilder().setScheme("http").setHost(blazegraph.getHost())
                .setPort(blazegraph.getFirstMappedPort()).setPath("/blazegraph/namespace/kb/sparql");
        String kg_endpoint = kgbuilder.build().toString();

        URIBuilder ontopbuilder = new URIBuilder().setScheme("http").setHost(ontop.getHost())
                .setPort(ontop.getFirstMappedPort()).setPath("sparql");
        ...
        TimeSeriesRDBClientWithReducedTables<Instant> rdbClient = new TimeSeriesRDBClientWithReducedTables<>(
                Instant.class);
        rdbClient.setRdbURL(postgres.getJdbcUrl());
        rdbClient.setRdbUser(postgres.getUsername());
        rdbClient.setRdbPassword(postgres.getPassword());
        tsClient = new TimeSeriesClient<Instant>(kgClient, rdbClient);
