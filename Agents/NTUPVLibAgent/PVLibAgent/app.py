...
from PVLibAgent.data_retrieval.query_timeseries import query_latest_timeseries, query_all_timeseries, query_timeseries_within_bounds
...
from PVLibAgent.config.buses import BUSES
...
def api():
    ...
    if 'start' and 'end' in request.args:
        try:
            lower_bound = request.args['start']
            logging.info("Start: "+str(lower_bound))
            upper_bound = request.args['end']
            logging.info("End: "+str(upper_bound))
            bool_bounds = True
        except ValueError:
            logging.error("Unable to parse time series bounds.")
            return "Unable to parse time series bounds"
    else:
        bool_bounds = False
        logging.info("No time series bounds")
    ...
    if device.__contains__('weatherStation'):
        ...
        try:    
            if bool_bounds:
                logging.info("Getting timeseries within bounds: "+str(lower_bound)+" and "+str(upper_bound))
                wind_speed = query_timeseries_within_bounds(wind_speed_iri, QUERY_ENDPOINT, UPDATE_ENDPOINT, DB_QUERY_URL, DB_QUERY_USER, DB_QUERY_PASSWORD, lower_bound, upper_bound)
                air_temperature = query_timeseries_within_bounds(air_temperature_iri, QUERY_ENDPOINT, UPDATE_ENDPOINT, DB_QUERY_URL, DB_QUERY_USER, DB_QUERY_PASSWORD, lower_bound, upper_bound)
                dni = query_timeseries_within_bounds(dni_iri, QUERY_ENDPOINT, UPDATE_ENDPOINT, DB_QUERY_URL, DB_QUERY_USER, DB_QUERY_PASSWORD, lower_bound, upper_bound)
            else:
        ...
        #### run model for each buse
        for bus in BUSES:

            logging.info("Bus: "+str(bus))

            pv_iri = QueryData.query_PV_panel_for_bus(bus, UPDATE_ENDPOINT) #update endpoint is the ntuenergy blazegraph

            logging.info("pv_iri: "+ str(pv_iri))

            # Construct and evaluate the model
            model = SolarModel('ModelChain', pv_iri, filepath, QUERY_ENDPOINT, UPDATE_ENDPOINT)

            iri_list = check_data_iris.check_data_iris_and_create_if_not_exist_AC_only(pv_iri, UPDATE_ENDPOINT) #update endpoint is the ntuenergy blazegraph

            # Get panel area
            pv_area = 1
            try:
                pv_area = QueryData.query_PV_area(pv_iri, UPDATE_ENDPOINT) #update endpoint is the ntuenergy blazegraph
                logging.info("area: "+str(pv_area))
            except Exception as ex:
                    logging.error("SPARQL query for area not successful.")
                    raise KGException("SPARQL query for area not successful.") from ex
           ...
           #     print(str(results))
            ...
            #value_list = [ac_power_list, dc_power_list]
            #we just want the ac_power
            value_list = [ac_power_list]
            ...
            try:
                ...
                if not boolean_value:
                    ...
                    #we just want the ac_power
                    timeseries_instantiation.init_timeseries(iri_list, [DATACLASS], TIME_FORMAT, QUERY_ENDPOINT, UPDATE_ENDPOINT, DB_UPDATE_URL, DB_UPDATE_USER, DB_UPDATE_PASSWORD)
                    timeseries_instantiation.link_to_NTU_KG(pv_iri, iri_list, QUERY_ENDPOINT, UPDATE_ENDPOINT)
            except Exception as ex:
                logging.error("Unable to initialise timeseries")
                raise TSException("Unable to initialise timeseries") from ex

            logging.info("timestamp_list: " + str(len(timestamp_list)))
            logging.info("result_iri_list: " + str(len(iri_list)))
            logging.info("result_value_list: " + str(len(value_list)))
            logging.info("result_value_list length of each: " + str(len(value_list[0])))
            logging.info("timestamp_list string: " + str(timestamp_list))
            logging.info("result_iri_list string: " + str(iri_list))
            logging.info("result_value_list string: " + str(value_list))
