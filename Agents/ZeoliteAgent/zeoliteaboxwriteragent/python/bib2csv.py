...
def str2owlsafe(string):
    output = string
    output = output.replace(u"\u03b1", "alpha").replace(u"\u03b2", "beta").replace(u"\u03b3", "gamma")
...
class OntoBibo:
    ...
    def __init__(self, filePath = [], uuidDB = None,
        ...
        if [] == filePath:
            ...
            logging.error(" Unknown file path in OntoBibo: '%s'.", str(filePath))
    ...
    def readBib(self, filePath):
        ...
        if os.path.isdir(filePath):
            ...
            logging.error(" In bib2csv file '%s' does not exist.", filePath)
            self.errCount += 1
            return
        ...
        for entry in library.entries:
            ...
            if entry.key in self.entryInBibFile:
                logging.error(" Bibliography entry repeated in '%s'," +
                               ...
                               filePath, self.entryInBibFile[entry.key])
                self.errCount += 1
            else:
        ...
        for field in entity.fields:
            if   "author" == field.key.lower():
                ...
                output.append([dctPrefix + "title", "Data Property",
                               self.uuid, "", str2owlsafe(field.value), "xsd:string"])
#def str2owlsafe(string):
                ...
                #print(field.key, "not implemented")
                output.append([dctPrefix + "title", "Data Property",
                               jUuid, "", str2owlsafe(field.value), "xsd:string"])
    ...
    def _csvBtpInProceedings(self, entity, subject, predicate, options):
        ...
        logging.error(" Invalid function '%s'", className)
        errCount += 1
