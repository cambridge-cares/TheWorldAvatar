...
address = "http://localhost:8080/blazegraph/namespace/zeo05x/sparql"
...
#LIMIT 10
    ...
    def makeCifFile( self, zeoName, filename ):
        ...
        if not isinstance( zeoName, str ):
            print( "Error! zeoName must be a string, but got '" + str(zeoName) + "'." )
...
#SELECT ?x ?y ?z
SELECT ?zeoname ?afx ?afy ?afz ?a1label #?a1occup #?aF_Unit
WHERE {
  ?zeo       zeo:hasFrameworkCode     '""" +
  zeoName +
  """'.
