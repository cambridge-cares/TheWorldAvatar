...
class HPLCInputAgent(AsyncAgent):
    def monitor_local_report_folder(self):
        ...
        config = HPLCInputAgentConfig(str(Path(__file__).absolute().parent.parent) + '/conf/agent_properties.json')

        # Get the local report folder path of the HPLC
        hplc_dir, file_extension = self.sparql_client.get_hplc_local_report_folder_path(config.HPLC_DIGITAL_TWIN)
        ...
        if currt_time - last_report_time < config.HPLC_REPORT_PERIODIC_TIMESCALE:            
            ...
            with open(hplc_report_path, 'rb') as file_obj:
                ...
                response = requests.post(config.FILE_SERVER_UPLOAD, auth=('fs_user', 'fs_pass'), files=files)
                ...
                if (response.status_code == status_codes.codes.OK):
                    ...
                    hplc_report_instance = HPLCReport(instance_iri=INSTANCE_IRI_TO_BE_INITIALISED,namespace_for_init=getNameSpace(config.HPLC_DIGITAL_TWIN),hasReportPath=remote_file_path)
