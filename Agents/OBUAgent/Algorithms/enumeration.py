...
def merge_csv_files(file1_path, file2_path, output_union):
    ...
    print(f'Merged file saved to {output_union}')
...
def merge_counts(base_path, alg1_path, alg2_path, union_path, output_union):
    ...
    try:
        with open(output_union, 'w', newline='') as f:
            writer = csv.writer(f)
        ...
        print(f'Merged counts saved to {output_union}')
    except Exception as e:
        print(f'Error occurred while writing merged counts to {output_union}: {e}')
