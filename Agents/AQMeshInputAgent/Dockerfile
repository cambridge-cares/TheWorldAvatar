...
COPY --from=builder /root/AQMeshInputAgent/target/aqmesh_inputAgent-1.0.0.jar /app
# Copy the downloaded dependencies from the builder
COPY --from=builder /root/AQMeshInputAgent/target/aqmesh_inputAgent-1.0.0.lib /app/aqmesh_inputAgent-1.0.0.lib

# Run the jar to start the agent
ENTRYPOINT ["java", "-jar", "aqmesh_inputAgent-1.0.0.jar", "config/agent.properties", "config/client.properties", "config/api.properties"]
#==================================================================================================