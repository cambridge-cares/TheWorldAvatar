...
def readText(baseMVAName, busName, genName, branchName, splitCharacter, optimal, areasName, genCostName):
    ...
    ppc["baseMVA"] = numpy.float64(items[0])
    file.close()
    #bus (length)
    length = 0
    file = open(busName, "r")
    ...
    while (readLoop):
        ...
        if (len(singleLine) < 4):
            ...
            length += 1
    ppc["bus"] = numpy.zeros((length, 13), dtype=numpy.float64)
    #bus (values)
    ...
    while (readLoop < length):
        ...
        singleLine = singleLine.strip()
        items = singleLine.split(splitCharacter)
        ppc["bus"][readLoop][0] = numpy.float64(float(items[0])-1.0)
        valueLoop = 1
        while (valueLoop < 13):
            ppc["bus"][readLoop][valueLoop] = numpy.float64(items[valueLoop])
            valueLoop += 1
        readLoop += 1
    file.close()
    #gen (length)
    ...
    while (readLoop):
        ...
        if (len(singleLine) < 4):
            ...
            length += 1
    ppc["gen"] = numpy.zeros((length, 21), dtype=numpy.float64)
    #gen (values)
    ...
    while (readLoop < length):
        ...
        singleLine = singleLine.strip()
        items = singleLine.split(splitCharacter)
        ppc["gen"][readLoop][0] = numpy.float64(float(items[0])-1.0)
        valueLoop = 1
        while (valueLoop < 21):
            ppc["gen"][readLoop][valueLoop] = numpy.float64(items[valueLoop])
            valueLoop += 1
        readLoop += 1
    file.close()
    #branch (length)
    ...
    while (readLoop):
        ...
        if (len(singleLine) < 4):
            ...
            length += 1
    ppc["branch"] = numpy.zeros((length, 13), dtype=numpy.float64)
    #branch (values)
    ...
    while (readLoop < length):
        ...
        singleLine = singleLine.strip()
        items = singleLine.split(splitCharacter)
        ppc["branch"][readLoop][0] = numpy.float64(float(items[0])-1.0)
        ppc["branch"][readLoop][1] = numpy.float64(float(items[1])-1.0)
        valueLoop = 2
        while (valueLoop < 13):
            ppc["branch"][readLoop][valueLoop] = numpy.float64(items[valueLoop])
            valueLoop += 1
        readLoop += 1
    file.close()
    ...
    if (optimal == 1):
        ...
        while (readLoop):
            ...
            if (len(singleLine) < 4):
                ...
                length += 1
        file.close()
        lengths = numpy.zeros((length), dtype=numpy.float64)
        file = open(genCostName, "r")
        readLoop = 0
        while (readLoop > -1):
            singleLine = file.readline()
            ...
            if (len(singleLine) < 4):
                ...
                readLoop += 1
        file.close()
        ppc["gencost"] = numpy.zeros((length, int(lengths[0])), dtype=numpy.float64)
        file = open(genCostName, "r")
        readLoop = 0
        while (readLoop < length):
            singleLine = file.readline()
            ...
            while (valueLoop < int(lengths[0])):
                ppc["gencost"][readLoop][valueLoop] = numpy.float64(items[valueLoop])
                valueLoop += 1
            readLoop += 1
        file.close()
        #print(ppc["gencost"])
...
def mainJAPowerFlow(baseMVAName, busName, genName, branchName, splitCharacter, outputBusName, outputBranchName, outputGenName, printOutput, optimal, areasName, genCostName, convergedOutputName):
    ...
    if (optimal == 1):
        ...
        busGenP = numpy.zeros(busCount, dtype=numpy.float64)
        busGenQ = numpy.zeros(busCount, dtype=numpy.float64)
