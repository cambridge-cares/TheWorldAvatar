        ...
        if(Object.keys(a)[0] === "id"){
            ...
            if(a.iri.includes("https://www.theworldavatar.com/kg/ontopowsys/NTU_Venue")){
                ...
                */
                ...
                //Tuesday
                var iframe = document.createElement("iframe");
                var header = document.createElement("p");  // Using h2 as an example, you can adjust the heading level as needed
                header.textContent = "Tuesday";
                // Set attributes for the iframe
                iframe.src = "http://localhost:3838/analytics/d-solo/a3e4e240-87cc-455e-888a-fd3af4e9e64d/ntu-dashboard?orgId=1&var-schedule=All&from=1672531200000&to=1672617599000&var-classroom=LT"+ a[Object.keys(a)[0]] +"&theme=light&panelId=2";
                iframe.width = "450";
                iframe.height = "100";
                ...
                var iframe = document.createElement("iframe");
                var header = document.createElement("p");  // Using h2 as an example, you can adjust the heading level as needed
                header.textContent = "Wednesday";
                // Set attributes for the iframe
                iframe.src = "http://localhost:3838/analytics/d-solo/a3e4e240-87cc-455e-888a-fd3af4e9e64d/ntu-dashboard?orgId=1&var-schedule=All&from=1672617600000&to=1672703999000&var-classroom=LT"+ a[Object.keys(a)[0]] +"&theme=light&panelId=2";
                iframe.width = "450";
                iframe.height = "100";
                iframe.frameBorder = "0"; // Note: It's frameBorder, not frameborder
                // Append the iframe to the container
                ...
                var iframe = document.createElement("iframe");
                var header = document.createElement("p");  // Using h2 as an example, you can adjust the heading level as needed
                header.textContent = "Thursday";
                // Set attributes for the iframe
                iframe.src = "http://localhost:3838/analytics/d-solo/a3e4e240-87cc-455e-888a-fd3af4e9e64d/ntu-dashboard?orgId=1&var-schedule=All&from=1672704000000&to=1672790399000&var-classroom=LT"+ a[Object.keys(a)[0]] +"&theme=light&panelId=2";
                iframe.width = "450";
                iframe.height = "100";
                iframe.frameBorder = "0"; // Note: It's frameBorder, not frameborder
                // Append the iframe to the container
                ...
                var iframe = document.createElement("iframe");
                var header = document.createElement("p");  // Using h2 as an example, you can adjust the heading level as needed
                header.textContent = "Friday";
                // Set attributes for the iframe
                iframe.src = "http://localhost:3838/analytics/d-solo/a3e4e240-87cc-455e-888a-fd3af4e9e64d/ntu-dashboard?orgId=1&var-schedule=All&from=1672876800000&to=1672963199000&var-classroom=LT"+ a[Object.keys(a)[0]] +"&theme=light&panelId=2";
                iframe.width = "450";
                iframe.height = "100";
                iframe.frameBorder = "0"; // Note: It's frameBorder, not frameborder from=1672876800000&to=1672963199000
                // Append the iframe to the container
                //container.appendChild(header);
                container.appendChild(iframe);
                */
...
class MapHandler_Cesium extends MapHandler {
    ...
    initialiseMap(mapOptions) {
        ...
        if (!MapHandler.MAP) {
            ...
            /* Original screenSpace event handler without click to show coordinate
            ...
            */
            ...
            screenSpaceEventHandler.setInputAction(event => {
                ...
                let ray = MapHandler.MAP.scene.camera.getPickRay(event.position);
                let cartesian = MapHandler.MAP.scene.globe.pick(ray, MapHandler.MAP.scene);
                if (cartesian) {
            ...
            console.log("MapHandler.MAP? ", MapHandler.MAP);
    ...
    addClassScheduleButton(){
            ...
            /*
            const htmlOverlay = document.getElementById("getClassScheduleButton");
            const scratch = new Cesium.Cartesian2();
            MapHandler.MAP.scene.preRender.addEventListener(function () {
              const position = Cesium.Cartesian3.fromDegrees(103.6764504427, 1.3413099874);
    ...
    addWeatherDataButton(){

            const htmlOverlay = document.getElementById("getWeatherDataButton");
            const scratch = new Cesium.Cartesian2();
            MapHandler.MAP.scene.preRender.addEventListener(function () {
              const position = Cesium.Cartesian3.fromDegrees(103.6904125335, 1.3506208625);
              const canvasPosition = MapHandler.MAP.scene.cartesianToCanvasCoordinates(
                position,
                scratch
              );
    ...
    addOPFButton(){
        ...
        const scratch = new Cesium.Cartesian2();
        MapHandler.MAP.scene.preRender.addEventListener(function () {
          const position = Cesium.Cartesian3.fromDegrees(103.6904445966, 1.3493475184);
          const canvasPosition = MapHandler.MAP.scene.cartesianToCanvasCoordinates(
            position,
            scratch
          );
        ...
        document.getElementById('runOPFButton').addEventListener('click', function() {
            alert('Running Optimal Power Flow Agent...');
    ...
    addPVButton(){
        ...
        const htmlOverlay = document.getElementById("runPVButton");
        const scratch = new Cesium.Cartesian2();
        MapHandler.MAP.scene.preRender.addEventListener(function () {
        ...
        document.getElementById('runPVButton').addEventListener('click', function() {
            alert('Calculating solar panel output...');
...
handleClick(i) {
    if (MapHandler.ALLOW_CLICKS) {
        console.log("this allows click");
        let n = this;
        CesiumUtils.getFeature(i, function(a) {
            console.log("can get feature");
...
//add geojson and color the route with different colors
addGeoJSONFile(e, t) {
    Cesium.GeoJsonDataSource.load(e.uri)
        .then(e => {
            // Set visibility of the data source
            e.show = null == t.definition.visibility || "visible" === t.definition.visibility;
            // Assign layer ID to the data source
            ...
            console.info("Added GeoJSON source to map with layer ID: " + t.id);
            ...
            e.entities.values.forEach(function(entity) {
                // Check if the entity is a polyline
                if (Cesium.defined(entity.polyline)) {
                    // Access the "highway" property using getValue()
                    let highwayType = entity.properties.highway.getValue();

                    // Assign colors based on the "highway" property
                    switch (highwayType) {
                        case "residential":
...
/*
// just normally add geojson file to the map
addGeoJSONFile(e, t) {
    Cesium.GeoJsonDataSource.load(e.uri)
        .then(e => {
            // Set visibility of the data source
...
*/
    ...
    addTileset(e, t) {
        ...
        console.log("printing e:", e.uri);

        var data = [];
        // Color building related to power network
        /*
        var data = [
            {id: 120, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_SBS", color: "color('rgb(70,130,180)')"}, // Steel Blue
            {id: 40, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_N1_3", color: "color('rgb(34,139,34)')"}, // Forest Green
            {id: 442, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_NYA", color: "color('rgb(188,143,143)')"}, // Rosy Brown
            {id: 47, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_N_2", color: "color('rgb(210,105,30)')"}, // Chocolate
            {id: 454, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_N_2_1", color: "color('rgb(147,112,219)')"}, // Medium Purple
            {id: 488, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_SPMS", color: "color('rgb(72,209,204)')"}, // Medium Turquoise
            {id: 17, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_HALL_4", color: "color('rgb(255,105,180)')"}, // Hot Pink
            {id: 387, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_PIONEER_HALL", color: "color('rgb(255,165,0)')"}, // Orange
            {id: 277, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_THE_WAVE", color: "color('rgb(218,165,32)')"}, // Golden Rod
            {id: 190, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_CANTEEN_2", color: "color('rgb(205,133,63)')"}, // Peru
            {id: 1, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_EMB", color: "color('rgb(85,107,47)')"}, // Dark Olive Green
            {id: 92, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_RTP", color: "color('rgb(176,224,230)')"}, // Powder Blue
            {id: 399, iriValue: "https://www.theworldavatar.com/kg/ontopowsys/NTU_Building_NEC", color: "color('rgb(221,160,221)')"} // Plum
        ];
        */
            ...
            function processArray(index) {
                if (index < data.length) {
                    return $.ajax({
                        url: "http://localhost:3838/feature-info-agent/get",
                        data: { iri: data[index].iriValue },
                        dataType: 'json'
                    })
                    .then(function(e) {
                        ...
                        let t;
                        if (e === null || Array.isArray(e) && e.length === 0 || Object.keys(e).length === 0) {
                            // If the response is empty or null, handle it here
                            // i.showBuiltInData(a);
                        } else {
                            t = e.meta;
                            e = e.time;
                    ...
                    .then(function() {
                        // Regardless of success or failure, move on to the next item
            ...
            processArray(0)
            .then(function() {
                // Build conditions directly from data
                var conditions = data.map(item => {
                    return ["${id} === " + item.id, item.color];
                });

                // Add the default case
                conditions.push(["true", "color('AZURE')"]);
    ...
    addPointsAndLabels() {
      ...
      function createPointWithLabel(name, longitude, latitude) {
      ...
      // Create your points here...
        const SPMS = createPointWithLabel("SPMS", 103.6818313449, 1.3418622912);
        const Gaia = createPointWithLabel("Gaia", 103.6832968323, 1.3420784334);
        const EEE = createPointWithLabel("EEE", 103.6808374316, 1.3432265836);
        const NYA = createPointWithLabel("NYA", 103.6802170967, 1.3444732784);
        const SBS = createPointWithLabel("SBS", 103.6790837070, 1.3446677129);
        const RSIS = createPointWithLabel("RSIS", 103.6828289455, 1.3445778104);
        const SOH = createPointWithLabel("SOH", 103.6832219492, 1.3436959528);
        const WKWSCI = createPointWithLabel("WKWSCI", 103.6798157027, 1.3422039360);
        const SCSE = createPointWithLabel("SCSE", 103.6820219077, 1.3462758314);
        const CCEB = createPointWithLabel("CCEB", 103.6814839745, 1.3423278539);
        const ADM = createPointWithLabel("ADM", 103.6836713722, 1.3494452508);
        const MSE = createPointWithLabel("MSE", 103.6828206330, 1.3466488196);
        const MAE = createPointWithLabel("MAE", 103.6815714070, 1.3471301712);
        const SRC = createPointWithLabel("SRC", 103.6877626788, 1.3493380482);
        const Hall4 = createPointWithLabel("Hall4", 103.6859727443, 1.3443150373);
        const Hall5 = createPointWithLabel("Hall5", 103.6871059725, 1.3443975857);
        const Hall1 = createPointWithLabel("Hall1", 103.6874975524, 1.3454683228);
        const CrescentHall = createPointWithLabel("Crescent Hall", 103.6882582783, 1.3461486472);
        const Hall16 = createPointWithLabel("Hall16", 103.6811622680, 1.3503034094);
        const Hall3 = createPointWithLabel("Hall3", 103.6820282620, 1.3506749592);
        const Hall12 = createPointWithLabel("Hall12", 103.6807031375, 1.3511813697);
        const Hall13 = createPointWithLabel("Hall13", 103.6813738353, 1.3517889450);
        const Hall14 = createPointWithLabel("Hall14", 103.6820429886, 1.3523370583);
        const NIE = createPointWithLabel("NIE", 103.6783990817, 1.3488637461);
        const ASE = createPointWithLabel("ASE", 103.6793951143, 1.3467359942);
        const CEE = createPointWithLabel("CEE", 103.6801307860, 1.3458486846);
        const GH2 = createPointWithLabel("GH2", 103.6859232949, 1.3556889014);
        const Hall11 = createPointWithLabel("Hall11", 103.6864649318, 1.3547248513);
        const Hall10 = createPointWithLabel("Hall10", 103.6856292927, 1.3540245185);
        const Hall9 = createPointWithLabel("Hall9", 103.6854104020, 1.3525671634);
        const Hall8 = createPointWithLabel("Hall8", 103.6851604620, 1.3509547080);
        const NYV = createPointWithLabel("Nanyang View", 103.6874021080, 1.3523853151);
        const NYM = createPointWithLabel("Nanyang Meadows", 103.6891537361, 1.3513951034);
        const NEC = createPointWithLabel("NEC", 103.6879722516, 1.3512641616);
        const NYH = createPointWithLabel("NYH", 103.6867896898, 1.3498278509);
        const Hall6 = createPointWithLabel("Hall6", 103.6870895070, 1.3478470168);
        const Hall2 = createPointWithLabel("Hall2", 103.6861317072, 1.3478659302);
        const TLSK = createPointWithLabel("Tan Lark Sye Walk", 103.6840865406, 1.3460192930);
        const SSC = createPointWithLabel("SSC", 103.6833743723, 1.3454324190);
        const UHSB = createPointWithLabel("UHSB", 103.6825848272, 1.3454364470);



        // After all points are created, create the tube
        /*
        const waterTube = MapHandler.MAP.entities.add({
          name: "Water tube with rounded corners",
          polylineVolume: {
            positions: Cesium.Cartesian3.fromDegreesArray(coordinates),
            shape: computeCircle(3.0), // Increase the radius
            material: new Cesium.Color(0.0, 0.5, 1.0, 1.0), // Semi-transparent blue color
            /*
            material: new Cesium.PolylineGlowMaterialProperty({
              glowPower: 0.2, // between 0.0 and 1.0
              color: new Cesium.Color(0.0, 0.5, 1.0, 1.0), // Create a custom color
            }),
            */
        /*
          },
...
addWMS(e, t) {
    // Check if 'wmsLayer' parameter exists, if not, log an error
    var a = e.wmsLayer;
    if (a == null) {
    ...
    // Create a new WMS imagery provider with given parameters and assign it to 'n'

    var n = new Cesium.WebMapServiceImageryProvider({
        url: e.uri,
        layers: a,
        parameters: {
    ...
    /*
    var n = new Cesium.WebMapServiceImageryProvider({
              url: 'http://localhost:3838/geoserver/the_world_avatar/wms',
              layers: 'the_world_avatar:ntupowerfinal,the_world_avatar:arrows1,the_world_avatar:arrows2,the_world_avatar:arrows3,the_world_avatar:arrows4,the_world_avatar:arrows5,the_world_avatar:arrows6,the_world_avatar:arrows7,the_world_avatar:arrows8,the_world_avatar:arrows9, the_world_avatar:arrows10, the_world_avatar:arrows11,the_world_avatar:arrows12,the_world_avatar:arrows13,the_world_avatar:arrows14',
              parameters: {
                service: 'WMS',
                version: '1.1.0',
                request: 'GetMap',
    ...
    */
    ...
    // Get imageryLayers from the map
    var i = MapHandler.MAP.imageryLayers;


    // Add the new WMS layer to the map
    i.addImageryProvider(n);

    // Loop through all layers and make the newly added layer visible if its visibility is set to 'visible'
    for (let e = 0; e < i.length; e++) {
        if (i.get(e).imageryProvider === n) {
            i.get(e).show = (t.definition.visibility == null || t.definition.visibility === "visible");
        }
    }

    // If the DATA_SOURCES does not already have an entry for this layer id, create an empty array for it
    if (MapHandler_Cesium.DATA_SOURCES[t.id] == null) {
        MapHandler_Cesium.DATA_SOURCES[t.id] = [];
    }

    // Push the newly created WMS layer into DATA_SOURCES
    MapHandler_Cesium.DATA_SOURCES[t.id].push(n);
}
...
class CesiumUtils {
    ...
    /*
    static generateDefaultImagery() {
        const mapAPI = MapHandler.MAP_API;
        const e = {
            Light: "https://api.mapbox.com/styles/v1/mapbox/light-v11/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            Dark: "https://api.mapbox.com/styles/v1/mapbox/dark-v10/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            Outdoors: "https://api.mapbox.com/styles/v1/mapbox/outdoors-v12/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            "Satellite (Raw)": "https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            "Satellite (Labelled)": "https://api.mapbox.com/styles/v1/mapbox/satellite-streets-v11/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            Streets: "https://api.mapbox.com/styles/v1/mapbox/streets-v12/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            Custom: "https://api.mapbox.com/styles/v1/tianmingchuan/cll0g741u00on01qg017q07ry/tiles/256/{z}/{x}/{y}?access_token=${mapAPI}",
            default: "Dark"
        };
        Manager.SETTINGS.putSetting("imagery", e);
    }
    */
    ...
    static getFeature(event, callback) {
        ...
        // Use drillPick instead of pick to get the feature under the heatmap (3/7/2023)
        //click behind the entity!
        //const pickedObject = MapHandler.MAP.scene.drillPick(event.position || event.endPosition)[1];

        if (Cesium.defined(pickedObject)) {
            // Handle the picked object here
            console.log(pickedObject);
        }
        if (pickedObject == null) {
            //console.log("picked object is null");
            ...
            //console.log("picked object is not null");
