...
#
# rxnoptgoaliteragent release script
#
...
VENV_NAME='rogi_agent_venv'
TEST_VENV_NAME='test_venv'
PROJECT_NAME='rxnoptgoaliteragent'
...
usage() {
    ...
    echo ""
    echo "Example usage:"
    echo "./release_to_pypi.sh -v 0.0.5   - release version 0.0.5"
    echo "./release_to_pypi.sh -d 0.0.5a  - release version 0.0.5a to the TestPyPI repository"
    echo "==============================================================================================================="
...
main() {
    ...
    bump_rxnoptgoaliteragent_version_number
    install_rxnoptgoaliteragent_and_test
    build_rxnoptgoaliteragent_for_release
...
run_rxnoptgoaliteragent_tests() {
    ...
    echo
    echo $1
    $1 -m pytest --docker-compose=./docker-compose.test.yml --reruns 5 --reruns-delay 5
    TEST_RESULT=$?
...
install_rxnoptgoaliteragent_and_test() {
    ...
    echo
    echo
    install_rxnoptgoaliteragent $VENV_NAME $SPATH

    # run_rxnoptgoaliteragent_tests $PYTHON_EXEC
...
dev_release() {
    ...
    install_packages_for_building
    dev_release_bump_rxnoptgoaliteragent_version_number
    dev_release_install_rxnoptgoaliteragent_and_test
    build_rxnoptgoaliteragent_for_release
    release_to_pypi test-pypi
    read -n 1 -s -r -p "Press any key to continue"
    exit
...
dev_release_install_rxnoptgoaliteragent_and_test() {
    ...
    dev_release_rxnoptgoaliteragent_tests $PYTHON_EXEC

    STEP_NR=$((STEP_NR+1))
}

dev_release_rxnoptgoaliteragent_tests() {
    echo "-------------------------------"
    echo "Running the $PROJECT_NAME tests"
    echo "-------------------------------"
    echo
    echo
    echo $1
    $1 -m pytest -s --docker-compose=./docker-compose.test.yml --reruns 5 --reruns-delay 5
    TEST_RESULT=$?
    if [ $TEST_RESULT -eq 0 ]; then
        echo "All tests have completed successfully"
    else
