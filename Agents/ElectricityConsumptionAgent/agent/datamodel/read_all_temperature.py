...
def read_all_temperature_2021(limit = False):
    ...
    def read_nc(var_name,loc=True):
        ...
        and returns the grid of observations.
        '''
        fn = './Data/Climate Files/'+var_name+'_hadukgrid_uk_1km_mon_202101-202112.nc'
        ds = nc.Dataset(fn)
        var_grid = ds.variables[var_name][:]
        if loc == True:
            lon = ds.variables['longitude'][:]
    ...
    def from_grid_find_temp(lon,lat,nc_vars,month,overall_centroids):
        ...
        missing_data = []
        for idx, point in enumerate(tqdm(overall_centroids)):
    ...
    for i in tqdm(range(int(len(LSOA)))):
            ...
            for month_it in range(len(months)):
                ...
                startUTC = '2021-'+month_str+'-01T12:00:00'
                for var in range(len(LSOA_vars)):
                    clim_var = clim_vars[var]
...
def read_all_temperature_original(limit = False):
