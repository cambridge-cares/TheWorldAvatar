...
/**
 * Provides functions to classify the IfcCovering class into its correct subtype.
 *
 * @author qhouyee
 */
public class CoveringClassifier {
     ...
     *
     * @param bimClass     Their ontoBIM class in format "bim:Class".
    ...
    private static String createSelectQuery() {
        ...
        selectBuilder.addVar("?covering");
        selectBuilder.addWhere("?covering", "rdf:type", iriClass);
    ...
    private static void retrieveAndAddClassMap(String queryString, Model queryModel, Map<String, String> classMapping) {
        ...
        while (results.hasNext()) {
            ...
            String covering = StringUtils.getStringAfterLastCharacterOccurrence(
                    soln.get("covering").toString(), "/");
            // If the IRI has an # termination character, this will ensure that it is parsed and removed
            covering = StringUtils.getStringAfterLastCharacterOccurrence(covering, "#");
            classMapping.put(covering, bimName);
        }
    }
}
