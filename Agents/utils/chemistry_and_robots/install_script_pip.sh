#!/bin/bash
# J. Bai (jb2197@cam.ac.uk), based on https://github.com/cambridge-cares/TheWorldAvatar/blob/main/JPS_BASE_LIB/python_wrapper/install_script_pip.sh provided by D. Nurkowski (danieln@cmclinnovations.com)

AUTHOR="Jiaru Bai <jb2197@cam.ac.uk>, based on https://github.com/cambridge-cares/TheWorldAvatar/blob/main/JPS_BASE_LIB/python_wrapper/install_script_pip.sh provided by Daniel Nurkowski <danieln@cmclinnovations.com>"
SPATH="$( cd  "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
CREATE_VENV='n'
VENV_NAME='chemistry_and_robots_venv'
VENV_DIR=$SPATH
DEV_INSTALL=''
PROJ_NAME='chemistry_and_robots'
...
function install_agentlogging_workaround {
    ...
    echo "run_chemistry_and_robots_tests() and release_to_pypi(),"
