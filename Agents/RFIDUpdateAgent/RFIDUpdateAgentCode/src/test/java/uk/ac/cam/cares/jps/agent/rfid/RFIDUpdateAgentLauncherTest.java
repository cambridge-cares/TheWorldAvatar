...
public class RFIDUpdateAgentLauncherTest {

	
	private static final Logger LOGGER = LogManager.getLogger(RFIDUpdateAgentLauncherTest.class);
    ...
    private final String launcherPropertiesFilename = "launcher.properties";
    ...
    public void initializePropertyFile() throws IOException {
        ...
        File launcherPropertyFile = folder.newFile(launcherPropertiesFilename);
        ...
        String launcherPropertiesFile = launcherPropertyFile.getCanonicalPath();
        args = new String[] {agentPropertiesFile, clientPropertiesFile, apiPropertiesFile, launcherPropertiesFile};
    ...
    public void testProcessRequestParams() throws IOException {
    	RFIDUpdateAgentLauncher testLauncher = new RFIDUpdateAgentLauncher();
    ...
    testRequestParams.put("launcherProperties", "TEST_LAUNCHERPROPERTIES");
    ...
    testRequestParams.put("agentProperties", "TEST_AGENTPROPERTIE"); //missing a s in PROPERTIES
    testRequestParams.put("apiProperties", "TEST_APIPROPERTIES");
    testRequestParams.put("clientProperties", "TEST_CLIENTPROPERTIES");
    testRequestParams.put("launcherProperties", "TEST_LAUNCHERPROPERTIES");
    ...
    public void testMainNoArgs() {
        String[] args = {};
        try {
        	RFIDUpdateAgentLauncher.initializeAgent(args);
            Assert.fail();
        }
        catch (JPSRuntimeException e) {
            Assert.assertEquals("Need four properties files in the following order: 1) input agent 2) time series client 3) API connector 4) launcher properties.",
                    e.getMessage());
        }
    }
    ...
    public void testMainInvalidAgentPropertyFile() {
        ...
        try {
            ...
            Assert.assertEquals("The RFID Update agent could not be constructed!", e.getMessage());
        }
    }
    ...
    public void testMainErrorWhenCreatingAPIConnector() throws IOException {
        ...
        try(MockedConstruction<RFIDUpdateAgent> mockAgent = Mockito.mockConstruction(RFIDUpdateAgent.class)) {
            ...
            try {
                ...
                Assert.assertEquals("Could not construct the RFID API connector needed to interact with the API!", e.getMessage());
            }
        }
    ...
    public void testMainErrorWhenRetrievingReadings() throws IOException {
        ...
        createProperAPIPropertiesFile();
        // Use a mock for the input agent
        try(MockedConstruction<RFIDUpdateAgent> ignored = Mockito.mockConstruction(RFIDUpdateAgent.class)) {
            // Use a mock for the connector that throws an exception when readings are requested
            try(MockedConstruction<RFIDUpdateAPIConnector> mockConnector = Mockito.mockConstruction(RFIDUpdateAPIConnector.class,
    ...
    private void createProperAgentPropertiesFile() throws IOException {
        ...
        try (FileWriter writer = new FileWriter(propertiesFile, false)) {
            writer.write("rfid.mappingfolder=TEST_MAPPINGS");
        }
    }
    ...
    private void createProperAPIPropertiesFile() throws IOException {
        ...
        try (FileWriter writer = new FileWriter(propertiesFile, false)) {
            writer.write("limit=3\n");
            writer.write("variable=status\n");
            writer.write("path.url=http://localhost:9090\n");
            writer.write("keys=f6e2fac0-4b9a-11ec-bc75-d14ed3d0d946");
           
        }
    }
    
    private void createProperLauncherPropertiesFile() throws IOException {
        // Filepath for the properties file
        String propertiesFile = Paths.get(folder.getRoot().toString(), launcherPropertiesFilename).toString();
        try (FileWriter writer = new FileWriter(propertiesFile, false)) {
            writer.write("keys=tag_12345_status\n");
