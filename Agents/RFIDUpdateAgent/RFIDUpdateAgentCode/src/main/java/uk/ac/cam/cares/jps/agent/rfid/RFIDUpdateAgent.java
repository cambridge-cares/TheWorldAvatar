...
public class RFIDUpdateAgent{
     ...
     */
    public RFIDUpdateAgent(String propertiesFile) throws IOException {
    ...
    private void readMappings(String mappingFolder) throws IOException {
        ...
        // Create a mapper for each file
            ...
            for (File mappingFile: mappingFiles) {
                JSONKeyToIRIMapper mapper = new JSONKeyToIRIMapper(RFIDUpdateAgent.generatedIRIPrefix, mappingFile.getAbsolutePath());
                mappings.add(mapper);
     ...
     */
    public void updateData(JSONObject RFIDVariableReadings, String keys)throws IllegalArgumentException {
        ...
        if(!RFIDVariableReadingsMap.isEmpty() && !timeStampReadingsMap.isEmpty()) {
            List<TimeSeries<OffsetDateTime>> timeSeries;
          
            timeSeries = convertReadingsToTimeSeries(RFIDVariableReadingsMap, timeStampReadingsMap, keys);
     ...
     */
    public Map<String, List<?>> jsonObjectToMapForTimeStamp(JSONObject readings, String keyValues) {
        ...
        if (readings.has(keyValues) == true) {
        String[] keys = new String[readings.getJSONArray(keyValues).length()];
        JSONArray tsAndValue = new JSONArray();
        
                tsAndValue = readings.getJSONArray(keyValues);  
     ...
     */
    private Map<String, List<?>> jsonObjectToMap(JSONObject readings, String keyValues) {
        // First save the values as Object //
    	//readingsMap is a map with keys as String and each key is mapped to a list of objects.
        ...
        if (readings.has(keyValues) == true) {
                JSONArray tsAndValue = readings.getJSONArray(keyValues);
                //Go through the JSON objects in the array one by one
                for (int j = tsAndValue.length() - 1; j >= 0; j--) {
                // Get the value and add it to the corresponding list
                	...
                	try { 
                		value = timeSeriesEntry.get("value");
                				...
                				throw new JSONException ("The key value cannot be found in the JSONObject.");
                			}
                //The values are of string type in the JSON Object, convert them to double
                	try {
                    	value = (timeSeriesEntry.get("value").toString());
                    	}
                ...
                // If the key is not present yet initialize the list
                if (!readingsMap.containsKey(keyValues)) {
                    readingsMap.put(keyValues, new ArrayList<>());
                }
                readingsMap.get(keyValues).add(value);
            }
            

        // Convert the values to the proper datatype //
        
        for (String key1: readingsMap.keySet()) {
            // Get the class (datatype) corresponding to the key
            String datatype = getClassFromJSONKey(key1).getSimpleName();
            // Get current list with object type
            List<Object> valuesUntyped = readingsMap.get(key1);
            List<?> valuesTyped;
            // Use mapping to cast the values into integer, double, boolean or string
            // The Number cast is required for org.json datatypes
            if (datatype.equals(Double.class.getSimpleName())) {
     ...
     */
    private List<TimeSeries<OffsetDateTime>> convertReadingsToTimeSeries(Map<String, List<?>> RFIDVariableReadings,
                                                                        Map<String, List<?>> TimestampReadings, String keys
                                                                        )
            {
        ...
        List<OffsetDateTime> allTimestamps = TimestampReadings.get(RFIDUpdateAgent.timestampKey).stream()
        ...
        for (JSONKeyToIRIMapper mapping: mappings) {
            ...
            for(String key: mapping.getAllJSONKeys()) {
                ...
                if (RFIDVariableReadings.containsKey(key)) {
                    values.add(RFIDVariableReadings.get(key));
                }
    ...
    private OffsetDateTime convertStringToOffsetDateTime(String timestamp) {
        ...
        return OffsetDateTime.of(localTime, RFIDUpdateAgent.ZONE_OFFSET);
    }
    ...
    private Class<?> getClassFromJSONKey(String jsonKey) {
    	...
    	Class<?> a = null;
        if (jsonKey.contains(timestampKey) || jsonKey.contains("status")) {
        	a = String.class;
        }
     return a;   
    }
