...
public class TimeSeriesRDBClientIntegrationTest {
	...
	public void testGetMinMaxTimes() throws SQLException {
		...
		String finalIri = iri;
		JPSRuntimeException e = Assert.assertThrows(JPSRuntimeException.class,
				() -> client.getMinTime(finalIri, conn));
		Assert.assertEquals(JPSRuntimeException.class, e.getClass());
	...
	public void testDeleteTimeSeries() throws NoSuchFieldException, IllegalAccessException, SQLException {
		...
		// Check that timeseries table was correctly created (additionally to central
		// lookup table)
		Assert.assertEquals(2, context.meta().getTables().size());
		...
		for (String i : dataIRI_1) {
			...
			String tstable = context.select(tsTableNameColumn).from(table).where(dataIRIcolumn.eq(i))
					.fetch(tsTableNameColumn).get(0);
			String tscolumn = context.select(columnNameColumn).from(table).where(dataIRIcolumn.eq(i))
					.fetch(columnNameColumn).get(0);
		...
		// Delete first dataIRI column (1st out of 3): Verify correct deletion in
		// central lookup and time series table
		List<String> iris = new ArrayList<>(dataIRI_1);
		String iri = iris.remove(0);
		String deleted_column = context.select(columnNameColumn).from(table).where(dataIRIcolumn.eq(iri))
				.fetch(columnNameColumn).get(0);
		...
		// Delete second dataIRI column (2nd out of 3): Verify deletion of correct
		// amount of columns only
		iri = iris.remove(0);
		String tstable = context.select(tsTableNameColumn).from(table).where(dataIRIcolumn.eq(iri))
				.fetch(tsTableNameColumn).get(0);
		...
		String finalIri = iri;
		JPSRuntimeException e = Assert.assertThrows(JPSRuntimeException.class,
				() -> client.deleteTimeSeries(finalIri, conn));
		Assert.assertEquals(JPSRuntimeException.class, e.getClass());
		Assert.assertEquals("TimeSeriesRDBClient: <" + iri + "> does not have an assigned time series instance",
				e.getMessage());
	...
	public void testDeleteTimeSeriesTable() throws NoSuchFieldException, IllegalAccessException, SQLException {
		...
		// Check that timeseries tables were correctly created (additionally to central
		// lookup table)
