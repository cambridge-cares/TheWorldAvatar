...
public class RenamingToolTempStore {
	//********************************************
		
	boolean split = true;
	public int stepSize = 500000;
	
		public void perform(KnowledgeBaseClient sourceKB, KnowledgeBaseClient targetKB) throws ParseException {
			...
			long constructDuration = 0;
			long renameDuration = 0;
			...
			for(int offset = 0; offset<steps; offset++) {
				...
				Query construct = buildSparqlConstruct(stepSize);
				Model triples = sourceKB.queryConstruct(construct);		
				
				long endTime = System.nanoTime();
				constructDuration += (endTime - startTime)/1000000000;
				
				//build delete before rename
				UpdateBuilder deleteBuilder = new UpdateBuilder();
				deleteBuilder.addDelete(triples);	
				UpdateRequest deleteRequest = deleteBuilder.buildRequest();
				
				startTime = System.nanoTime();
				
				//rename intermediate
				UpdateRequest update = buildSparqlUpdateString(null);
				Dataset dataset = DatasetFactory.create(triples);
				UpdateProcessor updateExec = UpdateExecutionFactory.create(update, dataset);
				updateExec.execute();
				
				endTime = System.nanoTime();
				renameDuration += (endTime - startTime)/1000000000;
				
				startTime = System.nanoTime();
				
				//insert triples to target
				UpdateBuilder insertBuilder = new UpdateBuilder();
				insertBuilder.addInsert(dataset.getDefaultModel());
				UpdateRequest insertRequest = insertBuilder.buildRequest();
				targetKB.executeUpdate(insertRequest);
				
				endTime = System.nanoTime();
				insertDuration += (endTime - startTime)/1000000000;
				
				startTime = System.nanoTime();
				
				//delete triples from source
				sourceKB.executeUpdate(deleteRequest);
				
				endTime = System.nanoTime();
				deleteDuration += (endTime - startTime)/1000000000;
			}
			
			//Blank nodes
			//Get model using construct query
			Query constructAll = buildSparqlConstructAll();
			Model blankTriples = sourceKB.queryConstruct(constructAll);		
			//rename intermediate
			UpdateRequest update = buildSparqlUpdateString(null);
			Dataset dataset = DatasetFactory.create(blankTriples);
			UpdateProcessor updateExec = UpdateExecutionFactory.create(update, dataset);
			updateExec.execute();
			//insert triples to target
			UpdateBuilder insertBuilder = new UpdateBuilder();
			...
			UpdateRequest insertRequest = insertBuilder.buildRequest();
			targetKB.executeUpdate(insertRequest);
			//delete remaining triples from source
			sourceKB.executeUpdate("DELETE {?s ?p ?o} WHERE {?s ?p ?o}");
			
			System.out.println("construct duration: "+constructDuration);
			System.out.println("rename duration: "+renameDuration);
			System.out.println("insert duration: "+insertDuration);
			System.out.println("delete duration: "+deleteDuration);
		}
		 ...
		 */
		private static Query buildSparqlConstruct(int limit) throws ParseException {
			...
			WhereBuilder where = new WhereBuilder()
					.addWhere(S, P, O)
					.addFilter("!isblank(?S) && !isblank(?P) && !isblank(?O)");
		 ...
		 * @throws ParseException 
		 */
		private static Query buildSparqlConstructAll() throws ParseException {
	...
	//********************************************
		...
		*/
		 ...
		 * */
		public RenamingToolTempStore(String strTarget, String strReplacement) {
		 ...
		 * @param strReplacement 	replacement string or uri
		 * */
		public RenamingToolTempStore(String strMatch, String strTarget, String strReplacement) {
		...
		*/
		...
		private String countQuery(String graph) throws ParseException {
