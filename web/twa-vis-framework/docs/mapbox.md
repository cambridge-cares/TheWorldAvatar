...
Unfortunatly, vector graphics are not supported so icons must be generated as PNG or JPG files (we recommend creating the initial icons as vectors, then rasterising them to PNG files).
...
These are effectively PNG icons that only contains transparency and a single color; when used it allows Mapbox to dynamically change the colour of icons based on their dynamic styling system. 
