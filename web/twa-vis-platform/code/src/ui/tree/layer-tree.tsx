 ...
 */
function recurseParseTreeStructure(results: TreeGroup[], dataGroup: DataGroup, parentGroup: TreeGroup) {
    const treeGroup: TreeGroup = {
        name: dataGroup.name,
        address: (parentGroup == null) ? dataGroup.name : (parentGroup.address + "." + dataGroup.name),
...
function buildGroup(item: TreeGroup, depth: number): React.ReactElement {
    ...
    return (
        <div
            className={styles.treeEntry}
            key={item.name}>

            <div className={styles.treeEntryHeader} onClick={(e) => clickAction(e)}>
                {/* Spacer */}
                <span style={{width: spacing }}/>
