! <opifil.f90 - A component of the City-scale
...
!    30 Jul 2019  M. Karl: L119           intel-compatible check for missing filename
!
! ----------------------------------------------------------------------------------
...
! If first character is blank then no file
      ...
      IF (FN(1:1) .EQ. ' ') FE = .FALSE.

!MSK start 30.07.2019
      IF (FN(1:1) .EQ. CHAR(0)) FE = .FALSE.
!MSK end
...
! Binary mode
              print *,'opifil binary file name unit',UN,FN
...
! ASCII mode
              ...
              print *,'File: ',FN,'Unit ',UN,' try to open in ascii format.'
              OPEN (UNIT=UN,FILE=FN,ACTION='READ',ERR=9000)
              print *,'File: ',FN,' opened in ascii format.'
          ENDIF
...
! If error in open of file then stop
      ...
      IF (MESSFE) THEN
          WRITE (MESSUN,*) 'OPIFIL: Error in open of input file: ',  &
                          (FN(I:I),I=1,INDEX(FN,' ') - 1)
      ENDIF
      CALL STOPIT('OPIFIL: Error in open of input file!')
