! <wstatr.f90 - A component of the City-scale
...
!       The subroutine writes receptor hourly mean concentrations to file.
! ----------------------------------------------------------------------------------
! REVISION HISTORY
!
...
! If no receptor concentration file then return
         ...
         !print *,'wstatr, cmpd nr: ',cpnstat,NR

!      IF (RSTAFN(1)(1:1) .EQ. ' ') RETURN
!
...
!MSK six compounds for daily output

!          do 120 ip = 1, ncstat

!            print *,'wstatr, ic: ',ic, cmpnd(ic),ip,cpnstat(ip)

!            if ( cmpnd(ic) .eq. cpnstat(ip)  )  then


! Go through all receptor points (only first 100)
!              DO 110 IR = 1, 100


! Write receptor daily average concentration

!                 WRITE (RSTAUN(ip),2120) MOD(YEAR,100),MNTH,DAYM,HOUR,   &
!                             ' IC = ',IC,' IR = ',IR,     &
!                             CRAVED(IC,IR)


! Write the netCDF output file of all species, hourly receptor points concentration average
             ...
             if (ND2STAFE) then
                ...
                dopacking = .false.
                !dofloat   = .false.
                ! output type float will increase file size by factor of 4
                dofloat   = .true.
                domirror  = .false.
...
!MSK debug                        print *,'wstatr CR hourly IC ',    IC,J,I,CRAVED(ic,K),field2D(J,I,1)
!MSK debug                      endif
!MSK debug end
                ...
                call writeconcfield(ND2STAFN, namefield,unitname,field2D(1:NX*facres,1:NY*facres,1) , NX*facres ,NY*facres, 1,   &
...
! Number of stations = STATIONS
              ...
              ! print *,'wcstatr: write stations', ND3STAFE
...
!!===============================================================
                ...
                call writeconcfield(ND3STAFN, namefield,unitname,field1D(1:STATIONS,1:STATIONS, 1),       &
...
! Deallocate
 ...
 2100 format ('WSTATR: Write receptor  daily average concentrations',     &
         ' for date ',3I2.2)

 2120 format (4I3,A6,I3,A6,I7,F10.3)
 2130 format (A10)
