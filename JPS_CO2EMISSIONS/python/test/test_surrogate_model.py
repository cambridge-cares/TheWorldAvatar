...
def cost_emission_calculation(m, n):
    if m.shape[0] == 0:
        ...
        x1 = n.loc[:, ('age')].values.reshape((int(n.shape[0] / n.capacity_MW.nunique()), n.capacity_MW.nunique()))[:,
             0]
        y1 = n.loc[:, ('capacity_MW')].values.reshape(
            (int(n.shape[0] / n.capacity_MW.nunique()), n.capacity_MW.nunique()))[0, :]
...
def run_surrogate_model(powerplantInfo, workingDir):

    plantInfoDict = powerplantInfo
#     workingDir = 'C:/Users/WE/Desktop/JPS/JParkSimulator-git/JPS_CO2EMISSIONS/workingdir/'
    ...
    if generation_technology == 'ultrasupercritical':
        emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_ultrasupercritical_PC_coal.csv',
                                  header='infer', sep=',')

    if generation_technology == 'supercritical':
        if primary_fuel == 'anthracite':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_supercritical_PC_anthracite.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'subbituminous':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_supercritical_PC_subbituminous.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'lignite':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_supercritical_PC_lignite.csv',
                                      header='infer', sep=',')
    ...
    if generation_technology == 'subcritical' or generation_technology == 'cogeneration':
        if primary_fuel == 'anthracite':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_subcritical_PC_anthracite.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'subbituminous':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_subcritical_PC_subbituminous.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'lignite':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_subcritical_PC_lignite.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'coal_biomass':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_subcritical_PC_coal_biomass.csv',
                                      header='infer', sep=',')
        elif primary_fuel == 'bituminous' or primary_fuel == 'coal':
            emission_df = pd.read_csv(workingDir + 'data/input/baseplant/base_subcritical_PC_subbituminous.csv',
                                      header='infer', sep=',')
    ...
    return result_dict['annual_emission_ton']