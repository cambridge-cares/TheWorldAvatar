...
i generation source /n,c1,c2,co/
*The constant parameters
j parameter /delta,tau,D,L/
*t represents the time horizion (1 hour interval for a period of 1 year)
t time /t1*t8760/
ls The column labels for gemerator para/Yr,Cap,Fix,OM,Fuel,Carb,Ri,Ci,a,b,c/
lp The column labels for time profiles /Dt,Pt,TCAP/;
...
objective.. z=e=sum(t,tp(t,'Pt')*tp(t,'Dt')-sum(i,(gp(i,'OM')+gp(i,'Fuel')+ fc(t,i)+ cp('tau')*gp(i,'Carb')*Q(t,i))-(tp(t,'Pt')-cp('delta'))*M(t)+((tp(t,'Pt')-cp('delta'))*X(t)))-sum(i,(a(i)-d(i))*dC(i)));
flct(t,i)..      fc(t,i)=e=    gp(i,'a') +gp(i,'b')*Q(t,i)+gp(i,'c')*Q(t,i)*Q(t,i);
