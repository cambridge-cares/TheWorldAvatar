...
def HNG(file_address, driver):
	...
	try:
		...
		ACSV(file_address, [page.headers['Date'], 'Prior Settlement (USD per mmBTU)', 10.0]+price, ['Date', 'Price type', 'Size (mmBTU)']+delivery)
		time.sleep(0.5)

	else:
		merged = [[],[]]
		merged[0],merged[1] = adjust_columns(prev_headers, prev_data, delivery,[page.headers['Date'], 'Prior Settlement (USD per mmBTU)', 10.0]+['-']*(len(remove_duplicates(prev_headers+delivery))-len(['Date', 'Price type', 'Size (mmBTU)']+delivery))+price)
		page.close()
...
def run(file_address):		
	...
	#file_address = r'C:\Users\Janusz\Desktop\Commodity_prices\Market_data\HNG_data.csv'
	HNG(file_address, driver)
	driver.quit()
	print('Success')
