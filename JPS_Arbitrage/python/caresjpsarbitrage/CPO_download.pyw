...
def CPO(file_address, driver):
	url = 'http://www.cmegroup.com/trading/agricultural/grain-and-oilseed/usd-malaysian-crude-palm-oil-calendar.html?optionProductId=8075'
	...
	#print(delivery)
	#delivery = delivery[:int(0.5*len(delivery))]
	...
	try:
		...
		ACSV(file_address, [page.headers['Date'], 'Prior Settlement (USD per tonne)', 25.0]+price, ['Date', 'Price type', 'Size (tonne)']+delivery)
		time.sleep(0.5)

	else:
		merged = [[],[]]
		merged[0],merged[1] = adjust_columns(prev_headers, prev_data, delivery,[page.headers['Date'], 'Prior Settlement (USD per tonne)', 25.0]+['-']*(len(remove_duplicates(prev_headers+delivery))-len(['Date', 'Price type', 'Size (tonne)']+delivery))+price)
		page.close()
...
def run(file_address):		
	...
	#file_address = r'C:\Users\Janusz\Desktop\Commodity_prices\Market_data\CPO_data.csv'
	CPO(file_address, driver)
	driver.quit()
	print('Success')
