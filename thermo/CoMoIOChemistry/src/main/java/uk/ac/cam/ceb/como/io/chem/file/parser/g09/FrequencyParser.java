...
//        TODO
    ...
    protected double[] extractEnergies(JobSection jobsec) {
        double thermal = Double.parseDouble(ParserHelper.parseLine(Pattern.compile("\\s*Thermal correction to Enthalpy.*?=\\s*(" + DoubleTool.NUMBER_REGEX + ").*"),
                jobsec.get(jobsec.getLastMatchPosition(0, "\\s*Thermal correction to Enthalpy.*?=\\s*(" + DoubleTool.NUMBER_REGEX + ").*"))));
        double zpe = Double.parseDouble(ParserHelper.parseLine(Pattern.compile("\\s*Zero.*?-point correction.*?=\\s*(" + DoubleTool.NUMBER_REGEX + ")\\s*\\(Hartree/Particle\\).*"),
                jobsec.get(jobsec.getLastMatchPosition(0, "\\s*Zero.*?-point correction.*?=\\s*(" + DoubleTool.NUMBER_REGEX + ")\\s*\\(Hartree/Particle\\).*"))));
    ...
    protected CMLModule extractFinalModule(CMLModule jobModule, Archive archive, MolecularInfoImpl molecularInfo, ComputedInfoImpl computedInfo, List<CMLMolecule> molecules) {
        ...
        if (zpeAndThermal != null && zpeAndThermal.length == 2) {
            property_list.addProperty(chelper.createCMLPropertyScalar(zpeAndThermal[0], "nonSi:hartree", "cc:zpe", CompChem.COMPCHEM_NS));
            property_list.addProperty(chelper.createCMLPropertyScalar(zpeAndThermal[1], "nonSi:hartree", "cc:thermalenergy", CompChem.COMPCHEM_NS));
        }
...
//        normalModes.setUnits("nonSi:cm^-1", "nonSi:amu", "nonSi:A^4/amu", "nonSi:mDyne/A^-1", "nonSi:Angstroms");
        while ((pos = sl.getFirstMatchPosition(from_line, "\\s*Frequencies\\s+--.*")) > -1) {
            ...
            for (String line : infos) {
                if (line.contains("Frequencies")) {
                    ...
                    normalModes.addAll(Vibrations.Property.REDUCED_MASS, normalModes.size() - local_rms.size(), local_rms);
                    ...
                    normalModes.addAll(Vibrations.Property.FORCE_CONSTANT, normalModes.size() - local_fcs.size(), local_fcs);
                ...
                if (local_disp.size() >= 5) {
                    ...
                    if ((local_disp.size() - 2) % 3 == 0) {
                        for (int i = 0; i < size; i++) {
                            ...
                            xyz.setAtomReference(Integer.toString(dispArrayList.get(indexCounter - size + i).size() + 1));
    ...
    protected List<CMLProperty> createCMLProperties(Vibrations normalModes) {
        ...
        CMLArray f_array = chelper.createCMLArray(normalModes.get(Vibrations.Property.FREQUENCY), "nonSi:cm^-1", CompChem.NONSI_NS);
        ...
        CMLArray redmass_array = chelper.createCMLArray(normalModes.get(Vibrations.Property.REDUCED_MASS), "nonSi:amu", CompChem.NONSI_NS);
        ...
        CMLArray raman_array = chelper.createCMLArray(normalModes.get(Vibrations.Property.IR_INTEN), "nonSi:A^4/amu", CompChem.NONSI_NS);
        ...
        CMLArray fc_array = chelper.createCMLArray(normalModes.get(Vibrations.Property.FORCE_CONSTANT), "nonSi:mDyne/A^-1", CompChem.NONSI_NS);
