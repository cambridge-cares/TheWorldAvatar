...
public class GaussianHRParser extends FrequencyParser {
    ...
    protected CMLModule extractFinalModule(CMLModule jobModule, Archive archive, MolecularInfoImpl molecularInfo, ComputedInfoImpl computedInfo, List<CMLMolecule> molecules) {
        ...
        try {
            ...
            for (int i = 0; i < torsionalBonds.size(); i++) {
                ...
                property_list_hr.appendChild(chelper.createCMLPropertyScalar((int) torsionalBonds.get(i).mode.mode, "si:none", "cc:vibrational_mode", CompChem.COMPCHEM_NS));
    ...
    private VibrationalNormalModes identifyRotationalModes(JobSection jobsec) {
        VibrationalNormalModes rotModes = new VibrationalNormalModes();
    ...
    private List<TorsionalBond> identifyTorsionalBonds(JobSection jobsec, VibrationalNormalModes rotModes) throws Exception {
