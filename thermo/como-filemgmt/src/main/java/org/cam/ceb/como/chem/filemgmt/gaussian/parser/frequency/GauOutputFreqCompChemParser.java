 ...
 */
package org.cam.ceb.como.chem.filemgmt.gaussian.parser.frequency;
...
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.extractArchive;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.extractJobSections;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.parseArchive;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.parseDoubles;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.parseInlineChargeAndSpinMultiplicity;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.parseInlineRotationalSymmetry;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.parseInlineSCFDone;
import static gigadot.chom.compchem.parser.gaussian.util.GaussianHelper.tokenize;
 ...
 * @author pb556
 */
public class GauOutputFreqCompChemParser extends ChemFileParser {
    ...
    public void parse() throws Exception {
        ...
        List<JobSection> jobsecs = jobsecs = extractJobSections(new File(this.path));
        for (Iterator<JobSection> jit = jobsecs.iterator(); jit.hasNext();) {
            ...
            for (int i = 0; i < jobsec.size(); i++) {
                ...
                if (line.contains("Input orientation:") || line.contains("Standard orientation:") || line.contains("Z-Matrix orientation:")) {
                    ...
                    // find and check position to extract
                    int freqPos = jobsec.getFirstMatchPosition(i, " Frequencies\\s+--.*");
                    int endPos = jobsec.getFirstMatchPosition(i, " -----*.*", " -\\sThermochemistry\\s-.*");
    ...
    protected void parseGeometry() {
        
    }
    
    protected void parseFrequency() {
        
    }
    
    protected void parseHinderedRotor() {
     ...
     */
    protected CMLMolecule parseCoordinatesAndCalculateBonds(final List<String> coords) {
        if (coords.size() > 0) {
    ...
    protected List<CMLProperty> parseVibrations(final List<String> rawFreqs) throws IOException {
