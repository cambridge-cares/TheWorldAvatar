 ...
 * @author pb556
 */
public class CanonicalSMILESBuilder extends OBOutputBuilder {
    
    public CanonicalSMILESBuilder() {
        super();
    }
    ...
    public String build(CMLMolecule mol) {
        ...
        try {
            ...
            try {
                OpenBabelConverter.convert(CommandLine.parse("babel -icml " + input.getAbsolutePath() + " -omol " + output.getAbsolutePath()), input, output, false, false);
                input = new File(tempDir, "input.mol");
                output = new File(tempDir, "output.can");
                String retVal = OpenBabelConverter.convert(CommandLine.parse("babel -imol " + input.getAbsolutePath() + " -ocan " + output.getAbsolutePath() + " -I --unique"), input, output, false, false);
