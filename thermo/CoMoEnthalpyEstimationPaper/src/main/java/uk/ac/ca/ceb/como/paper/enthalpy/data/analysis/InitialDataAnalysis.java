...
//	                        ctr++;
						...
						PoolModificationCalculator poolModCalc = new PoolModificationCalculator(ctrRes[k], solver,
								new MPSFormat(false, new ISGReactionType(true)));
...
//	                              double error = Math.abs(completeRList.get(ri).getSpecies().getHf()-completeRList.get(ri).calculateHf());
								  ...
								  System.out.println("target species name: " + target.getRef() + " ,  target ref enthalpy: " + targetSpeciesEnthalpy); //+ target.getHf()
								...
								/**
								 * Checks whether error bar for current species is smaller that error bar for that species calculated in pre-processing step.  
								 */
