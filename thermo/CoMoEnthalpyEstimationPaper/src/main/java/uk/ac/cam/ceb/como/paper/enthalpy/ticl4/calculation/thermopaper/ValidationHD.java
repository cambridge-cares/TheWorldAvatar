 ...
 * @author pb556
 */
public class ValidationHD {
    
    public static void main(String[] args) throws Exception {

//      String srcCompoundsRef = "W:\\projects\\enthalpy-methodology\\g09_ref_pool\\";
//      String srcRefPool = "W:\\projects\\enthalpy-methodology\\2016-05-17\\results\\calc-enthalpy_scaled_kJperMol.csv";
//      String srcSoiPool = "W:\\projects\\enthalpy-methodology\\2016-05-17\\results\\soi-calc-enthalpy_scaled_kJperMol.csv";
//      String destRList = "W:\\projects\\enthalpy-methodology\\results\\validation\\hco_hd\\";
        ...
        soiParser.parse();
        List<Species> soiSpecies = new ArrayList<>(soiParser.getRefSpecies());
        ...
        Map<String, Integer> spinMultiplicity = new HashMap<>();
        int ctr = 1;
        ...
        for (Species s : refSpecies) {
            ...
            if (f.exists()) {
                ...
                if (e != null) {
                    spinMultiplicity.put(s.getRef().trim(), e[1] + 1);
                } else {
        ...
        for (int z = 0; z < ctrRadicals.length; z++) {
            ...
            for (int i = 0; i < ctrRuns.length; i++) {
                for (int k = 0; k < ctrRes.length; k++) {
                    String config = "hd_runs" + ctrRuns[i] + "_res" + ctrRes[k] + "_radicals" + maxRadical + "_" + timeout + "s";
                    ...
                    for (Species target : soiSpecies) {
                        ...
                        for (Species ref : refSpecies) {
                            if (ref.getRef().trim().compareTo(target.getRef().trim()) == 0) {
                                refPool.remove(ref);
                                break;
                            }
                        }
                        // filter for radicals
                        for (String sSpin : spinMultiplicity.keySet()) {
                            try {
                                if (spinMultiplicity.get(sSpin) != null && spinMultiplicity.get(sSpin) - 1 > maxRadical) {
                                    for (Species ref : refPool) {
                                        if (ref.getRef().trim().compareTo(sSpin.trim()) == 0) {
                                            refPool.remove(ref);
                                            break;
                                        }
                                    }
