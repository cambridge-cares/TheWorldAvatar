 ...
 * @author pb556
 */
public class TDPPIASIredPartitionFunction extends PartitionFunction {
    ...
    public TDPPIASIredPartitionFunction() {
        super();
    }

    public TDPPIASIredPartitionFunction(FourierSeries approximation, double frequency) {
        super();
    ...
    private double calculateC(double reducedMoment, double k) {
    ...
    private class ITorsionBarrier1 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                double c = calculateC(redMoment, k);
                ...
                double fx = fx_A + fx_B;
                return Math.sqrt(redMoment) * Math.exp(-1 * (fx) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
    ...
    private class ITorsionBarrier2 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                double fx_B = (Double) ((DataPoint) approximation.f(new Double[]{x - c / 2})).getValue();
                double fx = fx_A + fx_B;
                return (fx) * Math.sqrt(redMoment) * Math.exp(-(fx) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
                Logger.getLogger(C1DHRPartitionFunction.class.getName()).log(Level.SEVERE, null, ex);
            }
            return 0.0;
    ...
    private class ITorsionBarrier3 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                double fx_B = (Double) ((DataPoint) approximation.f(new Double[]{x - c / 2})).getValue();
                double fx = fx_A + fx_B;
                return (fx) * (fx) * Math.sqrt(redMoment) * Math.exp(-(fx) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
                Logger.getLogger(C1DHRPartitionFunction.class.getName()).log(Level.SEVERE, null, ex);
            }
            return 0.0;
    ...
    private static double getBarrierValue(FourierSeries s) throws FunctionCalculationException {
        double max = 0.0;
