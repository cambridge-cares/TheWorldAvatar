 ...
 */
package uk.ac.cam.ceb.como.thermo.calculator.rotation.internal.hindered.tdppi.hs;
...
import uk.ac.cam.ceb.como.thermo.partition_function.rotation.internal.hindered.tdppi.hs.TDPPIHSNumDiffPartitionFunction;

/**
 *
 * @author pb556
 */
public class TDPPIHSNumDiffThermoCalculator extends IRStatMechThermoCalculator {

    protected Logger logger = Logger.getLogger(getClass());

    public TDPPIHSNumDiffThermoCalculator() {
        super();
    }
                    ...
                    if (getInternalRotorRule() == null || !hasInternalRotation()) {
                        ...
                        switch (getInternalRotorRule().getInternalRotorType()) {
                            ...
                            case CLASSICAL_LIMIT_HINDERED_ROTOR:
                                ...
                                TDPPIHSNumDiffPartitionFunction q_hind = new TDPPIHSNumDiffPartitionFunction((FourierSeries) getHindrancePotential(rBond), rot.getFrequency());
                                q_hind.setThermoAnalyzable(getThermoAnalyzable());
                                q_hind.setReducedMoment(getInternalRotorRule().getReducedMOI(rBond, getThermoAnalyzable().getVibrations().clonedRotorBonds()));
