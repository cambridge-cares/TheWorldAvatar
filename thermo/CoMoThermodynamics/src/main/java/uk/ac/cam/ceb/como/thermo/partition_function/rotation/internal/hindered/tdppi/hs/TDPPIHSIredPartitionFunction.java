 ...
 * @author pb556
 */
public class TDPPIHSIredPartitionFunction extends PartitionFunction {
    ...
    public TDPPIHSIredPartitionFunction() {
        super();
    }

    public TDPPIHSIredPartitionFunction(FourierSeries approximation, double frequency) {
        super();
    ...
    public PartitionValues getPartitionValues(double T) {
        ...
        } catch (FunctionCalculationException fce) {
            return null;
        } catch (Exception ex) {
    ...
    private class ITorsionBarrier1 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                return Math.sqrt(redMoment) * Math.exp(-1 * (fx_A + fx_B) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
    ...
    private class ITorsionBarrier2 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                double fx_A = (Double) ((DataPoint) approximation.f(new Double[]{x + c / 2})).getValue();
                double fx_B = (Double) ((DataPoint) approximation.f(new Double[]{x - c / 2})).getValue();
                return (fx_A + fx_B) * Math.sqrt(redMoment) * Math.exp(-(fx_A + fx_B) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
                Logger.getLogger(C1DHRPartitionFunction.class.getName()).log(Level.SEVERE, null, ex);
            }
            return 0.0;
    ...
    private class ITorsionBarrier3 extends Function<Double, Double> {
        ...
        public Double f(Double x, Object... additionalData) {
            try {
                ...
                double fx_A = (Double) ((DataPoint) approximation.f(new Double[]{x + c / 2})).getValue();
                double fx_B = (Double) ((DataPoint) approximation.f(new Double[]{x - c / 2})).getValue();
                return (fx_A + fx_B) * (fx_A + fx_B) * Math.sqrt(redMoment) * Math.exp(-(fx_A + fx_B) / (2 * PhysicalConstants.k_B * T));
            } catch (Exception ex) {
                Logger.getLogger(C1DHRPartitionFunction.class.getName()).log(Level.SEVERE, null, ex);
            }
            return 0.0;
