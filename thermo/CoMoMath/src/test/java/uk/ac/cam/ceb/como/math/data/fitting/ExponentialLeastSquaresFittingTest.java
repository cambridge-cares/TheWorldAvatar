...
import uk.ac.cam.ceb.como.math.data.fitting.ExponentialLeastSquaresFitting;
 ...
 * @author pb556
 */
public class ExponentialLeastSquaresFittingTest {
    ...
    public boolean validate(double a, double b, int ctr, double delta, double e) throws FittingException, FunctionCalculationException {
        ...
        for (int i = 0; i < ctr; i++) {
            ...
            yData.add(new Double[]{a * Math.exp(b * i * delta)});
        }

        ExponentialLeastSquaresFitting fitting = new ExponentialLeastSquaresFitting(xData, yData);
        boolean valid = true;
        for (int i = 0; i < ctr; i++) {
            xData.add(new Double[]{i * delta});
            yData.add(new Double[]{a * Math.exp(b * i * delta)});
            double vA = a * Math.exp(b * i * delta);
            double vB = fitting.f(new Double[]{i * delta})[0];
            double diff = Math.abs(a * Math.exp(b * i * delta) - fitting.f(new Double[]{i * delta})[0]);
