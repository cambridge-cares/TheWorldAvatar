...
import uk.ac.cam.ceb.como.math.data.fitting.LogarithmicLeastSquaresFitting;
 ...
 * @author pb556
 */
public class LogarithmicLeastSquaresFittingTest {
    @Test
    ...
    public boolean validate(double a, double b, int ctr, double delta, double e) throws FittingException, FunctionCalculationException {
        ...
        for (int i = 1; i < ctr; i++) {
            ...
            yData.add(new Double[]{a + b * Math.log(i * delta)});
        }

        LogarithmicLeastSquaresFitting fitting = new LogarithmicLeastSquaresFitting(xData, yData);
        boolean valid = true;
        for (int i = 1; i < ctr; i++) {
            xData.add(new Double[]{i * delta});
            yData.add(new Double[]{a + b * Math.log(i * delta)});
            double vA = a + b * Math.log(i * delta);
            double vB = fitting.f(new Double[]{i * delta})[0];
            double diff = Math.abs(a + b * Math.log(i * delta) - fitting.f(new Double[]{i * delta})[0]);
