 ...
 * @author pb556
 */
public class PowerLawLeastSquaresFitting extends Function<Double[], Double[]>{

    // http://mathworld.wolfram.com/LeastSquaresFitting.html
    private double[] a = null;
    private double[] b = null;
    
    public PowerLawLeastSquaresFitting(List<Double[]> xData, List<Double[]> yData) throws FittingException {
        fit(xData, yData);
    }
    ...
    protected final void fit(List<Double[]> xData, List<Double[]> yData) throws FittingException {
        ...
        for (int j = 0; j < dim; j++) {
            ...
            for (int i = 0; i < xData.size(); i++) {
                termNominator_b1 += Math.log(yData.get(i)[j]) * Math.log(xData.get(i)[j]);
                
                termNominator_b2_1 += Math.log(xData.get(i)[j]);
                termNominator_b2_2 += Math.log(yData.get(i)[j]);

                termDenominator_1 += Math.log(xData.get(i)[j]) * Math.log(xData.get(i)[j]);
                termDenominator_2 += Math.log(xData.get(i)[j]);
                
                termNominator_a1 += Math.log(yData.get(i)[j]);
            ...
            a[j] = Math.exp((termNominator_a1 - b[j] * termNominator_a2) / ((double) xData.size()));
        }
    }
