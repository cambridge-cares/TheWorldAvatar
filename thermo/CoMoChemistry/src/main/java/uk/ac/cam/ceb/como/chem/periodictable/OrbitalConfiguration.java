...
import static uk.ac.cam.ceb.como.chem.periodictable.Orbital.d;
import static uk.ac.cam.ceb.como.chem.periodictable.Orbital.f;
import static uk.ac.cam.ceb.como.chem.periodictable.Orbital.p;
import static uk.ac.cam.ceb.como.chem.periodictable.Orbital.s;
...
public class OrbitalConfiguration {
    ...
    public OrbitalConfiguration(int order, Orbital orbital, int numElectrons) {
        if (!verifyOrderAndOrbital(order, orbital) || ! verifyOrbitalAndNumberOfElectrons(orbital, numElectrons)) {
             logger.error("Invalid data were identified.", new Exception("Invalid data were identified."));
             return;
        }
        this.order = order;
