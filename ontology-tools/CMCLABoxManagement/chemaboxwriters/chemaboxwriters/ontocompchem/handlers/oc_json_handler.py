...
class OC_JSON_TO_OC_CSV_Handler(Handler):
    ...
    def _write_initial(self, writer: Abox_Writer, jobIRI, calc_id, spec_IRI):
        ...
        ).add_obj_prop(iri=jobIRI, rel=f"{onto_comp}#hasInitialization")
    ...
    def _write_mols(self, writer: Abox_Writer, calc_id, data):
        ...
        for k in range(
            ...
            writer.write_inst(
                iri=f"{table_pref}#{atom}", type=f"{table_pref}#Element"
            ).add_obj_prop(
                iri=f"{comp_pref}Atom_{calc_id}_{atom}{count}",
    ...
    def _write_level_of_theory(self, writer: Abox_Writer, calc_id, data):
        ...
        writer.write_inst(
            iri=f"{meth_iri}", type=f"{gain_pref}MethodologyFeature"
        ).add_obj_prop(
            iri=f"{init_mod_iri}",
    ...
    def _write_rotations(self, writer: Abox_Writer, jobIRI, calc_id, data):
        ...
        if "Rotational constants" in data:
            ...
            writer.write_inst(
                ...
                iri=f"{unit_pref}unit#GigaHertz",
                rel=f"{gain_pref}hasUnit",
                reverse=True,
            )
    ...
    def _write_geom_type(self, writer: Abox_Writer, jobIRI, calc_id, data):
        ...
        ).add_obj_prop(iri=jobIRI, rel=f"{gain_pref}isCalculationOn",).add_data_prop(
    ...
    def _write_zpe(self, writer: Abox_Writer, jobIRI, calc_id, data):
        ...
        if "Electronic and ZPE energy" in data and "Electronic energy" in data:
            ...
            writer.write_inst(
                ...
                iri=f"{unit_pref}unit#Hartree", rel=f"{gain_pref}hasUnit", reverse=True
            )
    ...
    def _write_geom_opt(self, writer: Abox_Writer, jobIRI, calc_id, data):
        ...
        writer.write_inst(
            ...
            iri=f"{gain_pref}atomicUnit", rel=f"{gain_pref}hasUnit", reverse=True
        )
    ...
    def _write_atom_info(self, writer: Abox_Writer, calc_id, data):
        ...
        for k in range(len(data["Atom types"])):
            ...
            writer.write_inst(
                iri=f"{comp_pref}Atom_{atom_id}", type=f"{gain_pref}Atom"
            ).add_obj_prop(
                iri=f"{comp_pref}Molecule_{calc_id}",
                rel=f"{gain_pref}hasAtom",
            ).add_obj_prop(
                iri=f"{table_pref}#{atom}", rel=f"{gain_pref}isElement", reverse=True
            )
            for i in range(3):
                writer.write_inst(
                    iri=(
            ...
            writer.write_inst(
                ...
                iri=f"{unit_pref}unit#Dalton", rel=f"{gain_pref}hasUnit", reverse=True
            )
            count += 1
