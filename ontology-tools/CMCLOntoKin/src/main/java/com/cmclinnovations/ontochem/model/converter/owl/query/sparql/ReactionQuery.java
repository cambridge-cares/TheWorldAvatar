...
public class ReactionQuery extends OwlConverter implements IReactionQuery {
	...
	private String readOrderSpecies(String reactionOrderInstance){
		...
		String q = formQueryWithBaseURL(reactionOrderInstance, appConfigOntokin.getObjectPropertyAppliesTo());
		// Performs the query q and returns the result
		String orderSpeciesInstance = performQuery(q, 1);
		if(orderSpeciesInstance!=null && !orderSpeciesInstance.isEmpty()){
			return readName(orderSpeciesInstance);
		}
		return null;
	}
	...
	private void queryComment(String instance, Reaction reaction){
		...
		if(comment==null){
			...
			if(comment.contains("\\\\")){
				reaction.setComment(OwlConverterUtils.cTMLifyComment(comment.replace("\\\\", "\\")));
			} else if (comment.contains("\\\"")){
				reaction.setComment(OwlConverterUtils.cTMLifyComment(comment.replace("\\\"", "\"")));
			} else{
				reaction.setComment(OwlConverterUtils.cTMLifyComment(comment));
			}
		}
	}
	
	/**
	 * Queries the equation attached to the instance of a reaction in the
	 * mechanism OWL file being processed.
	 */
	...
	private String readReactants(String reactionInstance) {
		...
		if (queryResult != null && !queryResult.isEmpty()) {
			...
			for (String reactantSpecInstance : reactantSpecInstances) {
				q = formQueryWithBaseURL(reactantSpecInstance,
						appConfigOntokin.getObjectPropertyIndicatesMultiplicityOf());
				String reactantInstance = performQuery(q, 1);
				if (reactantInstance != null) {
					if(reactantInstance.startsWith(appConfigOntokin.getClassReactant())){
					String reactantInstanceLabel = readLabel(reactantInstance);
					String stoichioMetricCoeff = readStoichiometricCoeff(reactantSpecInstance);
					if (reactantInstanceLabel != null && !reactantInstanceLabel.isEmpty() && stoichioMetricCoeff != null
							&& !stoichioMetricCoeff.isEmpty()) {
						reactantVsStoichiometricCoeff = reactantVsStoichiometricCoeff.concat(reactantInstanceLabel).concat(COLON).concat(stoichioMetricCoeff).concat(SPACE);
					}
				}
				}
			}
	...
	private String readProducts(String reactionInstance) {		
		...
		if (queryResult != null && !queryResult.isEmpty()) {
			...
			for (String productSpecInstance : productSpecInstances) {
				q = formQueryWithBaseURL(productSpecInstance,
						appConfigOntokin.getObjectPropertyIndicatesMultiplicityOf());
				String productInstance = performQuery(q, 1);
				if (productInstance != null) {
				if(productInstance.startsWith(appConfigOntokin.getClassProduct())){
					String productInstanceLabel = readLabel(productInstance);
					String stoichioMetricCoeff = readStoichiometricCoeff(productSpecInstance);
					if (productInstanceLabel != null && !productInstanceLabel.isEmpty() && stoichioMetricCoeff != null
							&& !stoichioMetricCoeff.isEmpty()) {
						productVsStoichiometricCoeff = productVsStoichiometricCoeff.concat(productInstanceLabel).concat(COLON).concat(stoichioMetricCoeff).concat(SPACE);
					}
				}
				}
			}
...
//		String speciesInstance = "";
//		readReactantSpecificationSpecies(reactantSpecInstance);
//		if(queryResult!=null && !queryResult.isEmpty()){
//			Collections.sort(queryResult);
//			for(String result:queryResult){
//				if(result!=null && result.startsWith(appConfigOntokin.getClassSpecies())){
//					speciesInstance = result;
//					break;
//				}
//			}
//			queryResult = new ArrayList<String>();
...
//		String speciesVsStCoeff = "";
//		String speciesInstance = "";
//		readProductSpecificationSpecies(prodcutSpecInstance);
//		if(queryResult!=null && !queryResult.isEmpty()){
//			Collections.sort(queryResult);
//			for(String result:queryResult){
//				if(result!=null && result.startsWith(appConfigOntokin.getClassSpecies())){
...
//	}
	 ...
	 * @return
	 */
//	private void readReactantSpecificationSpecies(String reactantSpecInstance){
//		String q = formQueryWithBaseURLAndSameAsRelation(reactantSpecInstance,
//				appConfigOntokin.getObjectPropertyIndicatesMultiplicityOf(), appConfigOntokin.getClassSpecies());
//		performMultilineAnswerQuery(q, 1);
//	}
	 ...
	 * @return
	 */
//	private void readProductSpecificationSpecies(String prodcutSpecInstance){
//		String q = formQueryWithBaseURLAndSameAsRelation(prodcutSpecInstance,
//				appConfigOntokin.getObjectPropertyIndicatesMultiplicityOf(), appConfigOntokin.getClassSpecies());
//		performMultilineAnswerQuery(q, 1);
//	}
	
	/**
	 * Reads the reactant/product Stoichiometric coefficient of a reaction. 
	 * 
	 * @param stoichioMetricCoeffInstance
	 * @return
	 */
	private String readStoichiometricCoeff(String stoichioMetricCoeffInstance){
		String q = formQueryWithBaseURL(stoichioMetricCoeffInstance, appConfigOntokin.getDataPropertyStoichiometricCoefficient());
		return performQuery(q, 1);
	}
