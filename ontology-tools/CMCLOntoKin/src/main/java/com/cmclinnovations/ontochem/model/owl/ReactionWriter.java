...
public class ReactionWriter extends CtmlConverter implements IReactionWriter{
	...
	private void writeSiteFractionInfo() {
		try {
			...
			logger.error("The partial pressure infomration of a reaction could not be created.");
		}
	}
	...
	private void writeOrderDirectionPropertyValue() {
		try {
			...
			if(reactionOrder.getDirection()!=null && !reactionOrder.getDirection().isEmpty()){
				if(reactionOrder.getDirection().equalsIgnoreCase(appConfigCtml.getForwardReactionIndicator())){
					forwardReactionOrderSpecies.add(reactionOrder.getSpecies());
				}else if(reactionOrder.getDirection().equalsIgnoreCase(appConfigCtml.getReverseReactionIndicator())){
					reverseReactionOrderSpecies.add(reactionOrder.getSpecies());
				}
			}
		} catch (OntoException e) {
	...
	private void writeSpeciesOrderPropertyValue() {
		if(reactionOrder.getSpecies()!=null && !reactionOrder.getSpecies().isEmpty()){
			speciesLabelVsReactionOrderIdMap.put(reactionOrder.getSpecies(), reactionOrderInstanceId);
		}
	}
