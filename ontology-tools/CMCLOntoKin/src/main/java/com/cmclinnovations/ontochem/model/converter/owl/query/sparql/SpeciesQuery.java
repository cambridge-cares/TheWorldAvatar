...
public class SpeciesQuery extends OwlConverter implements ISpeciesQuery{
	...
	private void querySpecies(String phaseOwlId){
		String q = formSubjectRetrievalQuery(phaseOwlId, appConfigOntokin.getOntokinBelongsToPhase());
		...
		if(queryResult!=null){
			List<String> results = queryResult;
			queryResult = new ArrayList<String>();
			for(String result:results){
				if(result.startsWith(appConfigOntokin.getClassSpecies())){
	...
	private void queryComment(String instance, Species species){
		...
		if(comment==null){
			...
			comment = OwlConverterUtils.cTMLifyComment(comment);
	...
	private void queryThermoComment(String instance, SpeciesThermo thmo){
		...
		if(comment==null){
			...
			if(comment.contains("\\\\")){
				thmo.setComment(OwlConverterUtils.cTMLifyComment(comment.replace("\\\\", "\\")));
			} else if(comment.contains("\\\"")){
				thmo.setComment(OwlConverterUtils.cTMLifyComment(comment.replace("\\\"", "\"")));
			}
			else{
				thmo.setComment(OwlConverterUtils.cTMLifyComment(comment));
			}
		}
	}
	...
	private String getAtom(String elementSpecificationInstance){
		// Calls the method that forms a SPARQL query
		String q = formQueryWithBaseURL(elementSpecificationInstance, appConfigOntokin.getIndicatesNumberOf());
		// Performs the query q and returns the result
