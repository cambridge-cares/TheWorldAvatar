...
class CreateOntoCompChemEvaluationSet:
    ...
    def __init__(self):
        self.ontology = "ontocompchem"
        self.property_dict = \
            {"latent_relation_4":
                 ["rotational symmetry", " radial symmetry in geometry"],
             "latent_relation_3": ["rotational constants"],
             "latent_relation_2": ["vibration frequency", "vibrational frequency"],
             "latent_relation_9": ["geometry", "geometry type", "type of geometry"]}
        ...
        self.triples = pd.read_csv(os.path.join(self.dataset_dir, f"{self.ontology}-train.txt"), header=None, index_col=None,
                                   sep="\t")
...
if __name__ == "__main__":
    my_creator = CreateOntoCompChemEvaluationSet()
    my_creator.run()