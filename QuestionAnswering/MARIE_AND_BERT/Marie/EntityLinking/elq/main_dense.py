...
def _load_candidates(
        ...
        faiss_index="none", index_path=None,iddict_folder="",
        logger=None,
):
    if faiss_index == "none":
    ...
    id2title_path = os.path.join(iddict_folder, "id2title.json")
    id2text_path = os.path.join(iddict_folder, "id2text.json")
    id2wikidata_path = os.path.join(iddict_folder, "id2wikidata.json")

    if not os.path.exists(id2title_path):
        id2title = {}
        id2text = {}
        id2wikidata = {}
        local_idx = 0
        with open(entity_catalogue, "r") as fin:
            ...
            for line in lines:
                ...
                local_idx += 1
        json.dump(id2title, open(id2title_path, "w"))
        json.dump(id2text, open(id2text_path, "w"))
        json.dump(id2wikidata, open(id2wikidata_path, "w"))
    else:
        if logger: logger.info("Loading id2title")
        id2title = json.load(open(id2title_path))
        if logger: logger.info("Loading id2text")
        id2text = json.load(open(id2text_path))
        if logger: logger.info("Loading id2wikidata")
        id2wikidata = json.load(open(id2wikidata_path))
...
def load_models(args, logger):
    ...
    (
        ...
        args.faiss_index, args.index_path, args.iddict_path, logger=logger,
    )
