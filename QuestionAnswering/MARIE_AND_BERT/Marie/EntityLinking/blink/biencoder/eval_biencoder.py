...
class NEL():
    ...
    def __init__(self, params):
        ...
        self.logger = logger = utils.get_logger(params["output_path"])
        self.params = params
        # Init model
        self.reranker =reranker = BiEncoderRanker(params)
        self.tokenizer = reranker.tokenizer
        ...
        self.candidate_pool = load_or_generate_candidate_pool(
            self.tokenizer,
            params,
            logger,
            cand_pool_path,
        ...
        if cand_encode_path is not None:
            ...
            try:
                ...
                self.candidate_encoding = torch.load(cand_encode_path)
            except:
                logger.info("Loading failed. Generating candidate encoding.")


        if self.candidate_encoding is None:
            self.candidate_encoding = encode_candidate(
                reranker,
                self.candidate_pool,
    ...
    def infer(self,  test_samples):
        self.logger.info("Read %d test samples." % len(test_samples))

        test_data, test_tensor_data = data.process_mention_data(
            test_samples,
            self.tokenizer,
            self.params["max_context_length"],
            self.params["max_cand_length"],
            context_key=self.params['context_key'],
