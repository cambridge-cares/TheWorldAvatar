...
class OZFrameworkExampleMaker(ExampleMakerBase):
    ...
    def make_example(self, entity_iri: str):
        locate_strategy = np.random.choice(["concept", "name", "attr"], p=normalize_1d([0.05, 1, 12]))
        if locate_strategy == "concept":
            ...
            cond_num = np.random.choice([1, 2, 3, 4, 5], p=normalize_1d([3, 4, 5, 2, 1]))
            query_graph, verbn = self.locator.locate_concept_and_literal_multi(entity_iri, cond_num=cond_num)
        else:
            raise Exception("Unexpected `locate_strategy`: " + locate_strategy)
        
        ask_strategy = np.random.choice(["name", "attr"], p=normalize_1d([1, 10]))
