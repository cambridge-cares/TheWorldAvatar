...
def upload_inputChem(chemicals, synthesis_client, species_client):
    ...
    schema   = jschem.chemical_schema()
    full_prompt = f"{prompt}\n\n{file_content}"
    llm_out = chatgpt_api.make_llm_call_structured(full_prompt, JSON_LLM_TEMP)
    
    syn = ChemicalSynthesis.model_validate_json(llm_out)
    for chemical in syn.hasChemicalInput():
        check_iri_query = IRI_QUERY_TEMP.format(name=chemical.label)
        iri = client.perform_query(check_iri_query)
        chemical.instance_iri = iri
    syn.push_to_kg(client,5)
    ...
    for chemical in chemicals:
        ...
        phase_component_concentration                           = PhaseComponentConcentration(hasValue=add_value,HasUnit=add_unit)
        ...
        chemical_input.push_to_kg(synthesis_client,recursive_depth=4)
