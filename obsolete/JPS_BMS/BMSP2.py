	...
	if (value):
		...
		if(abbrPair):						# whether the case needs to get the abbriation from the dictionary
			...
			if(type == '1'):
				...
				if(value == '1'):
					...
					if(re.match('[0-9]',value.strip())):
						...
						if(value == '1'):
							...
							status = 'low'
						value = {'value':status , 'type': 'string', 'unit': {'isSI': isSI, 'unit': ''}}
					else:
				...
				elif('FCU' in EquipmentName or 'WFH' in tag):
					id = (re.search(idRegex,tag)).group(0)
					id = id.replace('0','').replace('_','-')[1:]
					filename = (filenameTemplate%id)
					targetIRI = (IRITemplate %id)
					...
					#print('tag ---', tag.split(' ')[1][:-1])
					value = {'value':tag.split(' ')[0] , 'type': 'string', 'unit': {'isSI': isSI, 'unit': ''}}
				...
				if('MAU' in EquipmentName):
					...
					filename = (filenameTemplate%id)
					if('%s' in IRITemplate):
						# if('V_CHWSTemperatureP1OfMAU' in IRITemplate):
							# 
						#MAU-%s_D12T_sensor1.owl,V_PreCoolTemperatureCHWOutP5OfMAU-C7-1
				...
				print('type is 4 with EquipmentName', EquipmentName )
				print('nameOfRow',nameOfRow)
				if('MAU' in EquipmentName):
...
with open('workingdir/backupmap.csv') as file:
	lines = file.readlines()
	for line in lines[1:]: 
		cols = line.split(',')
		if(len(cols) > 3):
	...
	# Read local xml file
			with open('workingdir/bmsResult.html') as file:
				...
				for line in lines:
					...
					if(re.match('<VAV_S[0-9]_[0-9]*.*',line)):
						currentVAV = re.search('<VAV_S[0-9]_[0-9]*.*>',line).group(0).strip()
