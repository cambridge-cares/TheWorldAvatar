...
from modelTopic.modelTopic import topicModel
import sys
import pickle
import traceback
import operator
...
class DomainMatcher(ElementMatcher):
    ...
    def compare(self, id1, id2):
        n1 = list(set(self.S.tokensDict[id1]))#
        n2 = list(set(self.T.tokensDict[id2]))
        ...
        def getTopic4Word(tokens, wordDict):
            ...
            for word in tokens:
                if word not in wordDict or len(wordDict[word]) is 0:
                    ...
                    #TODO: check this
                    #tw = wordDict[word][0][0]
                    tw = wordDict[word][0]#pick most likely topic only
                t.append(tw)
            return t,noneCount
