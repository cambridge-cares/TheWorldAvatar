...
# Wrapper script for docker-compose that deploys the requested stack in one of three
# modes (dev/test/prod).
#
...
if [ "$#" -lt 2 ]; then
  ...
  echo ""
  echo "                stack_name : the stack to deploy (agent/db/web)"
  ...
  echo ""
  echo "e.g. To deploy the web stack in dev mode:"
  echo "  $0 web dev"
  echo "============================================================================="
  exit 1
fi
...
exit_with_msg 0 "\n$stack stack deployed in $mode mode"