...
(SELECT parent,average_height, 50. AS elevation, ST_MakeValid(geometry) AS geometry FROM 
(SELECT parent,avg(height) AS average_height, ST_Union(wkb_geometry) AS geometry FROM 
(SELECT id,TO_NUMBER(height,'99G999D9S') AS height,case when rel IS NOT NULL then rel else id end AS parent, ST_FlipCoordinates(wkb_geometry) AS wkb_geometry FROM 
(SELECT split_part(id,'/',2) AS id, height, CAST(raw_gdal."@relations"->0->'rel' AS TEXT) as rel, wkb_geometry FROM "public"."raw_gdal" WHERE GeometryType(wkb_geometry) = 'POLYGON' or GeometryType(wkb_geometry) = 'MULTIPOLYGON') AS tmp
WHERE rel IS NOT NULL or height IS NOT NULL or id IN (SELECT CAST(raw_gdal."@relations"->0->'rel' AS TEXT) FROM raw_gdal)) AS tmp2
