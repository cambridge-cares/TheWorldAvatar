...
| [ng-pylons](../example_datasets/inputs/config/ng-pylons.json) | Uploads a [set of Shapefiles](../example_datasets/inputs/data/ng_pylons/vector/) into the stack as single vector layer along with several [.csv files](../example_datasets/inputs/data/ng_pylons/tabular/) that contain auxiliary data. Some of the auxiliary data is then used by a custom style ([ng-pylons.sld](../example_datasets/inputs/config/ng-pylons.sld)) to dynamically style the lines and towers when served through GeoServer. |
...
| [pylons](../example_datasets/inputs/config/pylons-and-veg.json) | An example of how to use the `"externalDatasets"` node to load multiple datasets by name of another config referencing other `"externalDatasets". |
...
| [ukpn-pylons](../example_datasets/inputs/config/ukpn-pylons.json) | Uploads a [set of Shapefiles](../example_datasets/inputs/data/ukpn_pylons/vector/) into the stack as single vector layer along with several [.csv files](../example_datasets/inputs/data/ukpn_pylons/tabular/) that contain auxiliary data. Some of the auxiliary data is then used by a custom style ([ukpn-pylons.sld](../example_datasets/inputs/config/ukpn-pylons.sld)) to dynamically style the lines and towers when served through GeoServer. There is also a OBDA mapping file ([ukpn_ontop.obda](../example_datasets/inputs/data/ukpn_pylons/ukpn_ontop.obda)), which provides an example of how to make the uploaded data queryable through the Ontop SPARQL endpoint. |
