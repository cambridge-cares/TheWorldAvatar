...
class DataStore {
	...
	loadDataGroups(a) {
		if (null != a) return new Promise((e, t) => {
			this.recurseLoadDataGroups(a, null, null, this.dataGroups.length), e("done")
		});
		{
			let t = this;
			return $.getJSON("./data.json", function(e) {
...
class Manager {
	...
	async readCredentials() {
		await $.get("mapbox_username", {}, function(e) {
			MapHandler.MAP_USER = e
		}).fail(function() {
			console.error("Could not read Mapbox username from 'mapbox_username' secret file.")
		}), await $.get("mapbox_api_key", {}, function(e) {
			MapHandler.MAP_API = e
		}).fail(function() {
	...
	initialiseMap() {
		...
		this.mapHandler.initialiseMap(e), this.controlHandler.showControls(), this.controlHandler.rebuildTree(Manager.DATA_STORE), this.panelHandler.toggleMode(), showAttributions(), buildDashboardButton();
		let t = this;
		document.addEventListener("keydown", function(e) {
			...
			} else(e.ctrlKey || e.metaKey) && e.altKey && "t" === e.key && (Manager.PROVIDER === MapProvider.CESIUM && (console.log("Camera Longitude: " + Cesium.Math.toDegrees(MapHandler.MAP.camera.positionCartographic.longitude)), console.log("Camera Latitude: " + Cesium.Math.toDegrees(MapHandler.MAP.camera.positionCartographic.latitude)), console.log("Camera Height: " + MapHandler.MAP.camera.positionCartographic.height), console.log("Camera Heading: " + Cesium.Math.toDegrees(MapHandler.MAP.camera.heading)), console.log("Camera Pitch: " + Cesium.Math.toDegrees(MapHandler.MAP.camera.pitch)), console.log("Camera Roll: " + Cesium.Math.toDegrees(MapHandler.MAP.camera.roll))), e.preventDefault())
		})
	}
	toggleFullscreen() {
		var e;
	...
	loadSettings() {
		return Manager.STACK_LAYERS = {}, Manager.SETTINGS = new Settings, Manager.SETTINGS.loadSettings("./settings.json").then(() => {
			var e = null != Manager.SETTINGS.getSetting("search"),
				t = document.getElementById("searchIconContainer");
			null != t && (t.style.display = e ? "block" : "none")
		})
	}
	loadDefinitions() {
		var e = this.loadSettings(),
			t = this.loadDefinitionsFromURL("./data.json");
		return Promise.all([e, t])
	}
	loadDefinitionsFromObject(e) {
		return Manager.DATA_STORE.reset(), Manager.DATA_STORE.loadDataGroups(e).then(() => {
			null != this.dataLoadCallback && this.dataLoadCallback()
		})
	}
	loadDefinitionsFromURL(e) {
		let t = this;
		return $.getJSON(e, function(e) {
			return e
		}).fail(e => {
			throw e
		}).then(e => t.loadDefinitionsFromObject(e))
	}
	loadImagesAndLinks() {
		var e = [],
	...
	plotData() {
		this.mapHandler.plotData(Manager.DATA_STORE), this.controlHandler.rebuildTree(Manager.DATA_STORE)
	}
	...
	showFeature(e, t) {
		...
		let a = getName(t),
			n = (null == a && (a = e.hasOwnProperty("id") && "object" != typeof e.id ? "Feature " + e.id : "Selected Feature"), this.panelHandler.setTitle("<h3>" + a + "</h2"), document.getElementById("titleContainer").classList.add("clickable"), t.description);
	...
	openSearch() {
		null != Manager.SETTINGS.getSetting("search") && (null !== this.searchHandler && void 0 !== this.searchHandler || Manager.PROVIDER === MapProvider.MAPBOX && (this.searchHandler = new SearchHandler_Mapbox), null != this.searchHandler) && this.searchHandler.toggle()
	}
	filterMatch(e, t) {
		...
		switch (Manager.PROVIDER) {
			case MapProvider.CESIUM:
				if (e instanceof Cesium.Cesium3DTileFeature) {
					...
					if (!a.hasOwnProperty("layerID")) return null;
					var d, c, u = a.layerID;
					for ([d, c] of Object.entries(Manager.STACK_LAYERS))
						if (c.includes(u)) return d
				} else {
					a = e.id;
					if (null != a) {
			...
			case MapProvider.MAPBOX:
				var g = e.layer.id;
				if (null != g)
					for (var [y, M] of Object.entries(Manager.STACK_LAYERS))
						if (M.includes(g)) return y
		}
		return null
	}
}
...
async function getVisVersion() {
	return fetch("./twa-vf/VERSION").then(e => e.text())
}
...
function getDefaultImagery() {
	let e = Manager.SETTINGS.getSetting("imagery");
	null == e && Manager.PROVIDER === MapProvider.MAPBOX && (MapboxUtils.generateDefaultImagery(), e = Manager.SETTINGS.getSetting("imagery"));
	var t = e.default;
	let a = e[t];
	return a.endsWith("_token=") && (a += MapHandler.MAP_API), a
}
...
function buildDashboardButton() {
	var e, t, a;
	null != Manager.SETTINGS.getSetting("dashboard") && (e = (new DOMParser).parseFromString(`
            <div id="dashButtonContainer" class="controlBlock expanded" onclick="openDashboard()" style="cursor: pointer;">
                <p>Open dashboard</p>
            </div>
        `, "text/html"), t = document.getElementById("controlContainer"), a = document.getElementById("helpandsearch"), t.insertBefore(e.body.firstChild, a))
}

function openDashboard() {
	var e = Manager.SETTINGS.getSetting("dashboard");
	null != e && window.open(e, "_blank").focus()
}
...
var SEARCH_TYPE, JsonView = function(e) {
	...
	function l(e, t = "All Entries:") {
		e = "string" == typeof e ? JSON.parse(e) : e, t = u({
			value: e,
			key: t,
			type: c(e)
		});
...
class PanelHandler {
	...
	returnToDefault() {
		null != this._defaultHTML && (document.getElementById("sidePanelGeneral").innerHTML = this._defaultHTML), document.getElementById("returnContainer").style.display = "none", window.currentFeature = null, $("#sidePanelInner").tabs("option", "active", 0), Manager.PROVIDER === MapProvider.CESIUM && CesiumUtils.clearSilhouette()
	}
	toggleMode() {
	...
	toggleExpansion() {
		var e = document.getElementById("sidePanel"),
			...
			r = document.getElementById("sliderParent");
		e.classList.contains("small") && (e.classList.contains("collapsed") ? (e.classList.replace("collapsed", "expanded"), a.classList.replace("fa-chevron-left", "fa-chevron-right"), document.getElementById("map").style.width = "calc(100% - 500px)", n.style.visibility = "visible", t.style.visibility = "visible", null != i && (i.style.width = "calc(100% - 540px)"), null != r && r.classList.replace("collapsed", "expanded")) : e.classList.contains("expanded") && (e.classList.replace("expanded", "collapsed"), a.classList.replace("fa-chevron-right", "fa-chevron-left"), document.getElementById("map").style.width = "calc(100% - 28px)", n.style.visibility = "hidden", t.style.visibility = "hidden", null != i && (i.style.width = "calc(100% - 80px)"), null != r) && r.classList.replace("expanded", "collapsed")), MapHandler.MAP.resize()
	}
	addSupportingData(e, n) {
		var t = (n = filterNulls(n)).iri,
			i = n.endpoint,
			e = Manager.findStack(e, n);
		if (console.log("Attempting to contact agent with stack at '" + e + "'..."), console.log("   ...will submit IRI for query '" + t + "'"), null != t && null != e) {
			this.prepareMetaContainers(!0, !0), document.getElementById("metaTreeContainer").innerHTML = "<i>Retrieving data...</i>";
			let a = this;
			return $.getJSON(e + "/feature-info-agent/get", {
				iri: t,
				endpoint: i
			}, function(e) {
				var t;
				null == e || Array.isArray(e) && 0 == e.length || 0 == Object.keys(e).length ? a.showBuiltInData(n) : (t = e.meta, e = e.time, document.getElementById("metaTreeContainer").innerHTML = "", null != t ? (t = JSONFormatter.formatJSON(t), document.getElementById("metaTreeContainer"), Array.isArray(t) && 0 === t.length || "string" == typeof t && "" === t ? this.showBuiltInData(n) : (t = JsonView.renderJSON(t, document.getElementById("metaTreeContainer")), JsonView.expandChildren(t), JsonView.selectiveCollapse(t))) : a.showBuiltInData(n), document.getElementById("metaTimeContainer").innerHTML = "", null != e ? (a.timeseriesHandler.parseData(e), a.timeseriesHandler.showData("metaTimeContainer"), document.getElementById("time-series-select").onchange(null)) : console.warn("No 'time' node found, skipping timeseries visualisation."), a.prepareMetaContainers(!0, null != e))
			}).fail(function() {
				console.warn("Could not get valid response from the agent, will show any in-model content instead..."), a.showBuiltInData(n)
			})
		}
		console.warn("Feature is missing required information to get metadata/timeseries, will show any in-model content instead..."), this.showBuiltInData(n)
	}
	showBuiltInData(e) {
...
class SearchHandler {
	...
	changeProperty(t) {
		...
		null == t ? this.property = this.allSearchProperties[0] : this.allSearchProperties.forEach(e => {
	...
	startSearch() {
		...
		switch (e) {
			case "string":
			case "number":
				var t = document.getElementById("finderField");
				null != t && (t = t.value, this.runSearch(t, e));
				break;
			case "boolean":
				"true" === document.getElementById("finderRangeSelect").value ? this.runSearch(!0, e) : this.runSearch(!1, e)
		}
	}
}! function(e) {
	...
	buildRecurse(e, t, a, n, i) {
		...
		for ([l, s] of Object.entries(d)) {
			...
			switch (a ? e += "style='display:none;'>" : e += ">", e = (e = (e = (e += "<label>") + ("<input class='hummingbird-end-node' id='" + t + "' data-id='" + t + "' type='checkbox'/><span>")) + l) + "</span></label>" + "</li>", r += e, Manager.PROVIDER) {
				...
				case MapProvider.CESIUM:
					var u = s[0],
						p = MapHandler_Cesium.DATA_SOURCES[u.id];
					null == p || 0 === p.length ? null != (p = u.definition.visibility) && "visible" !== p || n.push(t) : u.isVisible() && n.push(t)
			}
		}
		e.push(r);
	...
	async loadIcon(i, r) {
		await new Promise((a, n) => {
			MapHandler.MAP.loadImage(r, (e, t) => {
				e ? (console.log("ERROR: Could not load image at URL " + r), n(e)) : (MapHandler.MAP.addImage(i, t, {
					sdf: r.includes("-sdf")
				}), a(t))
			})
		})
...
class MapHandler_Mapbox extends MapHandler {
	...
	plotData(e) {
		let t = new Set,
			a = (MapHandler.MAP.getStyle().layers.forEach(function(e) {
				"CMCL" === e.metadata?.attribution && (t.add(e.source), MapHandler.MAP.removeLayer(e.id))
			}), t.forEach(e => MapHandler.MAP.removeSource(e)), []);
		e.dataGroups.forEach(e => {
			e = e.flattenDown();
			a = a.concat(e)
		}), (a = a.sort((e, t) => e.order > t.order ? 1 : e.order < t.order ? -1 : 0)).forEach(e => this.plotLayer(null, e))
	}
	plotLayer(e, t) {
		var a = t.source;
	...
	addSource(e) {
		...
		null == t && ((t = {
			...e.definition
		}).id && delete t.id, t.metaFiles && delete t.metaFiles, t.timeseriesFiles && delete t.timeseriesFiles, "video" === e.type || "image" === e.type || t.attribution || (t.attribution = "CMCL"), MapHandler.MAP.addSource(e.id, t), console.info("Added source to Mapbox map: " + e.id))
	}
	addLayer(e) {
		var t = MapHandler.MAP.getLayer(e.id);
		null == t && ((t = {
			...e.definition
		}).metadata || (t.metadata = {}), t.metadata.attribution || (t.metadata.attribution = "CMCL"), t.clickable ? (t.metadata.clickable = t.clickable, delete t.clickable) : t.metadata.clickable = !0, t.treeable ? (t.metadata.treeable = t.treeable, delete t.treeable) : t.metadata.treeable = !0, t.id = e.id, delete t.name, delete t.order, MapHandler.MAP.addLayer(t), console.info("Added layer to Mapbox map '" + e.id + "'."))
	}
	addIcons(e) {
		return $.getJSON(e, function(e) {
			return e
...
class SearchHandler_Mapbox extends SearchHandler {
	constructor() {
		super(...arguments), this.cachedSources = {}
	}
	turnOffClustering() {
		this.cachedSources = {};
	...
	cacheExisting() {
		for (const a of MapHandler.MAP.getStyle().layers) {
			var e, t = a.metadata;
			null == t || null == (t = t.attribution) || "CMCL" !== t || (t = a.id).endsWith("_cluster") || (e = MapHandler.MAP.getFilter(t), SearchHandler_Mapbox.OLD_FILTERS[t] = e)
		}
	}
	runSearch(e, t) {
		if (0 === e.length) Object.entries(SearchHandler_Mapbox.OLD_FILTERS).forEach(([e, t]) => {
			MapHandler.MAP.setFilter(e, t)
		});
		else
			for (const i of MapHandler.MAP.getStyle().layers) {
				var a, n = i.metadata;
				null == n || null == (n = n.attribution) || "CMCL" !== n || (n = i.id).endsWith("_cluster") || (a = SearchHandler_Mapbox.OLD_FILTERS[n], null != (a = this.createFilter(a, e, t)) && MapHandler.MAP.setFilter(n, a))
			}
	}
	cancelSearch() {
		null != SearchHandler_Mapbox.OLD_FILTERS && (Object.entries(SearchHandler_Mapbox.OLD_FILTERS).forEach(([e, t]) => {
			MapHandler.MAP.setFilter(e, t)
		}), SearchHandler_Mapbox.OLD_FILTERS = {}, document.getElementById("finderField").value = "")
	}
	createFilter(e, t, a) {
		var n = this.property.property;
		let i = null;
		var r = document.getElementById("finderRangeSelect");
		if ("boolean" === a) switch (r.value) {
			case "true":
				i = ["all", ["has", n],
					["==", ["to-boolean", ["get", n]], !0]
				];
		...
		} else if ("number" === a) switch (r.value) {
			case "equals":
				i = ["all", ["has", n],
					["==", Number(t), ["to-number", ["get", n]]]
				];
				break;
			case "lesser":
				i = ["all", ["has", n],
					["<", ["to-number", ["get", n]], Number(t)]
				];
				break;
			case "greater":
				i = ["all", ["has", n],
		...
		} else if ("string" === a) switch (t = t.toLowerCase(), r.value) {
			case "contains":
				i = ["all", ["has", n],
					["in", t, ["downcase", ["get", n]]]
				];
			...
			case "starts":
				i = ["all", ["has", n],
					["==", ["index-of", t, ["downcase", ["get", n]]], 0]
				]
		}
		return null === i || void 0 === i ? null : i = null != e ? ["all", e, i] : i
	}
}
...
class MapboxUtils {
	static isCMCLLayer(e) {
		try {
			var t = e.layer;
			return "none" === MapHandler.MAP.getLayoutProperty(t.id, "visibility") ? !1 : !!t.metadata && !(!t.metadata.attribution || "CMCL" !== t.metadata.attribution || !t.metadata.clickable)
		} catch (e) {
			return !1
		}
	}
	...
	static changeTerrain(t) {
		...
		if (null != a) {
			let e = a[t];
			null != e && (e.endsWith("_token=") && (e += MapHandler.MAP_API), MapHandler.MAP.setStyle(e), MapHandler.MAP.setProjection({
				name: "mercator"
			}), window.terrain = t, MapboxUtils.hideBuildings())
		}
	}
...
class MapHandler_Cesium extends MapHandler {
	constructor(e) {
		super(e), this.clipHandler = new ClipHandler;
		e = CesiumUtils.getLayersWithClipping(Manager.DATA_STORE);
		this.clipHandler.setLayers(e)
	}
	initialiseMap(e) {
		var t;
		MapHandler.MAP_OPTIONS = e, null === MapHandler.MAP || void 0 === MapHandler.MAP ? (MapHandler.MAP = new Cesium.Viewer("map", {
			timeline: !1,
			animation: !1,
			baseLayerPicker: !1,
			homeButton: !1,
		...
		}), MapHandler.MAP.scene.requestRender(), CesiumUtils.setupKeyboardShortcuts(), this.addTerrain(), t = CesiumUtils.getLayersWithClipping(Manager.DATA_STORE), this.clipHandler.setLayers(t), this.clipHandler.addControls()) : (MapHandler.MAP.camera.setView({
			destination: Cesium.Cartesian3.fromDegrees(e.center[0], e.center[1], e.center[2]),
			orientation: {
				heading: Cesium.Math.toRadians(e.heading),
				pitch: Cesium.Math.toRadians(e.pitch),
		...
		if (MapHandler.ALLOW_CLICKS) {
			...
			CesiumUtils.getFeature(i, function(a) {
				if ((window.currentFeature = a) instanceof Cesium.ImageryLayerFeatureInfo) {
					...
					let t = {};
					e = a?.content?.metadata;
					if (Cesium.defined(e)) t = {
						...e._properties
					};
					else {
						if ("function" != typeof a.getPropertyIds) return;
						e = a.getPropertyIds();
						null != e && e.forEach(e => {
							t[e] = a.getProperty(e)
						})
					}
					n.manager.showFeature(a, t), CesiumUtils.setSelectedSilhouette(a, i)
				}
			})
		}
	}
	...
	addKMLFile(e, t) {
		Cesium.KmlDataSource.load(e.uri).then(e => {
	...
	addGLTFFile(e, t) {
		var a = e.position;
		"position" in e && console.error("Cannot plot a glTF/glB data source that has no 'position' parameter");
		let n = [0, 0, 0];
		"rotation" in e && (n = e.orientation);
...
class CesiumUtils {
	...
	static toggleLayer(e, t) {
		...
		if (null !== a)
			for (let e = 0; e < a.length; e++) {
				...
				} else if (n.show = t, n instanceof Cesium.Cesium3DTileset)
					if (null == n.clippingPlanes) return
			}
		MapHandler.MAP.scene.requestRender()
	}
    ...
    static generateDefaultImagery() {
        var e = {};
        e.Light = "https://api.mapbox.com/styles/v1/mapbox/light-v11/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e.Dark = "https://api.mapbox.com/styles/v1/mapbox/dark-v11/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e.Outdoors = "https://api.mapbox.com/styles/v1/mapbox/outdoors-v12/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e["Satellite (Raw)"] = "https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e["Custom"] = "https://api.mapbox.com/styles/v1/tianmingchuan/cll0g741u00on01qg017q07ry/tiles/512/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e.Streets = "https://api.mapbox.com/styles/v1/mapbox/streets-v12/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e["Custom2"] = "https://api.mapbox.com/styles/v1/tianmingchuan/cll0hnua2008z01pm7vc5f6qi/tiles/256/{z}/{x}/{y}?access_token=" + MapHandler.MAP_API;
        e.default = "Custom2";
        Manager.SETTINGS.putSetting("imagery", e);
    }
    /*
	...
	*/
	...
	static clearSilhouette() {
		CesiumUtils.OUTLINE_GREEN.selected = []
	}
	static getFeature(e, t) {
		if (!t) throw "Callback function is required!";
		var a = MapHandler.MAP.scene.drillPick(e.position || e.endPosition, 2);
		let n = null;
		null != a && a.forEach(e => {
			(null == e?.id?._name || "clipping-plane" !== e.id._name) && (n = e)
		}), null === n || void 0 === n ? (a = MapHandler.MAP.camera.getPickRay(e.position || e.endPosition), e = MapHandler.MAP.imageryLayers.pickImageryLayerFeatures(a, MapHandler.MAP.scene), Cesium.defined(e) && Promise.resolve(e).then(function(e) {
			0 < e.length && t(e[0])
		})) : t(n)
	}
	...
	static getLayersWithClipping(e) {
		let t = {};
		return e.dataGroups.forEach(e => {
			this.recurseLayersWithClipping(e, t)
		}), t
	}
	static recurseLayersWithClipping(e, a) {
		e.dataLayers.forEach(e => {
			var t;
			e.definition.hasOwnProperty("clipping") && (t = e.id, e = e.definition.name, a.hasOwnProperty(e) ? a[e] = a[e] + "|" + t : a[e] = t)
		}), 0 < e.subGroups.length && e.subGroups.forEach(e => {
			this.recurseLayersWithClipping(e, a)
		})
	}
	static getPrimitive(t) {
		var a = MapHandler.MAP.scene.primitives;
		for (let e = 0; e < a.length; e++)
			if (a.get(e)?.layerID === t) return a.get(e);
		return null
	}
}
class ClipHandler {
	...
	addControls() {
		let n = this;
		if (null == document.getElementById("clipControlContainer")) {
			var i = document.createElement("div"),
				r = (i.id = "clipControlContainer", i.classList.add("controlBlock"), i.innerHTML = `
                <div id="controlTitle" class="controlTitle">
                    <p style="width: 100%;">Clipping Planes</p>
                </div>
            `, document.createElement("div")),
				l = (r.classList.add("controlContents"), i.appendChild(r), document.getElementById("controlContainer")),
				s = document.getElementById("helpContainer"),
				l = (l.insertBefore(i, s), document.createElement("div")),
				i = (l.id = "clipEnableContainer", r.appendChild(l), document.createElement("p"));
			i.innerHTML = "Enable clipping planes?", l.appendChild(i);
			let e = document.createElement("input");
			e.type = "checkbox", e.id = "clipEnableCheck", l.appendChild(e), e.addEventListener("change", function() {
				n.onEnableChange(e.checked)
			});
			s = document.createElement("div"), i = (s.id = "clipShowContainer", s.style.display = "none", r.appendChild(s), document.createElement("p"));
			i.innerHTML = "Show plane geometry?", s.appendChild(i);
			let t = document.createElement("input");
			t.type = "checkbox", t.checked = !0, t.id = "clipShowCheck", s.appendChild(t), t.addEventListener("change", function() {
				n.onShowGeometryChange(t.checked)
			});
			l = document.createElement("div"), i = (l.id = "clipSelectContainer", l.style.display = "none", r.appendChild(l), document.createElement("p"));
			i.innerHTML = "Target layer:", l.appendChild(i);
			let a = document.createElement("select");
			a.id = "clipSelectCombo", l.appendChild(a);
			s = document.createElement("option");
			s.selected = !0, s.disabled = !0, s.hidden = !0, s.text = "Please select a layer...", a.appendChild(s), a.addEventListener("change", function() {
				n.onTargetLayerChange(a.value)
			})
		} else {
			let e = document.getElementById("clipEnableCheck"),
				t = (e.addEventListener("change", function() {
					n.onEnableChange(e.checked)
				}), document.getElementById("clipShowCheck")),
				a = (t.addEventListener("change", function() {
					n.onShowGeometryChange(t.checked)
				}), document.getElementById("clipSelectCombo"));
			a.addEventListener("change", function() {
				n.onTargetLayerChange(a.value)
			})
		}
		var e, t = document.getElementById("clipSelectCombo");
		for (e in this.layers) {
			var a = document.createElement("option");
			a.value = this.layers[e], a.text = e, t.appendChild(a)
		}
	}
	onEnableChange(e) {
		var t = document.getElementById("clipShowContainer"),
			a = document.getElementById("clipSelectContainer"),
			n = document.getElementById("clipSelectCombo");
		null != t && (t.style.display = e ? "block" : "none"), null != a && (a.style.display = e ? "block" : "none"), e ? 0 !== (t = document.getElementById("clipSelectCombo")).selectedIndex ? this.onTargetLayerChange(n.value) : 2 === $("#clipSelectCombo option").length && (t.selectedIndex = 1, this.onTargetLayerChange(n.value)) : (a = document.getElementById("sliderParent"), document.body.removeChild(a), e = ClipHandler.SELECTED_PLANE.id, null != (t = CesiumUtils.getPrimitive(e))?.clippingPlanes && (t.clippingPlanes.enabled = !1, ClipHandler.GEOMETRIES[e].plane.show = !1), ClipHandler.SELECTED_PLANE = null)
	}
	onShowGeometryChange(e) {
		var t;
		null != ClipHandler.SELECTED_PLANE && (t = ClipHandler.SELECTED_PLANE.id, ClipHandler.GEOMETRIES[t].plane.show = e)
	}
	onTargetLayerChange(e) {
		var t, a;
		console.log("Changing target layers for clipping planes to: " + e), null != ClipHandler.SELECTED_PLANE && (t = ClipHandler.SELECTED_PLANE.id, null != (a = CesiumUtils.getPrimitive(t))?.clippingPlanes) && (a.clippingPlanes.enabled = !1, ClipHandler.GEOMETRIES[t].plane.show = !1);
		let r = !1;
		(e.includes("|") ? e.split("|") : [e]).forEach(t => {
			var a = Manager.DATA_STORE.getLayerWithID(t);
			if (!r && null != a && a.definition.hasOwnProperty("clipping")) {
				var n = a.source;
				"position" in n.definition ? ClipHandler.OFFSET = n.definition.position[2] : ClipHandler.OFFSET = 0;
				let e = a.definition.clipping.start;
				null != ClipHandler.PLANES[t] && (e = ClipHandler.PLANES[t].get(0).distance, e += ClipHandler.OFFSET), this.createSlider(a.definition.clipping.min, a.definition.clipping.max, e, a.definition.clipping.labels), this.labels = a.definition.clipping.labels;
				var n = CesiumUtils.getPrimitive(t),
					i = ClipHandler.PLANES[t];
				null == n.clippingPlanes ? n.clippingPlanes = i : n.clippingPlanes.enabled = !0, null == ClipHandler.GEOMETRIES[t] ? (this.linkClippingPlane(t, n, a.definition.clipping), ClipHandler.SELECTED_PLANE = ClipHandler.PLANES[t].get(0)) : (ClipHandler.SELECTED_PLANE = ClipHandler.PLANES[t].get(0), i = document.getElementById("clipShowCheck"), ClipHandler.GEOMETRIES[t].plane.show = i.checked), r = !0
			}
		})
	}
	initialiseClippingPlane(e, t) {
		var a = Manager.DATA_STORE.getLayerWithID(e).source,
			a = ("position" in a.definition && (t -= a.definition.position[2]), new Cesium.ClippingPlane(new Cesium.Cartesian3(0, 0, -1), t)),
			t = (a.id = e, new Cesium.ClippingPlaneCollection({
				planes: [a],
				edgeWidth: 0
			}));
		return ClipHandler.PLANES[e] = t
	}
	linkClippingPlane(i, r, e) {
		let l = ClipHandler.PLANES[i],
			s = l.get(0),
			o = e.start;
		r.readyPromise.then(function() {
			var e = r.boundingSphere.radius,
				t = (Cesium.Matrix4.equals(r.root.transform, Cesium.Matrix4.IDENTITY) || (l.modelMatrix = Cesium.Matrix4.fromTranslation(new Cesium.Cartesian3(0, 0, o))), r.boundingSphere.center),
				t = Cesium.Cartographic.fromCartesian(t),
				t = Cesium.Cartesian3.fromRadians(t.longitude, t.latitude, 0),
				t = Cesium.Matrix4.multiplyByPoint(Cesium.Transforms.eastNorthUpToFixedFrame(t), new Cesium.Cartesian3(0, 0, ClipHandler.OFFSET), new Cesium.Cartesian3),
				a = Manager.DATA_STORE.getLayerWithID(i).source;
			let n = null;
			"rotation" in a.definition && (a = a.definition.rotation, a = new Cesium.HeadingPitchRoll(a[2], a[1], a[0]), n = Cesium.Transforms.headingPitchRollQuaternion(t, a));
			a = document.getElementById("clipShowCheck"), a = null == a || a.checked, t = MapHandler.MAP.entities.add({
				position: t,
				orientation: n,
				id: i,
				show: a,
				plane: {
					dimensions: new Cesium.Cartesian2(1.41 * e, 1.41 * e),
					material: Cesium.Color.LIGHTSKYBLUE.withAlpha(.33),
					outline: !0,
					outlineColor: Cesium.Color.LIGHTSKYBLUE.withAlpha(.66),
					plane: new Cesium.CallbackProperty(ClipHandler.createPlaneUpdateFunction(s, i), !1),
					show: a
				}
			});
			return t.name = "clipping-plane", t.plane.name = "clipping-plane", ClipHandler.GEOMETRIES[i] = t, r
		})
	}
	resetSlider() {
		var e = document.getElementById("sliderParent");
		null != e && document.body.removeChild(e)
	}
	createSlider(e, t, a, n) {
		this.minValue = e, this.maxValue = t, this.startValue = a, this.resetSlider();
		var i = document.createElement("div"),
			t = (i.id = "sliderParent", i.classList.add("expanded"), i.innerHTML = `
            <div id="sliderTop">
                <div id="sliderLeft">
                </div>
                <div id="sliderRight">
        ...
        `, document.body.appendChild(i), this.slider = $("#sliderRight").slider({
				min: e,
				max: t,
				value: a,
				step: (t - e) / 1e3,
		...
		t.value = a, t.addEventListener("change", this.updateSlider), this.addLabels(n);
		let r = this;
		new ResizeObserver(function() {
			r.addLabels(r.labels)
		}).observe(i)
	}
	addLabels(e) {
		var n, i = this.maxValue - this.minValue,
			r = $("#sliderRight").height(),
			l = document.getElementById("sliderLeft");
		for (n in null != l && (l.innerHTML = ""), e) {
			var s = Number(n),
				o = (s - this.minValue) / i * r;
			let t = document.createElement("div"),
				a = (t.id = String(s), t.classList.add("sliderStep"), t.style.top = String(r - o + 20), t.innerHTML = e[n], l.appendChild(t), this);
			t.addEventListener("click", function() {
				var e = Number(t.id);
				a.slider.slider("value", e)
			})
		}
	}
	updateInput() {
		var e = document.getElementById("sliderInput"),
			t = Number($("#sliderRight").slider("value")).toFixed(2);
		e.value = String(t), ClipHandler.movePlane(Number(e.value) - ClipHandler.OFFSET)
	}
	updateSlider() {
		var e = document.getElementById("sliderInput"),
			e = Number(e.value).toFixed(2);
		$("#sliderRight").slider("value", e), ClipHandler.movePlane(Number(e) - ClipHandler.OFFSET)
	}
	static movePlane(e) {
		var t;
		Cesium.defined(ClipHandler.SELECTED_PLANE) ? (t = ClipHandler.SELECTED_PLANE.id._id, ClipHandler.HEIGHTS[t] = e, ClipHandler.SELECTED_PLANE.distance = e) : console.error("Clipping plane for this layer has not been defined.")
	}
	static createPlaneUpdateFunction(t, a) {
		return function() {
			if (Cesium.defined(ClipHandler.SELECTED_PLANE)) {
				var e = ClipHandler.SELECTED_PLANE.id._id;
				if (a !== e) return t;
				e = ClipHandler.HEIGHTS[a];
				t.distance = e
			}
			return t
		}
...
ClipHandler.PLANES = {}, ClipHandler.HEIGHTS = {}, ClipHandler.GEOMETRIES = {}, ClipHandler.OFFSET = 0;